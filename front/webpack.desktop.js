...
module.exports = (env, argv) => {
	return {
		...
		optimization: {
			minimize: true,
			minimizer: [new EsbuildPlugin({ target: 'es6' })]
		},
		resolve: {
			fallback: {
		...
		module: {
			rules: [
				{
					test: /\.tpl/,
					include: [path.resolve(__dirname, 'src/assets/template/desktop/index.tpl')],
					loader: 'html-loader',
					options: {
						sources: false
					}
		...
		plugins: [
			...
			new CleanWebpackPlugin({
				...
				cleanOnceBeforeBuildPatterns: [path.join(__dirname, 'stage/build/desktop')]
			}),
