type TLayout = 'normal' | 'bottom' | 'left' | 'right' | 'center' | string;
type TSearchFilter =
	| 'mathBlock'
	| 'table'
	| 'blockquote'
...
interface IOpenFileOptions {
	...
	custom?: {
		...
		data?: any;
		id: string;
		fn?: (options: { tab: import('../layout/Tab').Tab; data: any }) => import('../layout/Model').Model; // plugin 0.8.3 历史兼容
	};
