...
const renderCompare = (app: App, element: HTMLElement) => {
	...
	fetchPost('/api/repo/openRepoSnapshotDoc', { id: element.getAttribute('data-id') }, (response) => {
		...
		if (
			Constants.SIYUAN_ASSETS_IMAGE.concat(Constants.SIYUAN_ASSETS_AUDIO)
			...
			leftEditor.protyle.options.history.snapshot = dialogContainerElement
				.querySelector('.b3-dialog__header code')
		...
		fetchPost('/api/repo/openRepoSnapshotDoc', { id: id2 }, (response) => {
			...
			if (
				...
				rightEditor.protyle.options.history.snapshot = dialogContainerElement
					.querySelectorAll('.b3-dialog__header code')[1]
					.getAttribute('data-snapshot');
				onGet({
					data: response,
			...
			rightElement.querySelector('.history__date').textContent = dayjs(response.data.updated).format(
				'YYYY-MM-DD HH:mm'
			);
		});
	} else {
...
const genHTML = (left: string, right: string, dialog: Dialog, direct: string) => {
    ...
    <span class="block__icon block__icon--show b3-tooltips b3-tooltips__s" aria-label="${
			window.siyuan.languages.switchDirect
		}" data-direct="${direct}"><svg><use xlink:href="#iconScrollHoriz"></use></svg></span>
    <span class="fn__space"></span>
    ...
    <div class="history__diff">
        <ul class="b3-list b3-list--background">
            <li class="b3-list-item">
                ...
                <span class="counter${response.data.updatesLeft.length === 0 ? ' fn__none' : ''}">${
        ...
        <ul class="b3-list b3-list--background">
            <li class="b3-list-item">
                ...
                <span class="counter${response.data.addsLeft.length === 0 ? ' fn__none' : ''}">${
        ...
        <ul class="b3-list b3-list--background">
            <li class="b3-list-item">
                ...
                <span class="counter${response.data.removesRight.length === 0 ? ' fn__none' : ''}">${
