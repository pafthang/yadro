...
export const globalTouchStart = (event: TouchEvent) => {
	...
	if (
		backgroundElement &&
		target.tagName === 'IMG' &&
		backgroundElement.firstElementChild.querySelector('.protyle-icons').classList.contains('fn__none')
	) {
		...
		const height =
			((target as HTMLImageElement).naturalHeight * target.clientWidth) / (target as HTMLImageElement).naturalWidth -
			target.clientHeight;
		...
		documentSelf.ontouchmove = (moveEvent) => {
			target.style.objectPosition = `center ${(
				((y - moveEvent.touches[0].clientY) / height) * 100 +
				originalPositionY
			).toFixed(2)}%`;
		};
...
export const globalTouchEnd = (event: TouchEvent, yDiff: number, time: number, app: App) => {
	...
	if (typeof yDiff === 'undefined' && new Date().getTime() - time > 900) {
		...
		const fileItemElement =
			hasClosestByAttribute(target, 'data-type', 'navigation-root') ||
			hasClosestByAttribute(target, 'data-type', 'navigation-file');
		if (fileItemElement) {
