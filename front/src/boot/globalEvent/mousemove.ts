...
export const windowMouseMove = (event: MouseEvent & { target: HTMLElement }, mouseIsEnter: boolean) => {
	...
	if (
		...
		if (event.clientX < Math.max(document.getElementById('dockLeft').clientWidth + 1, 16)) {
			if (
				!window.siyuan.layout.leftDock.pin &&
				window.siyuan.layout.leftDock.layout.element.clientWidth > 0 &&
				// 隐藏停靠栏会导致点击两侧内容触发浮动面板弹出，因此需减小鼠标范围
				(window.siyuan.layout.leftDock.element.clientWidth > 0 ||
					(window.siyuan.layout.leftDock.element.clientWidth === 0 && event.clientX < 8))
			) {
				if (
					event.clientY > document.getElementById('toolbar').clientHeight &&
					event.clientY <
						window.innerHeight -
							document.getElementById('status').clientHeight -
							document.getElementById('dockBottom').clientHeight
				) {
					if (
			...
			if (
				!window.siyuan.layout.rightDock.pin &&
				window.siyuan.layout.rightDock.layout.element.clientWidth > 0 &&
				(window.siyuan.layout.rightDock.element.clientWidth > 0 ||
					(window.siyuan.layout.rightDock.element.clientWidth === 0 && event.clientX > window.innerWidth - 8))
			) {
				if (
					event.clientY > document.getElementById('toolbar').clientHeight &&
					event.clientY <
		...
		if (
			...
			Math.min(
				...
				window.innerHeight -
					(window.siyuan.config.uiLayout.hideDock ? 0 : document.getElementById('dockBottom').clientHeight) -
					document.querySelector('#status').clientHeight
			)
		) {
	...
	if (
		eventPath0 &&
		eventPath0.nodeType !== 3 &&
		eventPath0.classList.contains('protyle-wysiwyg') &&
		eventPath0.style.paddingLeft
	) {
		// 光标在编辑器右边也需要进行显示
		const mouseElement = document.elementFromPoint(
			eventPath0.getBoundingClientRect().left + parseInt(eventPath0.style.paddingLeft) + 13,
			event.clientY
		);
		const blockElement = hasClosestBlock(mouseElement);
		if (blockElement) {
			const targetBlockElement = getRightBlock(
				blockElement,
				blockElement.getBoundingClientRect().left + 1,
				event.clientY
			);
			...
			allModels.editor.find((item) => {
				if (item.editor.protyle.wysiwyg.element.isSameNode(eventPath0)) {
					item.editor.protyle.gutter.render(
						item.editor.protyle,
						targetBlockElement,
						item.editor.protyle.wysiwyg.element,
						rowElement
					);
					findNode = true;
					return true;
	...
	if (
		eventPath0 &&
		eventPath0.nodeType !== 3 &&
		(eventPath0.classList.contains('li') || eventPath0.classList.contains('list'))
	) {
	...
	if (
		blockElement &&
		blockElement.style.cursor !== 'col-resize' &&
		!hasClosestByClassName(blockElement, 'protyle-wysiwyg__embed')
	) {
		const cellElement = (hasClosestByMatchTag(target, 'TH') ||
			hasClosestByMatchTag(target, 'TD')) as HTMLTableCellElement;
		if (cellElement) {
			...
			if (rect.right - event.clientX < 3 && rect.right - event.clientX > 0) {
				...
				resizeElement.setAttribute(
					'style',
					`height:${tableHeight}px;left: ${Math.round(
						cellElement.offsetWidth + cellElement.offsetLeft - blockElement.firstElementChild.scrollLeft - 3
					)}px;display:block`
				);
				...
				resizeElement.setAttribute(
					'style',
					`height:${tableHeight}px;left: ${Math.round(
						cellElement.offsetLeft - blockElement.firstElementChild.scrollLeft - 3
					)}px;display:block`
				);
			}
		}
