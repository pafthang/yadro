...
const dialogArrow = (app: App, element: HTMLElement, event: KeyboardEvent) => {
	...
	if (currentLiElement) {
		...
		if (event.key === 'ArrowUp') {
			...
			const sideElement =
				currentLiElement.parentElement.previousElementSibling || currentLiElement.parentElement.nextElementSibling;
			if (sideElement) {
				const tempLiElement =
					sideElement.querySelector(`[data-index="${currentLiElement.getAttribute('data-index')}"]`) ||
					sideElement.lastElementChild;
				if (tempLiElement) {
...
const editKeydown = (app: App, event: KeyboardEvent) => {
	...
	if (!isFileFocus && matchHotKey(window.siyuan.config.keymap.editor.general.spaceRepetition.custom, event)) {
		fetchPost('/api/riff/getTreeRiffDueCards', { rootID: protyle.block.rootID }, (response) => {
			openCardByData(
				...
				protyle.title.editElement.textContent || 'Untitled'
			);
		});
		event.preventDefault();
		return true;
	...
	if (matchHotKey(window.siyuan.config.keymap.editor.general.switchReadonly.custom, event)) {
		event.preventDefault();
		updateReadonly(
			protyle.breadcrumb.element.parentElement.querySelector('.block__icon[data-type="readonly"]'),
			protyle
		);
		return true;
	}
...
const fileTreeKeydown = (app: App, event: KeyboardEvent) => {
	...
	if (matchHotKey(window.siyuan.config.keymap.general.selectOpen1.custom, event)) {
		event.preventDefault();
		const element =
			document.querySelector('.layout__wnd--active > .fn__flex > .layout-tab-bar > .item--focus') ||
		...
		if (isFile) {
			...
			fetchPost('/api/riff/getTreeRiffDueCards', { rootID: id }, (response) => {
				openCardByData(
					...
					'doc',
					id,
					getDisplayName(liElements[0].getAttribute('data-name'), false, true)
				);
			});
		} else {
	...
	if (event.shiftKey) {
		if (event.key === 'ArrowUp') {
			...
			if (
				startEndElement.startElement.getBoundingClientRect().top >=
				startEndElement.endElement.getBoundingClientRect().top
			) {
		...
		if (
			(event.key === 'ArrowRight' &&
				!liElements[0].querySelector('.b3-list-item__arrow--open') &&
				!liElements[0].querySelector('.b3-list-item__toggle').classList.contains('fn__hidden')) ||
...
const panelTreeKeydown = (app: App, event: KeyboardEvent) => {
	...
	if (
		!matchHotKey(window.siyuan.config.keymap.editor.general.collapse.custom, event) &&
	...
	if (
		activePanelElement.classList.contains('sy__inbox') ||
		activePanelElement.classList.contains('sy__globalGraph') ||
		activePanelElement.classList.contains('sy__graph')
	) {
		return false;
	}
	const model = (getInstanceById(activePanelElement.getAttribute('data-id'), window.siyuan.layout.layout) as Tab)
		?.model;
	if (!model) {
		return false;
	}
	...
	if (
		(event.key === 'ArrowRight' &&
			...
			!arrowElement.parentElement.classList.contains('fn__hidden')) ||
		(event.key === 'ArrowLeft' &&
			arrowElement.classList.contains('b3-list-item__arrow--open') &&
			!arrowElement.parentElement.classList.contains('fn__hidden'))
	) {
	...
	if (event.key === 'ArrowUp') {
		...
		while (previousElement) {
			if (previousElement.previousElementSibling) {
				if (previousElement.previousElementSibling.tagName === 'LI') {
					...
					previousElement.previousElementSibling.tagName === 'UL' &&
					previousElement.previousElementSibling.classList.contains('fn__none')
				) {
					// 遇到折叠内容
					if (previousElement.previousElementSibling.previousElementSibling) {
						previousElement = previousElement.previousElementSibling.previousElementSibling;
		...
		if (
			previousElement.classList.contains('b3-list-item') &&
			!previousElement.classList.contains('b3-list-item--focus')
		) {
			activeItemElement.classList.remove('b3-list-item--focus');
			previousElement.classList.add('b3-list-item--focus');
			const previousRect = previousElement.getBoundingClientRect();
...
export const windowKeyDown = (app: App, event: KeyboardEvent) => {
	...
	if (
		...
		!event.altKey &&
		!['INPUT', 'TEXTAREA'].includes(target.tagName) &&
		['0', '1', '2', '3', '4', 'j', 'k', 'l', ';', 's', ' ', 'p', 'enter', 'a', 's', 'd', 'f', 'q', 'x'].includes(
	...
	if (!event.altKey && !event.shiftKey && isOnlyMeta(event)) {
		if ((isMac() ? event.key === 'Meta' : event.key === 'Control') || isOnlyMeta(event)) {
			...
			if (
				...
				window.siyuan.config.editor.floatWindowMode === 1 &&
				!event.repeat
			) {
				showPopover(app);
			}
	...
	if (
		matchHotKey(window.siyuan.config.keymap.general.goToEditTabNext.custom, event) ||
		...
		if (currentTabElement) {
			...
			getAllTabs()
				.sort((itemA, itemB) => {
					return itemA.headElement.getAttribute('data-activetime') > itemB.headElement.getAttribute('data-activetime')
						? -1
						: 1;
				})
				.forEach((item, index) => {
					...
					tabHtml += `<li data-index="${index}" data-id="${item.id}"${rootId} class="b3-list-item${
						currentId === item.id ? ' b3-list-item--focus' : ''
					}"${
						currentId === item.id ? ' data-original="true"' : ''
					}>${icon}<span class="b3-list-item__text">${escapeHtml(item.title)}</span></li>`;
				});
		}
		let dockHtml = '';
		if (!isTabWindow) {
...
<li data-type="riffCard" data-index="0" class="b3-list-item${!tabHtml ? ' b3-list-item--focus' : ''}">
    ...
    <div class="fn__flex" style="overflow:auto;">${dockHtml}
        <ul${
					!isTabWindow ? '' : ' style="border-left:0"'
				} class="b3-list b3-list--background fn__flex-1">${tabHtml}</ul>
    </div>
	...
	if (['Home', 'End', 'ArrowUp', 'ArrowDown'].includes(event.key)) {
		...
		window.siyuan.dialogs.forEach((item) => {
			if (
				[Constants.DIALOG_VIEWCARDS, Constants.DIALOG_HISTORYCOMPARE].includes(item.element.getAttribute('data-key'))
			) {
				matchDialog = item;
			}
		});
	...
	if (
		...
		matchHotKey(window.siyuan.config.keymap.general.config.custom, event)
	) {
		openSetting(app);
		event.preventDefault();
		return;
	...
	if (event.key === 'Escape' && !event.isComposing) {
		...
		if (!window.siyuan.menus.menu.element.classList.contains('fn__none')) {
			if (
				window.siyuan.dialogs.length > 0 &&
				window.siyuan.menus.menu.element.style.zIndex <
					(window.siyuan.dialogs[0].element.querySelector('.b3-dialog') as HTMLElement).style.zIndex
			) {
	...
	if (
		(matchHotKey(window.siyuan.config.keymap.general.closeLeft.custom, event) ||
			matchHotKey(window.siyuan.config.keymap.general.closeRight.custom, event)) &&
		!event.repeat
	) {
		const tab = getActiveTab(false);
		if (tab) {
	...
	if (
		...
		if (tab) {
			if (matchHotKey(window.siyuan.config.keymap.general.tabToWindow.custom, event)) {
				...
				const newWnd = tab.parent.split(
					matchHotKey(window.siyuan.config.keymap.general.splitMoveB.custom, event) ? 'tb' : 'lr'
				);
	...
	if (matchHotKey(window.siyuan.config.keymap.general.replace.custom, event)) {
		...
		!hasClosestByClassName(target, 'pdf__outer') &&
