...
export const searchKeydown = (app: App, event: KeyboardEvent) => {
	...
	const searchType = assetsElement.classList.contains('fn__none')
		? unRefElement.classList.contains('fn__none')
			? 'doc'
			: 'unRef'
		: 'asset';
	const listElement =
		searchType === 'asset'
			? assetsElement.querySelector('#searchAssetList')
			: searchType === 'doc'
			? element.querySelector('#searchList')
			: unRefElement.querySelector('#searchUnRefList');
	...
	if (Constants.KEYCODELIST[event.keyCode] === 'PageDown') {
		if (searchType === 'asset') {
			...
			if (!nextElement.getAttribute('disabled')) {
				if (
					config.page <
					parseInt(nextElement.parentElement.querySelector('#searchResult').getAttribute('data-pagecount'))
				) {
					config.page++;
					inputEvent(element, config, edit);
				}
		...
		if (!currentList.nextElementSibling) {
			if (config.group === 1 && searchType === 'doc') {
				if (currentList.parentElement.nextElementSibling) {
					currentList.parentElement.nextElementSibling.nextElementSibling.firstElementChild.classList.add(
						'b3-list-item--focus'
					);
				} else {
		...
		if (!currentList.previousElementSibling) {
			if (config.group === 1 && searchType === 'doc') {
				if (currentList.parentElement.previousElementSibling.previousElementSibling) {
					currentList.parentElement.previousElementSibling.previousElementSibling.lastElementChild.classList.add(
						'b3-list-item--focus'
					);
				} else {
