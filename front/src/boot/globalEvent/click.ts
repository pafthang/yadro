...
export const globalClick = (event: MouseEvent & { target: HTMLElement }) => {
	if (
		!window.siyuan.menus.menu.element.contains(event.target) &&
		!hasClosestByAttribute(event.target, 'data-menu', 'true')
	) {
		if (
			getSelection().rangeCount > 0 &&
			window.siyuan.menus.menu.element.contains(getSelection().getRangeAt(0).startContainer) &&
	...
	// 点击空白，pdf 搜索、更多消失
	if (
		hasClosestByAttribute(event.target, 'id', 'secondaryToolbarToggle') ||
	...
	getAllModels().asset.find((item) => {
		if (item.pdfObject && !item.pdfObject.appConfig.appContainer.classList.contains('fn__none')) {
	...
	if (!currentPDFViewerObject.supportsIntegratedFind && currentPDFViewerObject.findBar.opened) {
