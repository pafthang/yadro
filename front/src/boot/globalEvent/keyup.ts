...
export const windowKeyUp = (app: App, event: KeyboardEvent) => {
	...
	if (switchDialog && switchDialog.element.parentElement) {
		if (
			window.siyuan.config.keymap.general.goToEditTabNext.custom.endsWith(Constants.KEYCODELIST[event.keyCode]) ||
			...
			if (currentLiElement) {
				...
				if (rootId) {
					...
					currentLiElement.parentElement.parentElement.nextElementSibling.innerHTML =
						currentLiElement.querySelector('.b3-list-item__text').innerHTML;
				}
			...
			window.siyuan.config.keymap.general.goToEditTabNext.custom.startsWith(Constants.KEYCODELIST[event.keyCode]) ||
