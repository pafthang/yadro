...
export class Dialog {
	...
	constructor(options: {
		...
		if (!isMobile() && options.positionId) {
			...
			if (dialogPosition) {
				if (
					dialogPosition.left + dialogPosition.width + 34 <= window.innerWidth &&
		...
		this.element.innerHTML = `<div class="b3-dialog" style="z-index: ${++window.siyuan.zIndex};${
			typeof left === 'string' ? 'display:block' : ''
		}">
<div class="b3-dialog__scrim"${options.transparent ? 'style="background-color:transparent"' : ''}></div>
<div class="b3-dialog__container" style="width:${options.width || 'auto'};height:${
			options.height || 'auto'
		};left:${left};top:${top}">
  <svg ${isMobile() && options.title ? 'style="top:0;right:0;"' : ''} class="b3-dialog__close${
			this.disableClose || options.hideCloseIcon ? ' fn__none' : ''
		}"><use xlink:href="#iconCloseRound"></use></svg>
  <div class="resize__move b3-dialog__header${options.title ? '' : ' fn__none'}" onselectstart="return false;">${
			options.title || ''
		}</div>
  ...
  <div class="resize__rd"></div><div class="resize__ld"></div><div class="resize__lt"></div><div class="resize__rt"></div><div class="resize__r"></div><div class="resize__d"></div><div class="resize__t"></div><div class="resize__l"></div>
	...
	public destroy(options?: IObject) {
		...
		if (
			(this.element.querySelector('.b3-dialog') as HTMLElement).style.zIndex <
