...
export const moveResize = (element: HTMLElement, afterCB?: (type: string) => void) => {
	element.addEventListener('mousedown', (event: MouseEvent & { target: HTMLElement }) => {
		...
		if (!iconsElement) {
			...
			y = event.clientY;
			iconsElement =
				hasClosestByClassName(event.target, 'resize__rd') ||
		...
		documentSelf.onmousemove = (moveEvent: MouseEvent) => {
			...
			if (type === 'move') {
				...
				if (type === 'r' && moveEvent.clientX - x + width > 200 && moveEvent.clientX - x + width < window.innerWidth) {
					element.style.width = moveEvent.clientX - x + width + 'px';
					element.style.maxWidth = 'none';
				} else if (
					type === 'd' &&
					moveEvent.clientY - y + height > 160 &&
					moveEvent.clientY - y + height < window.innerHeight - Constants.SIZE_TOOLBAR_HEIGHT
				) {
					element.style.height = moveEvent.clientY - y + height + 'px';
					element.style.maxHeight = '';
				} else if (
					type === 't' &&
					moveEvent.clientY > Constants.SIZE_TOOLBAR_HEIGHT &&
					y - moveEvent.clientY + height > 160
				) {
					element.style.top = moveEvent.clientY + 'px';
					element.style.maxHeight = '';
					element.style.height = y - moveEvent.clientY + height + 'px';
				} else if (type === 'l' && moveEvent.clientX > 0 && x - moveEvent.clientX + width > 200) {
					element.style.left = moveEvent.clientX + 'px';
					element.style.width = x - moveEvent.clientX + width + 'px';
					element.style.maxWidth = 'none';
				} else if (
					type === 'rd' &&
					moveEvent.clientX - x + width > 200 &&
					moveEvent.clientX - x + width < window.innerWidth &&
					moveEvent.clientY - y + height > 160 &&
					moveEvent.clientY - y + height < window.innerHeight - Constants.SIZE_TOOLBAR_HEIGHT
				) {
					element.style.height = moveEvent.clientY - y + height + 'px';
