...
export const progressLoading = (data: IWebSocketData) => {
	...
	if (!progressElement) {
		document.body.insertAdjacentHTML(
			'beforeend',
			`<div id="progress" style="z-index: ${++window.siyuan.zIndex}"></div>`
		);
...
<div class="b3-dialog__loading">
    ...
    <div style="margin: 8px 0;height: 8px;border-radius: var(--b3-border-radius);overflow: hidden;background-color:#fff;"><div style="width: ${
			(data.data.current / data.data.total) * 100
		}%;transition: var(--b3-transition);background-color: var(--b3-theme-primary);height: 8px;"></div></div>
    <div>${data.msg}</div>
</div>`;
	} else if (data.code === 1) {
...
export const downloadProgress = (data: { id: string; percent: number }) => {
	...
	if (btnElement) {
		if (data.percent >= 1) {
			...
			btnElement.parentElement.nextElementSibling.firstElementChild.innerHTML = `<span style="width: ${
				data.percent * 100
			}%"></span>`;
		}
	}
