...
export const showMessage = (message: string, timeout = 6000, type = 'info', messageId?: string) => {
	...
	if (existElement) {
		...
		existElement.innerHTML = `<div class="b3-snackbar__content${
			timeout === 0 ? ' b3-snackbar__content--close' : ''
		}">${messageVersion}</div>${
			timeout === 0 ? '<svg class="b3-snackbar__close"><use xlink:href="#iconCloseRound"></use></svg>' : ''
		}`;
		if (type === 'error') {
	...
	let messageHTML = `<div data-id="${id}" class="b3-snackbar--hide b3-snackbar${
		type === 'error' ? ' b3-snackbar--error' : ''
	}"><div class="b3-snackbar__content${timeout === 0 ? ' b3-snackbar__content--close' : ''}">${messageVersion}</div>`;
	if (timeout === 0) {
	...
	if (
		messagesElement.firstElementChild.nextElementSibling &&
