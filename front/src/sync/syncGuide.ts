				...
				switch (type) {
					...
					case 'removeCloud':
						confirmDialog(
							...
							`${window.siyuan.languages.confirmDeleteCloudDir} <i>${target.parentElement.getAttribute(
								'data-name'
							)}</i>`,
							() => {
								cloudPanelElement.innerHTML =
    ...
    <li class="b3-list--empty">
        ${window.siyuan.languages.cloudConfigTip}
			...
			response.data.syncDirs.forEach((item: { hSize: string; cloudName: string; updated: string }) => {
				/// #if MOBILE
				syncListHTML += `<li data-type="selectCloud" data-name="${
					item.cloudName
...
<span class="fn__flex-1 fn__space"></span>
<span data-type="removeCloud" class="b3-tooltips b3-tooltips__w b3-list-item__action" aria-label="${
	...
	if (0 === window.siyuan.config.sync.provider && needSubscribe('') && app) {
		...
		if (window.siyuan.user) {
			...
			dialogSetting.element
				.querySelector('.config__tab-container[data-name="account"]')
				.setAttribute('data-action', 'go-repos');
		}
		return;
	}
