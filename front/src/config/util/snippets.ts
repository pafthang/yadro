...
export const renderSnippet = () => {
	fetchPost('/api/snippet/getSnippet', { type: 'all', enabled: 2 }, (response) => {
		response.data.snippets.forEach((item: ISnippet) => {
			...
			if (
				(!window.siyuan.config.snippet.enabledCSS && item.type === 'css') ||
...
<div class="fn__flex-1" style="overflow:auto;padding: 16px 24px">
    <div>
        <div class="fn__flex">
            ...
            <span aria-label="${
						...
						} CSS" id="addCodeSnippetCSS" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show">
                <svg><use xlink:href="#iconAdd"></use></svg>
            </span>
            <div class="fn__space"></div>
            <input data-action="toggleCSS" class="b3-switch b3-switch--side fn__flex-center" type="checkbox"${
							window.siyuan.config.snippet.enabledCSS ? ' checked' : ''
						}>
        </div>
        ${cssHTML}
    </div>
    <div class="fn__none">
        <div class="fn__flex">
            ...
            <span aria-label="${
							window.siyuan.languages.addAttr
						} JS" id="addCodeSnippetJS" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show">
                <svg><use xlink:href="#iconAdd"></use></svg>
            </span>
            <div class="fn__space"></div>
            <input data-action="toggleJS" class="b3-switch b3-switch--side fn__flex-center" type="checkbox"${
							window.siyuan.config.snippet.enabledJS ? ' checked' : ''
						}>
        </div>
        ${jsHTML}
    </div>
		...
		dialog.element.addEventListener('click', (event) => {
			...
			while (target && !target.isSameNode(dialog.element)) {
				if (target.id === 'addCodeSnippetCSS' || target.id === 'addCodeSnippetJS') {
					...
					removeIds.push(
						'#snippet' +
							(itemElement.getAttribute('data-type') === 'css' ? 'CSS' : 'JS') +
...
const genSnippet = (options: ISnippet) => {
    ...
    <div class="fn__flex">
        ...
        <span aria-label="${
				...
				}" data-action="remove" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show">
            <svg><use xlink:href="#iconTrashcan"></use></svg>
        </span>
        <div class="fn__space"></div>
        <input data-type="snippet" class="b3-switch b3-switch--side fn__flex-center" type="checkbox"${
					options.enabled ? ' checked' : ''
				}>
    </div>
    <div class="fn__hr"></div>
    <textarea class="fn__block b3-text-field" placeholder="${
			window.siyuan.languages.codeSnippet
		}" style="resize: vertical;font-family:var(--b3-font-family-code)" spellcheck="false"></textarea>
    <div class="fn__hr--b"></div>
</div>`;
};
...
const setSnippetPost = (dialog: Dialog, snippets: ISnippet[], removeIds: string[]) => {
	fetchPost('/api/snippet/setSnippet', { snippets }, () => {
		...
		window.siyuan.config.snippet.enabledCSS = (
			dialog.element.querySelector('.b3-switch[data-action="toggleCSS"]') as HTMLInputElement
		).checked;
		window.siyuan.config.snippet.enabledJS = (
			dialog.element.querySelector('.b3-switch[data-action="toggleJS"]') as HTMLInputElement
		).checked;
...
const setSnippet = (dialog: Dialog, oldSnippets: ISnippet[], removeIds: string[], confirm = false) => {
	...
	if (
		objEquals(oldSnippets, snippets) &&
		window.siyuan.config.snippet.enabledCSS ===
			(dialog.element.querySelector('.b3-switch[data-action="toggleCSS"]') as HTMLInputElement).checked &&
		window.siyuan.config.snippet.enabledJS ===
			(dialog.element.querySelector('.b3-switch[data-action="toggleJS"]') as HTMLInputElement).checked
	) {
		dialog.destroy({ cancel: 'true' });
	} else {
		if (confirm) {
