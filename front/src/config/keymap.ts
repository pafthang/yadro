...
export const keymap = {
    ...
    <span data-type="reset" class="b3-list-item__action b3-tooltips b3-tooltips__w" aria-label="${
			window.siyuan.languages.reset
		}">
        <svg><use xlink:href="#iconUndo"></use></svg>
    </span>
    <span data-type="clear" class="b3-list-item__action b3-tooltips b3-tooltips__w" aria-label="${
			window.siyuan.languages.remove
		}">
        <svg><use xlink:href="#iconTrashcan"></use></svg>
    </span>
    <span data-type="update" class="config-keymap__key">${keyValue}</span>
    <input data-key="${keys + Constants.ZWSP + key}" data-value="${keymap[key].custom}" data-default="${
					keymap[key].default
				}" class="b3-text-field fn__none" value="${keyValue}" spellcheck="false">
</label>`;
			}
		});
		return html;
    ...
    <span data-type="update" class="config-keymap__key">${keyValue}</span>
    <input data-key="plugin${Constants.ZWSP}${item.name}${Constants.ZWSP}${command.langKey}" data-value="${
					command.customHotkey
				}" data-default="${command.hotkey}" class="b3-text-field fn__none" value="${keyValue}" spellcheck="false">
</label>`;
			});
			if (commandHTML) {
...
<div class="b3-label file-tree config-keymap" id="keymapList">
    <div class="fn__flex config__item">
        <label class="b3-form__icon fn__block">
            ...
            <input id="keymapInput" class="b3-form__icon-input b3-text-field fn__block" placeholder="${
        ...
        <label class="b3-form__icon fn__block searchByKeyLabel">
            ...
            <input id="searchByKey" class="b3-form__icon-input b3-text-field fn__block" spellcheck="false" placeholder="${
    ...
    <div class="b3-list b3-list--border b3-list--background">
        ...
        <div class="b3-list__panel">
            ...
            <div class="fn__none b3-list__panel">${keymap._genItem(
							window.siyuan.config.keymap.editor.general,
							'editor' + Constants.ZWSP + 'general'
						)}</div>
            <div class="b3-list-item b3-list-item--narrow toggle">
                <span class="b3-list-item__toggle b3-list-item__toggle--hl">
                    <svg class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
            ...
            <div class="fn__none b3-list__panel">${keymap._genItem(
							window.siyuan.config.keymap.editor.heading,
							'editor' + Constants.ZWSP + 'heading'
						)}</div>
            <div class="b3-list-item b3-list-item--narrow toggle">
                <span class="b3-list-item__toggle b3-list-item__toggle--hl">
                    <svg class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
            ...
            <div class="fn__none b3-list__panel">${keymap._genItem(
							window.siyuan.config.keymap.editor.list,
							'editor' + Constants.ZWSP + 'list'
						)}</div>
            <div class="b3-list-item b3-list-item--narrow toggle">
                <span class="b3-list-item__toggle b3-list-item__toggle--hl">
                    <svg class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
            ...
            <div class="fn__none b3-list__panel">${keymap._genItem(
							window.siyuan.config.keymap.editor.table,
							'editor' + Constants.ZWSP + 'table'
						)}</div>
        </div>
    </div>
    ${pluginHtml}
	...
	search(value: string, keymapString: string) {
		keymap.element.querySelectorAll('#keymapList .b3-list-item--hide-action > .b3-list-item__text').forEach((item) => {
			...
			if (
				keymapString === '' ||
				(liElement.querySelector('.b3-text-field') as HTMLInputElement).value.indexOf(updateHotkeyTip(keymapString)) >
					-1
			) {
				matchedKeymap = true;
			}
			if (
				(item.textContent.toLowerCase().indexOf(value.toLowerCase()) > -1 ||
					value.toLowerCase().indexOf(item.textContent.toLowerCase()) > -1 ||
					value === '') &&
				matchedKeymap
			) {
			...
			if (!liElement.nextElementSibling) {
				...
				// 隐藏没有子项的快捷键项目
				if (
					liElement.parentElement.childElementCount ===
					liElement.parentElement.querySelectorAll('.b3-list-item.fn__none').length
				) {
		...
		// 隐藏没有子项的快捷键项目
		if (
			editorKeymapElement.querySelectorAll('.b3-list-item--hide-action.fn__none').length ===
			editorKeymapElement.querySelectorAll('.b3-list-item--hide-action').length
		) {
		...
		keymapListElement.querySelectorAll('label.b3-list-item input').forEach((item: HTMLInputElement) => {
			item.addEventListener('keydown', function (event: KeyboardEvent) {
				...
				timeout = window.setTimeout(() => {
					...
					if (
						['⌘', '⇧', '⌥', '⌃'].includes(keymapStr.substr(keymapStr.length - 1, 1)) ||
						[
							'⌘A',
							'⌘X',
							'⌘C',
						...
						(isMac() &&
							keys[0] === 'general' &&
							['goToEditTabNext', 'goToEditTabPrev'].includes(keys[1]) &&
							keymapStr.includes('⌘'))
					) {
