...
<span class="b3-chip b3-chip--primary b3-chip--hover${
			window.siyuan.user && window.siyuan.user.userSiYuanSubscriptionStatus === 2 ? ' fn__none' : ''
		}" id="trialSub">
...
<div class="ft__on-surface ft__smaller">
    ...
    ${Math.max(
			0,
			Math.floor((window.siyuan.user.userSiYuanProExpireTime - new Date().getTime()) / 1000 / 60 / 60 / 24)
		)} 
...
<div class="config-account__center">
    <div class="config-account__bg">
        ...
        <a href="${getCloudURL('settings/avatar')}" class="config-account__avatar" style="background-image: url(${
				window.siyuan.user.userAvatarURL
			})" target="_blank"></a>
        <h1 class="config-account__name">
            <a target="_blank" class="fn__a" href="${getCloudURL('member/' + window.siyuan.user.userName)}">${
				window.siyuan.user.userName
			}</a>
            <span class="ft__on-surface ft__smaller">${
    ...
    <div class="config-account__info">
        <div class="fn__flex">
            <a class="b3-button b3-button--text" href="${getCloudURL('settings')}" target="_blank">${
            ...
            <button class="b3-button b3-button--cancel${
							window.siyuan.config.system.container === 'ios' ? '' : ' fn__none'
						}" id="deactivateUser">
                ${window.siyuan.languages.deactivateUser}
            </button>
            <span class="fn__flex-1"></span>
            <button class="b3-button b3-button--cancel b3-tooltips b3-tooltips__n" id="refresh" aria-label="${
        ...
        <div class="fn__flex">  
            <label>
                ...
                <input class="b3-switch fn__flex-center" id="displayTitle" type="checkbox"${
									window.siyuan.config.account.displayTitle ? ' checked' : ''
								}/>
            </label>
            <div class="fn__flex-1"></div>
            <label>
                ${window.siyuan.languages.accountDisplayVIP}
                <input class="b3-switch fn__flex-center" id="displayVIP" type="checkbox"${
									window.siyuan.config.account.displayVIP ? ' checked' : ''
								}/>
            </label>
        </div>
    </div>
...
<div class="b3-form__space config-account__center">
    <div class="config-account__form" id="form1">
        <div class="b3-form__icon">
            ...
            <input id="userName" class="b3-text-field fn__block b3-form__icon-input" placeholder="${
        ...
        <div class="b3-form__icon">
            ...
            <input type="password" id="userPassword" class="b3-text-field b3-form__icon-input fn__block" placeholder="${
        ...
        <div class="b3-form__icon">
            ...
            <select class="b3-select b3-form__icon-input fn__block" id="cloudRegion">
                <option value="0"${window.siyuan.config.cloudRegion === 0 ? ' selected' : ''}>${
			window.siyuan.languages.cloudRegionChina
		}</option>
                <option value="1"${window.siyuan.config.cloudRegion === 1 ? ' selected' : ''}>${
        ...
        <div class="ft__center">
            <a href="${getCloudURL('forget-pwd')}" class="b3-button b3-button--cancel" target="_blank">${
			window.siyuan.languages.forgetPassword
		}</a>
            <span class="fn__space${window.siyuan.config.system.container === 'ios' ? ' fn__none' : ''}"></span>
            <a href="${getCloudURL('register')}" class="b3-button b3-button--cancel${
			window.siyuan.config.system.container === 'ios' ? ' fn__none' : ''
		}" target="_blank">${window.siyuan.languages.register}</a>
        </div>
    </div>
    <div class="fn__none config-account__form" id="form2">
        <div class="b3-form__icon">
            <svg class="b3-form__icon-icon"><use xlink:href="#iconLock"></use></svg>
            <input id="twofactorAuthCode" class="b3-text-field fn__block b3-form__icon-input" placeholder="${
...
<div class="config-account__center config-account__center--text${
	...
	bindEvent: (element: Element) => {
					...
					confirmDialog(window.siyuan.languages.activationCode, response.msg, () => {
						if (response.code === 0) {
							fetchPost(
								...
								{ data: (activationCodeElement.previousElementSibling as HTMLInputElement).value },
								() => {
		...
		captchaImgElement.addEventListener('click', () => {
			captchaImgElement.setAttribute(
				'src',
				getCloudURL('captcha') + `/login?needCaptcha=${needCaptcha}&t=${new Date().getTime()}`
			);
		});
		...
		cloudRegionElement.addEventListener('change', () => {
			...
			element.querySelector('#form1').lastElementChild.innerHTML = `<a href="${getCloudURL(
				'forget-pwd'
			)}" class="b3-button b3-button--cancel" target="_blank">${window.siyuan.languages.forgetPassword}</a>
<span class="fn__space${window.siyuan.config.system.container === 'ios' ? ' fn__none' : ''}"></span>
<a href="${getCloudURL('register')}" class="b3-button b3-button--cancel${
				window.siyuan.config.system.container === 'ios' ? ' fn__none' : ''
			}" target="_blank">${window.siyuan.languages.register}</a>`;
				...
				(data) => {
					...
					if (data.code === 1) {
						...
						if (data.data.needCaptcha) {
							...
							captchaElement.previousElementSibling.setAttribute(
								'src',
