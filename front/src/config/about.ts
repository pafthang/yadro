...
export const about = {
	...
	genHTML: () => {
		return `<label class="fn__flex b3-label${
			isBrowser() ||
			window.siyuan.config.system.isMicrosoftStore ||
			'std' !== window.siyuan.config.system.container ||
			'linux' === window.siyuan.config.system.os
				? ' fn__none'
				: ''
		}">
    ...
    <input class="b3-switch fn__flex-center" id="autoLaunch" type="checkbox"${
			window.siyuan.config.system.autoLaunch ? ' checked' : ''
		}>
</label>
<label class="fn__flex b3-label${
			isBrowser() || window.siyuan.config.system.isMicrosoftStore || window.siyuan.config.system.container !== 'std'
				? ' fn__none'
				: ''
		}">
    <div class="fn__flex-1">
    ...
    <div class="fn__space"></div>
    <input class="b3-switch fn__flex-center" id="downloadInstallPkg" type="checkbox"${
			window.siyuan.config.system.downloadInstallPkg ? ' checked' : ''
		}>
</label>
<label class="b3-label fn__flex">
    ...
    <div class="fn__space"></div>
    <input class="b3-switch fn__flex-center" id="googleAnalytics" type="checkbox"${
			window.siyuan.config.system.disableGoogleAnalytics ? '' : ' checked'
		}>
</label>
<label class="b3-label fn__flex">
    <div class="fn__flex-1">
    ...
    <div class="fn__space"></div>
    <input class="b3-switch fn__flex-center" id="uploadErrLog" type="checkbox"${
			window.siyuan.config.system.uploadErrLog ? ' checked' : ''
		}>
</label>
<label class="b3-label fn__flex">
    <div class="fn__flex-1">
    ...
    <div class="fn__space"></div>
    <input class="b3-switch fn__flex-center" id="networkServe" type="checkbox"${
			window.siyuan.config.system.networkServe ? ' checked' : ''
		}>
</label>
<div class="b3-label${
			window.siyuan.config.readonly || (isBrowser() && !isInIOS() && !isInAndroid() && !isIPad()) ? ' fn__none' : ''
		}">
    <div class="fn__flex">
        <div class="fn__flex-1">
    ...
    <label class="b3-label fn__flex${!window.siyuan.config.accessAuthCode || isBrowser() ? ' fn__none' : ''}">
        ...
        <div class="fn__space"></div>
        <input class="b3-switch fn__flex-center" id="lockScreenMode" type="checkbox"${
					window.siyuan.config.system.lockScreenMode === 1 ? ' checked' : ''
				}>
    </label>
</div>
<div class="b3-label config__item${isBrowser() && !isInAndroid() ? ' fn__none' : ' fn__flex'}">
    ...
    <div class="fn__space"></div>
    <button data-type="open" data-url="http://${
			window.siyuan.config.system.networkServe ? window.siyuan.config.localIPs[0] : '127.0.0.1'
		}:${location.port}" class="b3-button b3-button--outline fn__size200 fn__flex-center">
...
<div class="b3-label${
			window.siyuan.config.system.container === 'std' || window.siyuan.config.system.container === 'docker'
				? ''
				: ' fn__none'
		}">
    ...
    <div class="b3-label__text fn__flex config__item" style="padding: 4px 0 4px 4px;">
        <select id="aboutScheme" class="b3-select">
            <option value="" ${window.siyuan.config.system.networkProxy.scheme === '' ? 'selected' : ''}>${
			window.siyuan.languages.directConnection
		}</option>
            <option value="socks5" ${
							window.siyuan.config.system.networkProxy.scheme === 'socks5' ? 'selected' : ''
						}>SOCKS5</option>
            <option value="https" ${
							window.siyuan.config.system.networkProxy.scheme === 'https' ? 'selected' : ''
						}>HTTPS</option>
            <option value="http" ${
							window.siyuan.config.system.networkProxy.scheme === 'http' ? 'selected' : ''
						}>HTTP</option>
        </select>
        <span class="fn__space"></span>
        <input id="aboutHost" placeholder="Host/IP" class="b3-text-field fn__block" value="${
					window.siyuan.config.system.networkProxy.host
				}"/>
        <span class="fn__space"></span>
        <input id="aboutPort" placeholder="Port" class="b3-text-field fn__block" value="${
					window.siyuan.config.system.networkProxy.port
				}" type="number"/>
        <span class="fn__space"></span>
        <button id="aboutConfirm" class="b3-button fn__size200 b3-button--outline">${
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
		...
		lockScreenModeElement.addEventListener('change', () => {
			fetchPost(
				...
				{ lockScreenMode: lockScreenModeElement.checked ? 1 : 0 },
				() => {
