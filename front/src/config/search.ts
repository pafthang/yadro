...
export const initConfigSearch = (element: HTMLElement, app: App) => {
	const configIndex = [
		...
		getLang([
			'riffCard',
			'flashcardNewCardLimit',
			'flashcardNewCardLimitTip',
			'flashcardReviewCardLimit',
			'flashcardNewCardLimit',
	...
	const updateTab = () => {
		...
		configIndex.map((item, index) => {
			item.map((subItem) => {
				...
				if (
					subItem &&
					(inputValue.toLowerCase().indexOf(subItem.toLowerCase()) > -1 ||
						subItem.toLowerCase().indexOf(inputValue.toLowerCase()) > -1)
				) {
		...
		element.querySelectorAll('.b3-tab-bar li').forEach((item: HTMLElement, index) => {
			if (indexList.includes(index)) {
				...
				if (type === 'keymap') {
					...
					panelElement
						.querySelectorAll(`.config__tab-container[data-name="${type}"] .b3-label`)
