...
export const genCardHTML = (options: { id: string; cardType: TCardType; cardsData: ICardData; isTab: boolean }) => {
    ...
    <div data-type="count" class="${options.cardsData.cards.length === 0 ? 'fn__none' : 'fn__flex'}">${genCardCount(
		options.cardsData.unreviewedNewCardCount,
		options.cardsData.unreviewedOldCardCount
	)}</span></div>
    <svg class="toolbar__icon" data-id="${options.id || ''}" data-cardtype="${
		options.cardType
	}" data-type="filter"><use xlink:href="#iconFilter"></use></svg>
	...
	iconsHTML = `<div class="block__icons">
        ...
        <div data-type="count" class="ft__on-surface ft__smaller fn__flex-center${
					options.cardsData.cards.length === 0 ? ' fn__none' : ' fn__flex'
				}">${genCardCount(
		options.cardsData.unreviewedNewCardCount,
		options.cardsData.unreviewedOldCardCount
	)}</span></div>
        <div class="fn__space"></div>
        <button data-id="${options.id || ''}" data-cardtype="${
		options.cardType
	}" data-type="filter" class="block__icon block__icon--show">
            <svg><use xlink:href="#iconFilter"></use></svg>
        </button>
        <div class="fn__space"></div>
        <div data-type="fullscreen" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show" aria-label="${
					window.siyuan.languages.fullscreen
				}">
            <svg><use xlink:href="#iconFullscreen"></use></svg>
        </div>
        <div class="fn__space${options.isTab ? ' fn__none' : ''}"></div>
        <div data-type="sticktab" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show${
					options.isTab ? ' fn__none' : ''
				}" aria-label="${window.siyuan.languages.openInNewTab}">
...
${window.siyuan.config.flashcard.list ? 'card__block--hideli' : ''}" data-type="render"></div>
    <div class="card__empty card__empty--space${
			options.cardsData.cards.length === 0 ? '' : ' fn__none'
		}" data-type="empty">
        <div>🔮</div>
    ...
    <div class="fn__flex card__action${options.cardsData.cards.length === 0 ? ' fn__none' : ''}">
        ...
        <button data-type="-1" class="b3-button fn__flex-1">${window.siyuan.languages.cardShowAnswer} (${
		window.siyuan.languages.space
	} / Enter)</button>
    </div>
    <div class="fn__flex card__action fn__none">
        ...
        <div>
            <span></span>
            <button data-type="3" aria-label="3 / l / d / ${
							window.siyuan.languages.space
						} / Enter" class="b3-button b3-button--info b3-tooltips__n b3-tooltips">
                <div class="card__icon">😊</div>
...
export const bindCardEvent = async (options: {
	...
	if (window.siyuan.storage[Constants.LOCAL_FLASHCARD].fullscreen) {
		fullscreen(options.element.querySelector('.card__main'), options.element.querySelector('[data-type="fullscreen"]'));
	}
	let index = 0;
				...
				fetchPost(
					...
					(response) => {
						onGet({
							...
							action:
								response.data.rootID === response.data.id
	...
	options.element.addEventListener('click', (event: MouseEvent) => {
		...
		if (typeof event.detail === 'string') {
			...
			if (fullscreenElement) {
				fullscreen(
					options.element.querySelector('.card__main'),
					options.element.querySelector('[data-type="fullscreen"]')
				);
				resize(editor.protyle);
				window.siyuan.storage[Constants.LOCAL_FLASHCARD].fullscreen =
					!window.siyuan.storage[Constants.LOCAL_FLASHCARD].fullscreen;
								...
								movePathTo(
									(toPath, toNotebook) => {
										filterElement.setAttribute(
											'data-id',
											toPath[0] === '/' ? toNotebook[0] : getDisplayName(toPath[0], true, true)
										);
		...
		if (type === '-1') {
			...
			if (actionElements[0].classList.contains('fn__none')) {
				...
				editor.protyle.element.classList.remove(
		...
		if (['1', '2', '3', '4', '-3'].includes(type) && actionElements[0].classList.contains('fn__none')) {
			fetchPost(
				type === '-3' ? '/api/riff/skipReviewRiffCard' : '/api/riff/reviewRiffCard',
				{
...
export const openCardByData = async (
	app: App,
	cardsData: ICardData,
	cardType: TCardType,
	id?: string,
	...
	(dialog.element.querySelector('.b3-dialog__scrim') as HTMLElement).style.backgroundColor =
...
const nextCard = (options: {
	...
	options.countElement.innerHTML = genCardCount(
		options.cardsData.unreviewedNewCardCount,
		options.cardsData.unreviewedOldCardCount,
		options.index + 1
	);
