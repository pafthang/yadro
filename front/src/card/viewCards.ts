...
export const viewCards = (
	...
	deckID: string,
	title: string,
	deckType: 'Tree' | '' | 'Notebook',
	cb?: (response: IWebSocketData) => void
) => {
	let pageIndex = 1;
	let edit: Protyle;
	fetchPost(
		`/api/riff/get${deckType}RiffCards`,
		{
			id: deckID,
			page: pageIndex
		},
    ...
    <div class="block__icons">
        ...
        <span data-type="resetAll" class="block__icon block__icon--show b3-tooltips b3-tooltips__w${
					deckType === '' && deckID === '' ? ' fn__none' : ''
				}" aria-label="${window.siyuan.languages.reset}"><svg><use xlink:href='#iconUndo'></use></svg></span>
        <span class="fn__space"></span>
        <span data-type="previous" class="block__icon block__icon--show b3-tooltips b3-tooltips__w" disabled="disabled" aria-label="${
					window.siyuan.languages.previousLabel
				}"><svg><use xlink:href='#iconLeft'></use></svg></span>
        <span class="fn__space"></span>
        <span data-type="next" class="block__icon block__icon--show b3-tooltips b3-tooltips__w" disabled="disabled" aria-label="${
				...
				while (target && !dialog.element.isSameNode(target)) {
					...
					if (type === 'close') {
						...
						fetchPost(`/api/riff/get${deckType}RiffCards`, { id: deckID, page: pageIndex }, (cardsResponse) => {
							...
							nextElement.nextElementSibling.nextElementSibling.textContent = `${pageIndex}/${
								cardsResponse.data.pageCount || 1
							}`;
						...
						fetchPost(
							...
							() => {
								target.parentElement.querySelector('.ariaLabel.b3-list-item__meta').textContent =
						...
						confirmDialog(
							...
							window.siyuan.languages.resetCardTip.replace(
								'${x}',
								dialog.element.querySelector('.counter').textContent
							),
							() => {
								fetchPost(
									'/api/riff/resetRiffCards',
						...
						fetchPost(
							...
							(removeResponse) => {
								...
								dialog.element.querySelector('.counter').textContent = (
									parseInt(dialog.element.querySelector('.counter').textContent) - 1
								).toString();
								if (cb) {
									cb(removeResponse);
								}
...
const renderViewItem = (blocks: IBlock[], title: string, deckType: string) => {
	...
	blocks.forEach((item: IBlock) => {
		if (item.type) {
			...
			listHTML += `<div data-type="card-item" class="b3-list-item${isFirst ? ' b3-list-item--focus' : ''}${
				isMobile() ? '' : ' b3-list-item--hide-action'
			}" data-id="${item.id}">
<svg class="b3-list-item__graphic"><use xlink:href="#${getIconByType(item.type)}"></use></svg>
${unicode2Emoji(item.ial.icon, 'b3-list-item__graphic', true)}
<span class="b3-list-item__text">${item.content || Constants.ZWSP}</span>
<span class="${
				isMobile() || !hPath ? 'fn__none ' : ''
			}b3-list-item__meta b3-list-item__meta--ellipsis" title="${escapeAttr(hPath)}">${escapeHtml(hPath)}</span>
<span data-position="parentE" aria-label="${window.siyuan.languages.revisionCount}" class="ariaLabel counter${
				item.riffCard?.reps === 0 ? ' fn__none' : ''
			}">${item.riffCard?.reps}</span>
<span data-position="parentE" aria-label="${window.siyuan.languages.nextDue}" class="ariaLabel b3-list-item__meta${
				!item.riffCard?.due ? ' fn__none' : ''
			}">${dayjs(item.riffCard?.due).format('YYYY-MM-DD')}</span>
<span data-position="parentE" data-type="reset" data-id="${
				item.id
			}" class="b3-list-item__action ariaLabel" aria-label="${window.siyuan.languages.reset}">
    <svg><use xlink:href="#iconUndo"></use></svg>
</span>
<span data-position="parentE" data-type="remove" data-id="${
				item.id
			}" class="b3-list-item__action ariaLabel" aria-label="${window.siyuan.languages.removeDeck}">
			...
			fetchPost(
				...
				(getResponse) => {
					onGet({
						...
						action:
							getResponse.data.rootID === getResponse.data.id
								? [Constants.CB_GET_HTML]
