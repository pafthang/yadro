...
import { isInAndroid, isInIOS, isIPad, setStorageVal } from '../protyle/util/compatibility';
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
						...
						btnsElement[1].addEventListener('click', () => {
							fetchPost(
								...
								{
									path: pathPosix().join(
										pathPosix().dirname(window.siyuan.config.system.workspaceDir),
										inputElement.value
									)
								},
								() => {
					...
					click() {
						fetchPost('/api/system/getMobileWorkspaces', {}, (response) => {
							...
							response.data.forEach((item: string, index: number) => {
								selectHTML += `<option value="${item}"${
									index === 0 ? ' selected="selected"' : ''
								}>${pathPosix().basename(item)}</option>`;
							});
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
							...
							btnsElement[1].addEventListener('click', () => {
								...
								confirmDialog(
									...
									`${pathPosix().basename(window.siyuan.config.system.workspaceDir)} -> ${pathPosix().basename(
										openPath
									)}?`,
									() => {
										fetchPost(
							...
							if (hasClosestByClassName(event.target as Element, 'b3-menu__action')) {
								...
								fetchPost('/api/system/removeWorkspaceDir', { path: item.path }, () => {
									confirmDialog(
										...
										window.siyuan.languages.removeWorkspacePhysically.replace('${x}', item.path),
										() => {
						...
						submenu.push({
							...
							accelerator:
								window.siyuan.storage[Constants.LOCAL_DAILYNOTEID] === item.id
									? window.siyuan.config.keymap.general.dailyNote.custom
									: '',
							click: () => {
