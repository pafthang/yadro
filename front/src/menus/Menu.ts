...
export class Menu {
	...
	private preventDefault(event: KeyboardEvent) {
		if (
			!hasClosestByClassName(event.target as Element, 'b3-menu') &&
			// 移动端底部键盘菜单
	...
	public popup(options: IPosition) {
		...
		setPosition(
			this.element,
			options.x - (options.isLeft ? window.siyuan.menus.menu.element.clientWidth : 0),
			options.y,
			options.h,
			options.w
		);
	...
	constructor(options: IMenu) {
		...
		if (options.element) {
			...
			if (typeof options.iconHTML === 'string') {
				...
				html = `<svg class="b3-menu__icon ${options.iconClass || ''}" style="${
					options.icon === 'iconClose' ? 'height:10px;' : ''
				}"><use xlink:href="#${options.icon || ''}"></use></svg>${html}`;
			}
			if (options.accelerator) {
				html += `<span class="b3-menu__accelerator">${updateHotkeyTip(options.accelerator)}</span>`;
			}
			if (options.action) {
				html += `<svg class="b3-menu__action${
					options.action === 'iconCloseRound' ? ' b3-menu__action--close' : ''
				}"><use xlink:href="#${options.action}"></use></svg>`;
			}
