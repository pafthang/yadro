	...
	fetchPost(
		...
		(response) => {
			...
			response.data.forEach((item: { path: string; hName: string }, index: number) => {
				...
				}"><div class="b3-list-item__text">${item.hName}</div></div>`;
			});
...
<div id="preview" style="width: 360px;display: ${
			isMobile() || window.outerWidth < window.outerWidth / 2 + 260 ? 'none' : 'flex'
		};padding: 8px;overflow: auto;justify-content: center;align-items: center;word-break: break-all;"></div>
</div>`,
		bind(element) {
...
export const fileAnnotationRefMenu = (protyle: IProtyle, refElement: HTMLElement) => {
	...
	window.siyuan.menus.menu.append(
		new MenuItem({
			...
			type: 'readonly',
			label: `<div>ID</div>
<textarea rows="1" style="margin:4px 0;width: ${isMobile() ? '200' : '360'}px" class="b3-text-field" readonly>${
				refElement.getAttribute('data-id') || ''
			}</textarea>
...
export const refMenu = (protyle: IProtyle, element: HTMLElement) => {
	...
	window.siyuan.menus.menu.append(
		new MenuItem({
			...
			accelerator:
				window.siyuan.config.keymap.editor.general.insertBottom.custom +
				(window.siyuan.config.keymap.editor.general.insertBottom.custom ? '/' : '') +
				'⇧Click',
			click() {
				checkFold(refBlockId, (zoomIn, action) => {
	...
	if (!protyle.disabled) {
		...
		submenu = submenu.concat([
			...
			},
			{
				...
				icon: 'iconLink',
				click() {
					element.outerHTML = `<span data-type="a" data-href="siyuan://blocks/${element.getAttribute('data-id')}">${
						element.innerHTML
					}</span><wbr>`;
					nodeElement.setAttribute('updated', dayjs().format('YYYYMMDDHHmmss'));
					updateTransaction(protyle, id, nodeElement.outerHTML, oldHTML);
		...
		if (
			...
			submenu.push({
				...
				icon: 'iconSQL',
				click() {
					const html = `<div data-content="select * from blocks where id='${refBlockId}'" data-node-id="${id}" data-type="NodeBlockQueryEmbed" class="render-node" updated="${dayjs().format(
						'YYYYMMDDHHmmss'
					)}">${nodeElement.querySelector('.protyle-attr').outerHTML}</div>`;
...
export const zoomOut = (options: {
	...
	if (!options.reload && breadcrumbHLElement && breadcrumbHLElement.getAttribute('data-node-id') === options.id) {
		...
		const focusElement = options.protyle.wysiwyg.element.querySelector(
			`[data-node-id="${options.focusId || options.id}"]`
		);
		if (focusElement) {
	...
	fetchPost(
		'/api/filetree/getDoc',
		{
			id: options.id,
			size:
				options.id === options.protyle.block.rootID
		...
		(getResponse) => {
			if (options.isPushBack) {
				onGet({
					...
					action:
						options.id === options.protyle.block.rootID
							? [Constants.CB_GET_FOCUS, Constants.CB_GET_HTML]
							: [Constants.CB_GET_ALL, Constants.CB_GET_FOCUS, Constants.CB_GET_HTML],
...
<textarea style="margin:4px 0;width: ${
					isMobile() ? '200' : '360'
				}px" rows="1" class="b3-text-field">${imgElement.getAttribute('src')}</textarea>
<div class="fn__hr"></div>
				...
				bind(element) {
					...
					textElements[0].addEventListener('input', (event: InputEvent) => {
						...
						if (value.startsWith('assets/')) {
							...
							assetElement
								.querySelector('.protyle-action__drag')
								.insertAdjacentHTML(
									'afterend',
									'<span class="img__net"><svg><use xlink:href="#iconLanguage"></use></svg></span>'
								);
						}
					});
			...
			new MenuItem({
				...
				submenu: [
					...
					},
					{
						iconHTML: '',
						type: 'readonly',
						label: `<div style="margin: 4px 0;" aria-label="${
							width === 0 ? window.siyuan.languages.default : width + '%'
						}" class="b3-tooltips b3-tooltips__n${isMobile() ? '' : ' fn__size200'}">
...
<textarea style="width: ${isMobile() ? '200' : '360'}px;margin: 4px 0;" rows="1" class="b3-text-field"></textarea>`,
			bind(element) {
				...
				inputElements[1].addEventListener('compositionend', () => {
					linkElement.innerHTML = Lute.EscapeHTMLStr(
						inputElements[1].value.replace(/\n|\r\n|\r|\u2028|\u2029/g, '') || '*'
					);
				});
				inputElements[1].addEventListener('input', (event: KeyboardEvent) => {
					if (!event.isComposing) {
						...
						linkElement.innerHTML =
							Lute.EscapeHTMLStr(inputElements[1].value.replace(/\n|\r\n|\r|\u2028|\u2029/g, '')) || '*';
					}
				});
	...
	window.siyuan.menus.menu.removeCB = () => {
		if (inputElements[2].value) {
			linkElement.setAttribute(
				'data-title',
				Lute.EscapeHTMLStr(inputElements[2].value.replace(/\n|\r\n|\r|\u2028|\u2029/g, ''))
			);
		} else {
			linkElement.removeAttribute('data-title');
		}
		if (linkElement.getAttribute('data-type').indexOf('a') > -1) {
			linkElement.setAttribute(
				'data-href',
				Lute.EscapeHTMLStr(inputElements[0].value.replace(/\n|\r\n|\r|\u2028|\u2029/g, ''))
			);
		} else {
			linkElement.removeAttribute('data-href');
		}
	...
	const subMenus: IMenu[] = [
		{
			iconHTML: '',
			type: 'readonly',
			label: `<textarea rows="1" class="b3-text-field fn__size200" placeholder="${
				window.siyuan.languages.link
			}" style="margin: 4px 0">${iframeElement.getAttribute('src') || ''}</textarea>`,
			bind(element) {
				element.style.maxWidth = 'none';
	...
	const subMenus: IMenu[] = [
		{
			iconHTML: '',
			type: 'readonly',
			label: `<textarea rows="1" style="margin: 4px 0" class="b3-text-field" placeholder="${
				window.siyuan.languages.link
			}">${videoElement.getAttribute('src')}</textarea>`,
			bind(element) {
				element.style.maxWidth = 'none';
				element.querySelector('textarea').addEventListener('change', (event) => {
...
export const tableMenu = (protyle: IProtyle, nodeElement: Element, cellElement: HTMLTableCellElement, range: Range) => {
	...
	Array.from(tableElement.rows).find((item) => {
		...
		if (
			cellElement &&
			(cellElement.classList.contains('fn__none') || cellElement.colSpan > 1 || cellElement.rowSpan > 1)
		) {
			nextColIsPure = false;
			return true;
		}
	...
	if (
		(cellElement.parentElement.parentElement.tagName !== 'THEAD' &&
	...
	if (nodeElement.getAttribute('data-type') === 'NodeHeading') {
		if (fold === '0') {
			nodeElement.insertAdjacentHTML(
				'beforeend',
				'<div spin="1" style="text-align: center"><img width="24px" src="/stage/loading-pure.svg"></div>'
			);
			transaction(
				protyle,
				[
