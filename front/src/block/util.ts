...
export const cancelSB = (protyle: IProtyle, nodeElement: Element) => {
	...
	let previousId = nodeElement.previousElementSibling
		? nodeElement.previousElementSibling.getAttribute('data-node-id')
		: undefined;
	...
	undoOperations.push({
		...
		previousID: nodeElement.previousElementSibling
			? nodeElement.previousElementSibling.getAttribute('data-node-id')
			: undefined,
...
export const jumpToParentNext = (protyle: IProtyle, nodeElement: Element) => {
	...
	if (topElement) {
		const topParentElement =
			hasClosestByClassName(topElement, 'list') ||
			hasClosestByClassName(topElement, 'bq') ||
			hasClosestByClassName(topElement, 'sb') ||
			topElement;
...
export const insertEmptyBlock = (protyle: IProtyle, position: InsertPosition, id?: string) => {
	...
	if (
		blockElement.getAttribute('data-type') === 'NodeListItem' &&
		...
		updateTransaction(
			protyle,
			newElement.parentElement.getAttribute('data-node-id'),
			newElement.parentElement.outerHTML,
			parentOldHTML
		);
	} else {
...
export const genEmptyBlock = (zwsp = true, wbr = true, string?: string) => {
	...
	return `<div data-node-id="${Lute.NewNodeID()}" data-type="NodeParagraph" class="p"><div contenteditable="true" spellcheck="${
		window.siyuan.config.editor.spellcheck
	}">${html}</div><div contenteditable="false" class="protyle-attr">${Constants.ZWSP}</div></div>`;
};
...
export const genEmptyElement = (zwsp = true, wbr = true, id?: string) => {
	...
	element.innerHTML = `<div contenteditable="true" spellcheck="${window.siyuan.config.editor.spellcheck}">${
		zwsp ? Constants.ZWSP : ''
	}${wbr ? '<wbr>' : ''}</div><div class="protyle-attr" contenteditable="false">${Constants.ZWSP}</div>`;
	return element;
};
