	...
	constructor(options: {
		...
		for (let i = 0; i < window.siyuan.blockPanels.length; i++) {
			...
			if (
				...
				item.targetElement &&
				parseInt(item.element.getAttribute('data-level')) >= level
			) {
				item.destroy();
				i--;
			}
		...
		fetchPost('/api/block/getBlockInfo', { id: this.nodeIds[index] }, (response) => {
			...
			const editor = new Protyle(this.app, editorElement, {
				...
				after: (editor) => {
					...
					editor.protyle.scroll.element.parentElement.setAttribute(
						'style',
						`--b3-dynamicscroll-width:${Math.min(editor.protyle.contentElement.clientHeight - 49, 200)}px;${
							isMobile() ? '' : 'right:10px'
						}`
					);
				}
...
<div class="block__content">`;
		...
		if (html) {
			html +=
				'</div><div class="resize__rd"></div><div class="resize__ld"></div><div class="resize__lt"></div><div class="resize__rt"></div><div class="resize__r"></div><div class="resize__d"></div><div class="resize__t"></div><div class="resize__l"></div>';
		}
						...
						? () => {
								...
								if (this.targetElement && this.targetElement.classList.contains('protyle-wysiwyg__embed')) {
									...
									this.element.style.height =
										Math.min(window.innerHeight - Constants.SIZE_TOOLBAR_HEIGHT, targetRect.height + 42) + 'px';
									...
									this.element.style.maxHeight =
										Math.floor(window.innerHeight - Math.max(this.y, Constants.SIZE_TOOLBAR_HEIGHT) - 12) + 'px';
								}
