	...
	document.addEventListener('mouseover', (event: MouseEvent & { target: HTMLElement; path: HTMLElement[] }) => {
		...
		if (aElement) {
			...
			if (
				!tipElement ||
				(tipElement &&
					tipElement.clientHeight >= tipElement.scrollHeight &&
					tipElement.clientWidth >= tipElement.scrollWidth)
			) {
				hideTooltip();
			}
		}
...
const hidePopover = (event: MouseEvent & { path: HTMLElement[] }) => {
	...
	if (
		(target.id &&
			target.tagName !== 'svg' &&
			(target.id.startsWith('minder_node') || target.id.startsWith('kity_') || target.id.startsWith('node_'))) ||
	...
	popoverTargetElement =
		(hasClosestByAttribute(target, 'data-type', 'block-ref') as HTMLElement) ||
...
const getTarget = (event: MouseEvent & { target: HTMLElement }, aElement: false | HTMLElement) => {
	...
	popoverTargetElement =
		(hasClosestByAttribute(event.target, 'data-type', 'block-ref') as HTMLElement) ||
	...
	if (!popoverTargetElement && aElement) {
		if (
			aElement.getAttribute('data-href')?.startsWith('siyuan://blocks') &&
			...
			if (
				textElement &&
				textElement.dataset.type === 'url' &&
				textElement.dataset.href?.startsWith('siyuan://blocks')
			) {
...
export const showPopover = async (app: App, showRef = false) => {
	...
	if (
		!hasPin &&
		popoverTargetElement.parentElement &&
		...
		window.siyuan.blockPanels.push(
			new BlockPanel({
				...
				isBacklink:
					showRef ||
					popoverTargetElement.classList.contains('protyle-attr--refcount') ||
					popoverTargetElement.classList.contains('counter'),
				nodeIds: ids,
				defIds
			})
		);
