...
export class Wnd {
    ...
    <div class="layout-tab-container fn__flex-1">${dragHTML}</div>
		...
		this.headersElement.addEventListener(
			'mousewheel',
			(event: WheelEvent) => {
		...
		this.headersElement.parentElement.addEventListener(
			'dragover',
			function (
				event: DragEvent & {
					target: HTMLElement;
				...
				if (
					!newTabHeaderElement.isSameNode(oldTabHeaderElement) &&
					((oldTabHeaderElement.classList.contains('item--pin') &&
						newTabHeaderElement.classList.contains('item--pin')) ||
						(!oldTabHeaderElement.classList.contains('item--pin') &&
							!newTabHeaderElement.classList.contains('item--pin')))
				) {
		...
		this.headersElement.parentElement.addEventListener(
			...
			function (
				...
				if (event.dataTransfer.types.includes(Constants.SIYUAN_DROP_FILE)) {
					...
					event.dataTransfer
						.getData(Constants.SIYUAN_DROP_FILE)
						.split(',')
						.forEach((item) => {
							if (item) {
								openFileById({
				...
				const oldTab = getInstanceById(tabData.id) as Tab;
			...
			if (dragElement.style.height === '50%' || dragElement.style.width === '50%') {
				...
				if (dragElement.style.height === '50%') {
					...
					if (
						...
						newWnd.element.previousElementSibling &&
						targetWnd.element.parentElement
					) {
						// 交换位置
	...
	public showHeading() {
		...
		if (
			...
			this.headersElement.scrollLeft + this.headersElement.clientWidth
		) {
			this.headersElement.scrollLeft =
				currentElement.offsetLeft + currentElement.clientWidth - this.headersElement.clientWidth;
	...
	public switchTab(target: HTMLElement, pushBack = false, update = true, resize = true, isSaveLayout = true) {
		...
		if (currentTab && target === currentTab.headElement) {
			if (currentTab.model instanceof Graph) {
				...
				currentTab.model instanceof Asset &&
				currentTab.model.pdfObject &&
				currentTab.model.pdfObject.pdfViewer
			) {
		...
		if (currentTab && currentTab.model instanceof Editor) {
			...
			if (keepCursorId) {
				...
				Array.from(
					currentTab.model.editor.protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${keepCursorId}"]`)
				...
				if (item.headElement) {
					if (item.headElement.classList.contains('item--focus')) {
						...
						Array.from(item.headElement.parentElement.children).forEach((headItem: HTMLElement) => {
							if (
								!headItem.isSameNode(item.headElement) &&
								...
								if (!latestHeadElement) {
									...
									headItem.getAttribute('data-activetime') > latestHeadElement.getAttribute('data-activetime')
								) {
									latestHeadElement = headItem;
								}
							}
	...
	public moveTab(tab: Tab, nextId?: string) {
		...
		if (rangeData && tab.model instanceof Editor) {
			// DOM 移动后 range 会变化
			const range = focusByOffset(
				tab.model.editor.protyle.wysiwyg.element.querySelector(`[data-node-id="${rangeData.id}"]`),
				rangeData.start,
				rangeData.end
			);
			if (range) {
