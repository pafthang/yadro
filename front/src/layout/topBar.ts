...
<div id="barWorkspace" aria-label="${window.siyuan.languages.mainMenu} ${updateHotkeyTip(
		window.siyuan.config.keymap.general.mainMenu.custom
	)}" class="ariaLabel toolbar__item toolbar__item--active">
...
<button id="barBack" class="ariaLabel toolbar__item toolbar__item--disabled" aria-label="${
		window.siyuan.languages.goBack
	} ${updateHotkeyTip(window.siyuan.config.keymap.general.goBack.custom)}">
    <svg><use xlink:href="#iconBack"></use></svg>
</button>
<button id="barForward" class="ariaLabel toolbar__item toolbar__item--disabled" aria-label="${
		window.siyuan.languages.goForward
	} ${updateHotkeyTip(window.siyuan.config.keymap.general.goForward.custom)}">
...
<div id="barSearch" class="toolbar__item ariaLabel" aria-label="${
		window.siyuan.languages.globalSearch
	} ${updateHotkeyTip(window.siyuan.config.keymap.general.globalSearch.custom)}">
    <svg><use xlink:href="#iconSearch"></use></svg>
</div>
<div id="barZoom" class="toolbar__item ariaLabel${
		window.siyuan.storage[Constants.LOCAL_ZOOM] === 1 || isBrowser() ? ' fn__none' : ''
	}" aria-label="${window.siyuan.languages.zoom}">
    <svg><use xlink:href="#iconZoom${window.siyuan.storage[Constants.LOCAL_ZOOM] > 1 ? 'In' : 'Out'}"></use></svg>
</div>
<div id="barMode" class="toolbar__item ariaLabel${window.siyuan.config.readonly ? ' fn__none' : ''}" aria-label="${
		window.siyuan.languages.appearanceMode
	}">
    <svg><use xlink:href="#icon${
			window.siyuan.config.appearance.modeOS ? 'Mode' : window.siyuan.config.appearance.mode === 0 ? 'Light' : 'Dark'
		}"></use></svg>
</div>
<div id="barExit" class="toolbar__item ariaLabel${isInIOS() || isInAndroid() ? '' : ' fn__none'}" aria-label="${
...
<div class="fn__flex" id="windowControls"></div>`;
		...
		while (!target.classList.contains('toolbar')) {
			...
			if (targetId === 'barBack') {
					...
					const menuOptions: IMenu = {
						...
						icon:
							itemId === 'toolbarVIP'
								? 'iconAccount'
								: useElement
								? useElement.getAttribute('xlink:href').substring(1)
								: undefined,
						click: () => {
				...
				if (!window.siyuan.config.readonly) {
					...
					dialogSetting.element
						.querySelector('.b3-tab-bar [data-name="account"]')
	...
	barSyncElement.addEventListener('mouseenter', (event) => {
		...
		fetchPost('/api/sync/getSyncInfo', {}, (response) => {
			...
			if (!window.siyuan.config.sync.enabled || (0 === window.siyuan.config.sync.provider && needSubscribe(''))) {
				...
				html =
					window.siyuan.languages._kernel[82].replace('%s', dayjs(response.data.synced).format('YYYY-MM-DD HH:mm')) +
					'<br>';
				...
				if (response.data.kernels.length > 0) {
					...
					html +=
						'&emsp;' +
						response.data.kernel +
						'/' +
						window.siyuan.config.system.kernelVersion +
						' (' +
						window.siyuan.config.system.os +
						'/' +
						window.siyuan.config.system.name +
						')<br>';
					html += window.siyuan.languages.otherOnlineKernels + '<br>';
					response.data.kernels.forEach((item: { os: string; ver: string; hostname: string; id: string }) => {
	...
	barSyncElement.setAttribute(
		'aria-label',
		window.siyuan.config.sync.stat ||
			window.siyuan.languages.syncNow + ' ' + updateHotkeyTip(window.siyuan.config.keymap.general.syncNow.custom)
	);
};
...
const openPlugin = (app: App, target: Element) => {
	...
	if (!isHuawei()) {
		menu.addItem({
			...
			click() {
				openSetting(app)
					.element.querySelector('.b3-tab-bar [data-name="bazaar"]')
					.dispatchEvent(new CustomEvent('click'));
			}
		});
	}
					...
					click() {
						if (hasUnpin) {
							window.siyuan.storage[Constants.LOCAL_PLUGINTOPUNPIN].splice(
								window.siyuan.storage[Constants.LOCAL_PLUGINTOPUNPIN].indexOf(item.id),
								1
							);
							item.classList.remove('fn__none');
						} else {
							window.siyuan.storage[Constants.LOCAL_PLUGINTOPUNPIN].push(item.id);
							window.siyuan.storage[Constants.LOCAL_PLUGINTOPUNPIN] = Array.from(
								new Set(window.siyuan.storage[Constants.LOCAL_PLUGINTOPUNPIN])
							);
