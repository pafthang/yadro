...
export const initStatus = (isWindow = false) => {
	...
	let barDockHTML = '';
	if (!isWindow) {
		barDockHTML = `<div id="barDock" class="toolbar__item ariaLabel${
			window.siyuan.config.readonly || isWindow ? ' fn__none' : ''
		}" aria-label="${window.siyuan.languages.toggleDock} ${updateHotkeyTip(
