...
export const setPanelFocus = (element: Element) => {
	if (element.getAttribute('data-type') === 'wnd') {
		setTitle(
			element.querySelector('.layout-tab-bar .item--focus[data-type="tab-header"] .item__text')?.textContent ||
	...
	if (element.getAttribute('data-type') === 'wnd') {
		...
		Array.from(element.classList).find((item) => {
			if (item.startsWith('sy__')) {
				document
					.querySelector(`.dock__item[data-type="${item.substring(4)}"]`)
	...
	targetWnd.children.forEach((item) => {
		if (item.model instanceof Editor) {
			...
			const range = focusByOffset(
				item.model.editor.protyle.wysiwyg.element.querySelector(`[data-node-id="${rangeData.id}"]`),
				rangeData.start,
				rangeData.end
			);
			if (range) {
...
export const exportLayout = (options: { cb: () => void; errorExit: boolean }) => {
	if (isWindow()) {
...
export const layoutToJSON = (layout: Layout | Wnd | Tab | Model, json: any, breakObj?: IObject) => {
	if (layout instanceof Layout) {
		...
		if (layout.parent) {
			if (layout.element.classList.contains('fn__flex-1')) {
				...
				json.size =
					(layout.parent.direction === 'tb' ? layout.element.clientHeight : layout.element.clientWidth) + 'px';
			}
		}
	...
	if (layout instanceof Layout || layout instanceof Wnd) {
		if (layout instanceof Layout && (layout.type === 'bottom' || layout.type === 'left' || layout.type === 'right')) {
...
export const addResize = (obj: Layout | Wnd) => {
	...
	const getMinSize = (element: HTMLElement) => {
		...
		Array.from(element.querySelectorAll('.file-tree')).find((item) => {
			if (
				item.classList.contains('sy__backlink') ||
				item.classList.contains('sy__graph') ||
				item.classList.contains('sy__globalGraph') ||
				item.classList.contains('sy__inbox')
			) {
	...
	const resizeWnd = (resizeElement: HTMLElement, direction: string) => {
		...
		resizeElement.addEventListener('mousedown', (event: MouseEvent) => {
			...
			documentSelf.onmousemove = (moveEvent: MouseEvent) => {
				...
				if (
					window.siyuan.layout.leftDock?.layout.element.isSameNode(previousElement) &&
					previousNowSize < getMinSize(previousElement)
				) {
					return;
				}
				if (
					window.siyuan.layout.rightDock?.layout.element.isSameNode(nextElement) &&
					nextNowSize < getMinSize(nextElement)
				) {
					return;
				}
				if (window.siyuan.layout.bottomDock?.layout.element.isSameNode(nextElement) && nextNowSize < 64) {
					return;
				}
