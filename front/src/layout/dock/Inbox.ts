...
<div class="fn__flex-1"></div>`;
    ...
    <span class="fn__space"></span>
    <span data-type="previous" class="block__icon b3-tooltips b3-tooltips__w" disabled="disabled" aria-label="${
			window.siyuan.languages.previousLabel
		}"><svg><use xlink:href="#iconLeft"></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="next" class="block__icon b3-tooltips b3-tooltips__w" disabled="disabled" aria-label="${
			window.siyuan.languages.nextLabel
		}"><svg><use xlink:href="#iconRight"></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="more" data-menu="true" class="block__icon b3-tooltips b3-tooltips__w" aria-label="${
			window.siyuan.languages.more
		}"><svg><use xlink:href="#iconMore"></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="min" class="block__icon b3-tooltips b3-tooltips__w" aria-label="${
			window.siyuan.languages.min
		} ${updateHotkeyTip(
			...
			while (target && !target.isEqualNode(this.element)) {
				...
				if (type === 'min') {
					...
					selectAllElement
						...
						.setAttribute(
							'xlink:href',
							this.element.lastElementChild.querySelectorAll('[*|href="#iconCheck"]').length ===
								this.element.lastElementChild.querySelectorAll('.b3-list-item').length
								? '#iconCheck'
								: '#iconUncheck'
						);
					window.siyuan.menus.menu.remove();
...
${data.shorthandContent}
    ...
    <span class="b3-list-item__text" title="${item.shorthandTitle}${
			item.shorthandTitle === item.shorthandDesc ? '' : '\n' + item.shorthandDesc
		}">${item.shorthandTitle}</span>
				...
				new MenuItem({
					...
					click: () => {
						...
						ids.forEach((id, index) => {
							removeTitle +=
								'<code class="fn__code">' +
								escapeHtml(this.data[id].shorthandTitle) +
								'</code>' +
								(index === ids.length - 1 ? '' : ', ');
						});
						confirmDialog(
							window.siyuan.languages.deleteOpConfirm,
							`${window.siyuan.languages.confirmDelete} ${removeTitle}?`,
							() => {
								if (itemElement) {
    ...
    <li class="b3-list--empty">
		...
		fetchPost('/api/inbox/getShorthands', { page: this.currentPage }, (response) => {
			...
			this.element.firstElementChild
				.querySelector('[data-type="selectall"] use')
				.setAttribute(
					'xlink:href',
					this.element.lastElementChild.querySelectorAll('[*|href="#iconCheck"]').length === selectCount &&
						selectCount !== 0
						? '#iconCheck'
						: '#iconUncheck'
				);
