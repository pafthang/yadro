...
export class Outline extends Model {
    ...
    <span data-type="expand" class="block__icon b3-tooltips b3-tooltips__sw${
			window.siyuan.storage[Constants.LOCAL_OUTLINE].keepExpand ? ' block__icon--active' : ''
		}" aria-label="${window.siyuan.languages.stickOpen} ${updateHotkeyTip(
			window.siyuan.config.keymap.editor.general.expand.custom
		)}">
...
<div class="fn__flex-1" style="margin-bottom: 8px"></div>`;
				...
				if (this.isPreview) {
					...
					checkFold(id, (zoomIn) => {
						openFileById({
							...
							id,
							action: zoomIn
								? [Constants.CB_GET_FOCUS, Constants.CB_GET_ALL, Constants.CB_GET_HTML]
								: [Constants.CB_GET_FOCUS, Constants.CB_GET_SETID, Constants.CB_GET_CONTEXT, Constants.CB_GET_HTML]
						});
					});
				}
			},
		...
		options.tab.panelElement.querySelector('[data-type="expand"]').addEventListener(
			'click',
			(
				event: MouseEvent & {
					target: Element;
		...
		options.tab.panelElement.addEventListener('click', (event: MouseEvent & { target: HTMLElement }) => {
			...
			while (target && !target.isEqualNode(options.tab.panelElement)) {
				if (target.classList.contains('block__icon')) {
					...
					target.isSameNode(this.headerElement.nextElementSibling) ||
					target.classList.contains('block__icons')
				) {
...
<span class="b3-list-item__text">${escapeHtml(ial.title)}</span>`;
	...
	private onTransaction(data: IWebSocketData) {
		...
		data.data[0].doOperations.forEach((item: IOperation) => {
			if (
				(item.action === 'update' || item.action === 'insert') &&
				(item.data.indexOf('data-type="NodeHeading"') > -1 ||
					item.data.indexOf(
						`<div contenteditable="true" spellcheck="${window.siyuan.config.editor.spellcheck}"><wbr></div>`
					) > -1)
			) {
				needReload = true;
	...
	public update(data: IWebSocketData, callbackId?: string) {
		...
		if (
			!this.isPreview &&
			this.openNodes[this.blockId] &&
			!this.headerElement.querySelector('[data-type="expand"]').classList.contains('block__icon--active')
		) {
