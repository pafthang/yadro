...
export class Backlink extends Model {
	...
	constructor(options: { app: App; tab: Tab; blockId: string; rootId?: string; type: 'pin' | 'local' }) {
		super({
			app: options.app,
			id: options.tab.id,
			callback() {
    ...
    <input class="b3-text-field search__label fn__none fn__size200" placeholder="${
			window.siyuan.languages.filterKeywordEnter
		}" />
    <span data-type="search" class="block__icon b3-tooltips b3-tooltips__sw" aria-label="${
			window.siyuan.languages.filter
    ...
    <span class="fn__space"></span>
    <span data-type="sort" data-sort="3" class="block__icon b3-tooltips b3-tooltips__sw" aria-label="${
			window.siyuan.languages.sort
		}"><svg><use xlink:href='#iconSort'></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="expand" class="block__icon b3-tooltips b3-tooltips__sw" aria-label="${
			window.siyuan.languages.expand
		} ${updateHotkeyTip(window.siyuan.config.keymap.editor.general.expand.custom)}">
        <svg><use xlink:href="#iconExpand"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="collapse" class="block__icon b3-tooltips b3-tooltips__sw" aria-label="${
			window.siyuan.languages.collapse
		} ${updateHotkeyTip(window.siyuan.config.keymap.editor.general.collapse.custom)}">
...
<div class="block__icons">
    ...
    <span data-type="search" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${
			window.siyuan.languages.filter
		}"><svg><use xlink:href='#iconFilter'></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="mSort" data-sort="3" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${
			window.siyuan.languages.sort
		}"><svg><use xlink:href='#iconSort'></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="mExpand" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${
			window.siyuan.languages.expand
		}">
        <svg><use xlink:href="#iconExpand"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="mCollapse" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${
			window.siyuan.languages.collapse
		}">
        <svg><use xlink:href="#iconContract"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="layout" class="block__icon b3-tooltips b3-tooltips__nw" aria-label="${
			window.siyuan.languages.down
		}">
...
<div class="backlinkMList fn__flex-1"></div>`;
						...
						case 'layout':
							if (this.mTree.element.style.flex) {
								...
								if (target.getAttribute('aria-label') === window.siyuan.languages.down) {
									...
									this.mTree.element.setAttribute(
										'style',
										`flex:none;height:${
											this.element.clientHeight - this.tree.element.previousElementSibling.clientHeight * 2
										}px`
									);
									target.setAttribute('aria-label', window.siyuan.languages.down);
									target.querySelector('use').setAttribute('xlink:href', '#iconDown');
	...
	private showSortMenu(type: string, sort: string) {
		const clickEvent = (currentSort: string) => {
			(type === 'sort' ? this.tree : this.mTree).element.previousElementSibling
				.querySelector(`[data-type="${type}"]`)
				.setAttribute('data-sort', currentSort);
			this.searchBacklinks();
		};
	...
	private toggleItem(liElement: HTMLElement, isMention: boolean) {
		...
		if (svgElement.classList.contains('b3-list-item__arrow--open')) {
			...
			this.editors.find((item, index) => {
				if (
					item.protyle.block.rootID === docId &&
					liElement.nextElementSibling &&
					item.protyle.element.isSameNode(liElement.nextElementSibling)
				) {
					item.destroy();
	...
	public saveStatus() {
		...
		if (this.mTree.element.style.flex) {
			...
			if (
				this.mTree.element.previousElementSibling.querySelector('[data-type="layout"]').getAttribute('aria-label') ===
				window.siyuan.languages.down
			) {
	...
	public render(data: {
		...
		if (this.status[this.blockId].backlinkMStatus === 2 || this.status[this.blockId].backlinkMStatus === 1) {
			...
			this.mTree.element.setAttribute(
				'style',
				`flex:none;height:${this.element.clientHeight - this.tree.element.previousElementSibling.clientHeight * 2}px`
			);
			layoutElement.setAttribute('aria-label', window.siyuan.languages.down);
			layoutElement.querySelector('use').setAttribute('xlink:href', '#iconDown');
		}
		this.tree.element.previousElementSibling
			.querySelector('[data-type="sort"]')
			.setAttribute('data-sort', this.status[this.blockId].sort);
		this.mTree.element.previousElementSibling
			.querySelector('[data-type="mSort"]')
			.setAttribute('data-sort', this.status[this.blockId].mSort);
