	...
	resizeTimeout = window.setTimeout(() => {
		...
		models.backlink.forEach((item) => {
			...
			if (mTreeElement.style.height && mTreeElement.style.height !== '0px' && item.element.clientHeight !== 0) {
				mTreeElement.style.height =
					item.element.clientHeight - mTreeElement.previousElementSibling.clientHeight * 2 + 'px';
			}
...
export const newCenterEmptyTab = (app: App) => {
    ...
    <div class="b3-list-item" id="editorEmptySearch">
        ...
        <span class="b3-list-item__meta">${updateHotkeyTip(
					window.siyuan.config.keymap.general.globalSearch.custom
				)}</span>
    </div>
    <div id="editorEmptyRecent" class="b3-list-item">
        ...
        <span class="b3-list-item__meta">${updateHotkeyTip(
					window.siyuan.config.keymap.general.recentDocs.custom
				)}</span>
    </div>
    <div id="editorEmptyHistory" class="b3-list-item${window.siyuan.config.readonly ? ' fn__none' : ''}">
        ...
        <span class="b3-list-item__meta">${updateHotkeyTip(
					window.siyuan.config.keymap.general.dataHistory.custom
				)}</span>
    </div>
...
export const closeTabByType = async (tab: Tab, type: 'closeOthers' | 'closeAll' | 'other', tabs?: Tab[]) => {
	if (type === 'closeOthers') {
		for (let index = 0; index < tab.parent.children.length; index++) {
			if (
				tab.parent.children[index].id !== tab.id &&
				!tab.parent.children[index].headElement.classList.contains('item--pin')
			) {
		...
		for (let index = 0; index < tab.parent.children.length; index++) {
			if (!tab.parent.children[index].headElement.classList.contains('item--pin')) {
				tab.parent.children[index].parent.removeTab(tab.parent.children[index].id, true);
				index--;
			}
		}
	} else if (tabs.length > 0) {
		for (const element of tabs) {
			if (!element.headElement.classList.contains('item--pin')) {
				element.parent.removeTab(element.id);
			}
		}
	}
