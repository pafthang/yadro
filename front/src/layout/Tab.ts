...
<span class="item__close"><svg><use xlink:href="#iconClose"></use></svg></span>`;
	...
	public pin() {
		if (
			!this.headElement.previousElementSibling ||
			(this.headElement.previousElementSibling &&
				this.headElement.previousElementSibling.classList.contains('item--pin'))
		) {
		...
		if (this.docIcon) {
			...
			if (iconElement) {
				...
				this.headElement
					...
					.insertAdjacentHTML('beforebegin', `<span class="item__icon">${unicode2Emoji(icon)}</span>`);
			}
	...
	public unpin() {
		if (
			!this.headElement.nextElementSibling ||
			(this.headElement.nextElementSibling && !this.headElement.nextElementSibling.classList.contains('item--pin'))
		) {
