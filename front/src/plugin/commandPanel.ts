...
export const commandPanel = (app: App) => {
	...
	app.plugins.forEach((plugin) => {
		plugin.commands.forEach((command) => {
			...
			liElement.innerHTML = `<span class="b3-list-item__text">${plugin.displayName}: ${
				command.langText || plugin.i18n[command.langKey]
			}</span>
<span class="b3-list-item__meta${isMobile() ? ' fn__none' : ''}">${updateHotkeyTip(command.customHotkey)}</span>`;
	...
	if (listElement.childElementCount === 0) {
		...
		liElement.innerHTML = `<span class="b3-list-item__text" style="-webkit-line-clamp: inherit;">${
			isMobile() ? window.siyuan.languages._kernel[122] : window.siyuan.languages.commandEmpty
		}</span>`;
		liElement.addEventListener('click', () => {
			dialog.destroy();
			/// #if !MOBILE
