...
const runCode = (code: string, sourceURL: string) => {
	return window.eval(
		'(function anonymous(require, module, exports){'.concat(code, '\n})\n//# sourceURL=').concat(sourceURL, '\n')
	);
};
	...
	if (!isWindow() || isMobile()) {
		...
		plugin.topBarIcons.forEach((element) => {
			if (isMobile()) {
				...
				document
					.querySelector('#' + (element.getAttribute('data-position') === 'right' ? 'barPlugins' : 'drag'))
					.before(element);
			}
		});
	...
	Object.keys(plugin.docks).forEach((key) => {
		...
		if (dock.config.position.startsWith('Left')) {
			window.siyuan.layout.leftDock.genButton(
				...
				dock.config.position === 'LeftBottom' ? 1 : 0,
				dock.config.index
			);
			...
			window.siyuan.layout.bottomDock.genButton(
				...
				dock.config.position === 'BottomRight' ? 1 : 0,
				dock.config.index
			);
			...
			window.siyuan.layout.rightDock.genButton(
				...
				dock.config.position === 'RightBottom' ? 1 : 0,
				dock.config.index
			);
		}
	});
