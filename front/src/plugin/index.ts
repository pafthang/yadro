...
export class Plugin {
	...
	constructor(options: { app: App; name: string; displayName: string; i18n: IObject }) {
	...
	public addIcons(svg: string) {
		document.body.insertAdjacentHTML(
			'afterbegin',
			`<svg data-name="${this.name}" style="position: absolute; width: 0; height: 0; overflow: hidden;" xmlns="http://www.w3.org/2000/svg">
<defs>${svg}</defs></svg>`
		);
	}
	...
	public addStatusBar(options: { element: HTMLElement; position?: 'right' | 'left' }) {
		/// #if !MOBILE
