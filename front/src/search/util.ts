				...
				bind(element) {
					element.addEventListener('click', (itemEvent) => {
						if (hasClosestByClassName(itemEvent.target as Element, 'b3-menu__action')) {
							...
							setStorageVal(Constants.LOCAL_SEARCHKEYS, window.siyuan.storage[Constants.LOCAL_SEARCHKEYS]);
							if (
								element.previousElementSibling?.classList.contains('b3-menu__separator') &&
...
export const genSearch = (app: App, config: ISearchOption, element: Element, closeCB?: () => void) => {
	...
	element.innerHTML = `<div class="fn__flex-column" style="height: 100%;${
		closeCB ? 'border-radius: var(--b3-border-radius-b);overflow: hidden;' : ''
	}">
    <div class="block__icons" style="overflow: auto">
        <span data-position="9bottom" data-type="previous" class="block__icon block__icon--show ariaLabel" disabled="disabled" aria-label="${
					window.siyuan.languages.previousLabel
				}"><svg><use xlink:href='#iconLeft'></use></svg></span>
        <span class="fn__space"></span>
        <span data-position="9bottom" data-type="next" class="block__icon block__icon--show ariaLabel" disabled="disabled" aria-label="${
        ...
        <span id="searchPathInput" data-position="9bottom" class="search__path ft__on-surface fn__flex-center ft__smaller fn__ellipsis ariaLabel" aria-label="${escapeAriaLabel(
					config.hPath
				)}">
        ...
        <span class="fn__space"></span>
        <span data-position="9bottom" id="searchInclude" ${enableIncludeChild ? '' : 'disabled'} aria-label="${
        ...
        <span class="fn__space"></span>
        <span id="searchPath" aria-label="${
					window.siyuan.languages.specifyPath
				}" class="block__icon block__icon--show ariaLabel" data-position="9bottom">
            <svg><use xlink:href="#iconFolder"></use></svg>
        </span>
        <span class="fn__space"></span>
        <span id="searchMore" aria-label="${
					window.siyuan.languages.more
				}" class="block__icon block__icon--show ariaLabel" data-position="9bottom">
            <svg><use xlink:href="#iconMore"></use></svg>
        </span>
        <span class="${closeCB ? '' : 'fn__none '}fn__space"></span>
        <span id="searchOpen" aria-label="${window.siyuan.languages.openInNewTab}" class="${
		closeCB ? '' : 'fn__none '
	}block__icon block__icon--show ariaLabel" data-position="9bottom">
            <svg><use xlink:href="#iconLayoutRight"></use></svg>
        </span>
        <span class="fn__space"></span>
        <span id="searchUnRef" aria-label="${
					window.siyuan.languages.listInvalidRefBlocks
				}" class="block__icon block__icon--show ariaLabel" data-position="9bottom">
            <svg><use xlink:href="#iconLinkOff"></use></svg>
        </span>
        <span class="fn__space"></span>
        <span id="searchAsset" aria-label="${
					window.siyuan.languages.searchAssetContent
				}" class="block__icon block__icon--show ariaLabel" data-position="9bottom">
    ...
    <div class="b3-form__icon search__header">
        <div style="position: relative" class="fn__flex-1">
            ...
            <input id="searchInput" class="b3-text-field b3-text-field--text" placeholder="${
							window.siyuan.languages.showRecentUpdatedBlocks
						}">
        </div>
        <div class="block__icons">
            <span id="searchFilter" aria-label="${
							window.siyuan.languages.searchType
						}" class="block__icon ariaLabel" data-position="9bottom">
                <svg><use xlink:href="#iconFilter"></use></svg>
            </span> 
            <span class="fn__space"></span>
            <span id="searchSyntaxCheck" aria-label="${
							window.siyuan.languages.searchMethod
						} ${methodText}" class="block__icon ariaLabel" data-position="9bottom">
                <svg><use xlink:href="#iconRegex"></use></svg>
            </span>
            <span class="fn__space"></span>
            <span id="searchReplace" aria-label="${
							window.siyuan.languages.replace
						}" class="block__icon ariaLabel" data-position="9bottom">
                <svg><use xlink:href="#iconReplace"></use></svg>
            </span>
            <span class="fn__space"></span>
            <span id="searchRefresh" aria-label="${
            ...
            <div class="fn__flex${config.group === 0 ? ' fn__none' : ''}">
                <span class="fn__space"></span>
                <span id="searchExpand" class="block__icon block__icon--show ariaLabel" data-position="9bottom" aria-label="${
									window.siyuan.languages.expand
								}">
                    <svg><use xlink:href="#iconExpand"></use></svg>
                </span>
                <span class="fn__space"></span>
                <span id="searchCollapse" class="block__icon block__icon--show ariaLabel" data-position="9bottom" aria-label="${
									window.siyuan.languages.collapse
								}">
    ...
    <div class="b3-form__icon search__header${config.hasReplace ? '' : ' fn__none'}">
        ...
        <span id="replaceFilter" aria-label="${
					window.siyuan.languages.replaceType
				}" class="block__icon ariaLabel fn__flex-center" data-position="9bottom">
            <svg><use xlink:href="#iconFilter"></use></svg>
        </span>
        <span class="fn__space"></span>
        <button id="replaceAllBtn" class="b3-button b3-button--small b3-button--outline fn__flex-center">${
					window.siyuan.languages.replaceAll
				}</button>
        <div class="fn__space"></div>
        <button id="replaceBtn" class="b3-button b3-button--small b3-button--outline fn__flex-center">↵ ${
					window.siyuan.languages.replace
				}</button>
        <div class="fn__space"></div>
    </div>
    ...
    <div class="search__tip${closeCB ? '' : ' fn__none'}">
        ...
        <kbd>${updateHotkeyTip(window.siyuan.config.keymap.editor.general.insertRight.custom)}/${updateHotkeyTip(
		'⌥Click'
	)}</kbd> ${window.siyuan.languages.searchTip4}
        <kbd>Esc</kbd> ${window.siyuan.languages.searchTip5}
    </div>
</div>
<div class="fn__flex-column fn__none" id="searchAssets" style="height: 100%;${
		closeCB ? 'border-radius: var(--b3-border-radius-b);overflow: hidden;' : ''
	}"></div>
<div class="fn__flex-column fn__none" id="searchUnRefPanel" style="height: 100%;${
		closeCB ? 'border-radius: var(--b3-border-radius-b);overflow: hidden;' : ''
	}">
    <div class="block__icons">
        <span data-type="unRefPrevious" class="block__icon block__icon--show ariaLabel" data-position="9bottom" disabled="disabled" aria-label="${
					window.siyuan.languages.previousLabel
				}"><svg><use xlink:href='#iconLeft'></use></svg></span>
        <span class="fn__space"></span>
        <span data-type="unRefNext" class="block__icon block__icon--show ariaLabel" data-position="9bottom" disabled="disabled" aria-label="${
					window.siyuan.languages.nextLabel
				}"><svg><use xlink:href='#iconRight'></use></svg></span>
        <span class="fn__space"></span>
        ...
        <span class="fn__space"></span>
        <span id="unRefMore" aria-label="${
					window.siyuan.languages.more
				}" class="block__icon block__icon--show ariaLabel" data-position="9bottom">
            <svg><use xlink:href="#iconMore"></use></svg>
        </span>
        <span class="fn__space"></span>
        <span id="searchUnRefClose" aria-label="${
					!closeCB ? window.siyuan.languages.stickSearch : window.siyuan.languages.globalSearch
				}" class="block__icon block__icon--show ariaLabel" data-position="9bottom">
...
<div class="fn__loading fn__loading--top"><img width="120px" src="/stage/loading-pure.svg"></div>`;
	...
	dragElement.addEventListener('mousedown', (event: MouseEvent) => {
		...
		const direction =
			window.siyuan.storage[Constants.LOCAL_SEARCHKEYS][closeCB ? 'layout' : 'layoutTab'] === 1 ? 'lr' : 'tb';
		...
		documentSelf.onmouseup = () => {
			...
			window.siyuan.storage[Constants.LOCAL_SEARCHKEYS][
				direction === 'lr' ? (closeCB ? 'col' : 'colTab') : closeCB ? 'row' : 'rowTab'
			] = nextElement[direction === 'lr' ? 'clientWidth' : 'clientHeight'] + 'px';
			setStorageVal(Constants.LOCAL_SEARCHKEYS, window.siyuan.storage[Constants.LOCAL_SEARCHKEYS]);
			if (direction === 'lr') {
				resize(edit.protyle);
			}
			...
			while (target && !target.isSameNode(element)) {
				...
				if (type === 'removeCriterion') {
					...
					if (!target.getAttribute('disabled')) {
						if (
							config.page < parseInt(target.parentElement.querySelector('#searchResult').getAttribute('data-pagecount'))
						) {
							config.page++;
					...
					movePathTo(
						(toPath, toNotebook) => {
							fetchPost('/api/filetree/getHPathsByPaths', { paths: toPath }, (response) => {
								...
								config.page = 1;
								searchPathInputElement.innerHTML = `${escapeGreat(
									config.hPath
								)}<svg class="search__rmpath"><use xlink:href="#iconCloseRound"></use></svg>`;
					...
					if (!target.getAttribute('disabled')) {
						let currentPage = parseInt(unRefPanelElement.querySelector('#searchUnRefResult').textContent);
						if (
							currentPage < parseInt(unRefPanelElement.querySelector('#searchUnRefResult').textContent.split('/')[1])
						) {
							currentPage++;
							getUnRefList(unRefPanelElement, unRefEdit, currentPage);
						}
					...
					openFile({
						...
						searchData: config,
						position:
							window.siyuan.layout.centerLayout.children.length > 1 || window.innerWidth > 1024 ? 'right' : undefined
					});
					if (closeCB) {
						closeCB();
					}
					...
					if (!target.getAttribute('disabled')) {
						let currentPage = parseInt(
							assetsElement.querySelector('#searchAssetResult .fn__flex-center').textContent.split('/')[0]
						);
						if (currentPage > 1) {
							currentPage--;
					...
					if (!target.getAttribute('disabled')) {
						...
						if (
							currentPage <
							parseInt(assetsElement.querySelector('#searchAssetResult .fn__flex-center').textContent.split('/')[1])
						) {
							currentPage++;
							assetInputEvent(assetsElement, localSearch, currentPage);
						}
					...
					if (type === 'search-new') {
						...
						const searchType = target.dataset.id
							? 'asset'
							: unRefPanelElement.classList.contains('fn__none')
							? 'doc'
							: 'unRef';
							...
							clickTimeout = window.setTimeout(() => {
								if (searchType === 'asset') {
									if (!target.classList.contains('b3-list-item--focus')) {
										...
										renderPreview(
											element.querySelector('#searchAssetPreview'),
											target.dataset.id,
									...
									if (event.altKey) {
										...
										(searchType === 'doc' ? searchPanelElement : unRefPanelElement)
...
const renderNextSearchMark = (options: { id: string; edit: Protyle; target: Element }) => {
	let matchElement;
	...
	if (matchElement) {
		...
		options.edit.protyle.contentElement.scrollTop =
			options.edit.protyle.contentElement.scrollTop +
...
export const getArticle = (options: { id: string; config?: ISearchOption; edit: Protyle; value?: string }) => {
				...
				fetchPost(
					...
					(getResponse) => {
						onGet({
							...
							action: zoomIn
								? [Constants.CB_GET_ALL, Constants.CB_GET_HTML]
								: [Constants.CB_GET_HL, Constants.CB_GET_HTML]
						});
						const matchElement = options.edit.protyle.wysiwyg.element.querySelector(
							`div[data-node-id="${options.id}"] span[data-type~="search-mark"]`
						);
						if (matchElement) {
							matchElement.classList.add('search-mark--hl');
		...
		(response) => {
			...
			if (config.group === 1) {
				if (currentList.nextElementSibling || currentList.previousElementSibling) {
					...
					const nextDocElement =
						currentList.parentElement.nextElementSibling ||
						currentList.parentElement.previousElementSibling.previousElementSibling?.previousElementSibling;
					if (nextDocElement) {
...
<span class="ft__on-surface">${window.siyuan.languages.findInDoc
						.replace('${x}', response.data.matchedRootCount)
						.replace('${y}', response.data.matchedBlockCount)}</span>`;
					loadingElement.classList.add('fn__none');
...
<span class="b3-list-item__text ariaLabel" style="color: var(--b3-theme-on-surface)" aria-label="${escapeAriaLabel(
				title
			)}">${escapeGreat(title)}</span>
</div><div>`;
			item.children.forEach((childItem, childIndex) => {
				resultHTML += `<div style="padding-left: 36px" data-type="search-item" class="b3-list-item${
					childIndex === 0 && index === 0 ? ' b3-list-item--focus' : ''
				}" data-node-id="${childItem.id}" data-root-id="${childItem.rootID}">
...
${getAttr(childItem)}
			...
			resultHTML += `<div data-type="search-item" class="b3-list-item${
				index === 0 ? ' b3-list-item--focus' : ''
			}" data-node-id="${item.id}" data-root-id="${item.rootID}">
<svg class="b3-list-item__graphic"><use xlink:href="#${getIconByType(item.type)}"></use></svg>
${unicode2Emoji(item.ial.icon, 'b3-list-item__graphic', true)}
<span class="b3-list-item__text">${item.content}</span>
${getAttr(item)}
<span class="b3-list-item__meta b3-list-item__meta--ellipsis ariaLabel" aria-label="${escapeAriaLabel(
				title
			)}">${escapeGreat(title)}</span>
</div>`;
		}
    ...
    <span class="b3-list-item__text">
        ${window.siyuan.languages.newFile} <mark>${
					(element.querySelector('#searchInput') as HTMLInputElement).value
