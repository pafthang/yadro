...
const getUnInitTab = (options: IOpenFileOptions) => {
	return getAllTabs().find((item) => {
		...
		if (initData) {
			...
			if (initObj.instance === 'Editor' && (initObj.rootId === options.rootID || initObj.blockId === options.rootID)) {
				...
				initObj.instance === 'Custom' &&
				options.custom &&
				objEquals(initObj.customModelData, options.custom.data)
			) {
				item.parent.switchTab(item.headElement);
				return true;
			}
		...
		tab = new Tab({
			...
			callback(tab) {
				if (options.custom.id) {
					...
					// plugin 0.8.3 历史兼容
					console.warn(
						'0.8.3 将移除 custom.fn 参数，请参照 https://github.com/siyuan-note/plugin-sample/blob/91a716358941791b4269241f21db25fd22ae5ff5/src/index.ts 将其修改为 custom.id'
					);
					tab.addModel(
						options.custom.fn({
							tab,
...
export const updatePanelByEditor = (options: {
	...
	if (options.protyle && options.protyle.path) {
		...
		if (
			options.protyle.element.classList.contains('fn__none') ||
...
export const updateBacklinkGraph = (models: IModels, protyle: IProtyle) => {
	// https://ld246.com/article/1637636106054/comment/1641485541929#comments
	if (
		(protyle && protyle.element.classList.contains('fn__none')) ||
		(protyle &&
