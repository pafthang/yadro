...
export const loadAssets = (data: IAppearance) => {
	...
	if (
		window.siyuan.config.appearance.modeOS &&
	...
	const defaultThemeAddress = `/appearance/themes/${data.mode === 1 ? 'midnight' : 'daylight'}/theme.css?v=${
	...
	if ((data.mode === 1 && data.themeDark !== 'midnight') || (data.mode === 0 && data.themeLight !== 'daylight')) {
		const themeAddress = `/appearance/themes/${data.mode === 1 ? data.themeDark : data.themeLight}/theme.css?v=${
			data.themeVer
		}`;
		if (styleElement) {
	...
	const pdfTheme =
		window.siyuan.config.appearance.mode === 0
			? window.siyuan.storage[Constants.LOCAL_PDFTHEME].light
	...
	const themeScriptAddress = `/appearance/themes/${data.mode === 1 ? data.themeDark : data.themeLight}/theme.js?v=${
		data.themeVer
	}`;
	...
	const iconURL = `/appearance/icons/${['ant', 'material'].includes(data.icon) ? data.icon : 'material'}/icon.js?v=${
		Constants.SIYUAN_VERSION
	}`;
...
export const initAssets = () => {
	...
	window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (event) => {
		...
		if (
			(window.siyuan.config.appearance.mode === 0 && OSTheme === 'light') ||
...
export const setInlineStyle = (set = true) => {
	...
	let style = `.b3-typography, .protyle-wysiwyg, .protyle-title {font-size:${
...
.protyle-wysiwyg [data-node-id].li > .protyle-action:after {height: ${window.siyuan.config.editor.fontSize}px;width: ${
		window.siyuan.config.editor.fontSize
	}px;margin:-${window.siyuan.config.editor.fontSize / 2}px 0 0 -${window.siyuan.config.editor.fontSize / 2}px}
.protyle-wysiwyg [data-node-id].li > .protyle-action svg {height: ${Math.max(
		14,
		window.siyuan.config.editor.fontSize - 8
	)}px}
...
.protyle-wysiwyg [data-node-id] {${window.siyuan.config.editor.rtl ? ' direction: rtl;' : ''}${
		window.siyuan.config.editor.justify ? ' text-align: justify;' : ''
	}}
...
.protyle-gutters button svg {height:${height}px}`;
	...
	if (set) {
		...
		if (siyuanStyle) {
			...
			document
				.querySelector('#pluginsStyle')
				.insertAdjacentHTML('beforebegin', `<style id="siyuanStyle">${style}</style>`);
		}
	}
	return style;
};
...
const updateMobileTheme = (OSTheme: string) => {
	if (isInIOS() || isInAndroid()) {
		setTimeout(() => {
			const backgroundColor = rgba2hex(
				getComputedStyle(document.body).getPropertyValue('--b3-theme-background').trim()
			);
			...
			if (isInIOS()) {
				window.webkit.messageHandlers.changeStatusBar.postMessage(
					(backgroundColor || (mode === 0 ? '#fff' : '#1e1e1e')) + ' ' + mode
				);
			} else if (isInAndroid()) {
