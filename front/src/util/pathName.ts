...
<ul id="foldList" class="fn__flex-1 fn__none b3-list b3-list--background${
			isMobile() ? ' b3-list--mobile' : ''
		}" style="overflow: auto;position: relative"></ul>
<div id="foldTree" class="fn__flex-1${
			isMobile() ? ' b3-list--mobile' : ''
		}" style="overflow: auto;position: relative"></div>
...
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.siyuan.languages.flashcardCard}">${item.flashcardCount}</span>`;
						...
						fileHTML += `<li class="b3-list-item${fileHTML === '' ? ' b3-list-item--focus' : ''}" data-path="${
							item.path
						}" data-box="${item.box}">
	...
	inputElement.addEventListener('keydown', (event: KeyboardEvent) => {
		...
		const currentPanelElement = searchListElement.classList.contains('fn__none')
			? searchTreeElement
			: searchListElement;
		...
		if (searchListElement.classList.contains('fn__none')) {
			if (
				(event.key === 'ArrowRight' &&
					!currentItemElement.querySelector('.b3-list-item__arrow--open') &&
					!currentItemElement.querySelector('.b3-list-item__toggle').classList.contains('fn__hidden')) ||
			...
			if (event.key === 'ArrowDown') {
				...
				if (
					currentPanelElement.scrollTop <
						currentItemElement.offsetTop - currentPanelElement.clientHeight + lineHeight ||
...
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.siyuan.languages.flashcardCard}">${item.flashcardCount}</span>`;
				...
				fileHTML += `<li title="${getDisplayName(item.name, true, true)} ${item.hSize}${
					item.bookmark ? '\n' + window.siyuan.languages.bookmark + ' ' + item.bookmark : ''
				}${item.name1 ? '\n' + window.siyuan.languages.name + ' ' + item.name1 : ''}${
					item.alias ? '\n' + window.siyuan.languages.alias + ' ' + item.alias : ''
				}${item.memo ? '\n' + window.siyuan.languages.memo + ' ' + item.memo : ''}${
					item.subFileCount !== 0 ? window.siyuan.languages.includeSubFile.replace('x', item.subFileCount) : ''
...
data-box="${notebookId}" class="b3-list-item" data-path="${item.path}">
    <span style="padding-left: ${
		...
		}px" class="b3-list-item__toggle b3-list-item__toggle--hl${item.subFileCount === 0 ? ' fn__hidden' : ''}">
        <svg class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
    </span>
    ${unicode2Emoji(
			item.icon || (item.subFileCount === 0 ? Constants.SIYUAN_IMAGE_FILE : Constants.SIYUAN_IMAGE_FOLDER),
