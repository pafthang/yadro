...
const focusStack = async (app: App, stack: IBackStack) => {
	...
	if (!document.contains(stack.protyle.element)) {
		...
		if (wnd) {
			...
			if (info.data.rootID === stack.id) {
				...
				Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${stack.id}"]`)).find(
					(item: HTMLElement) => {
	...
	Array.from(stack.protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${stack.id}"]`)).find(
		(item: HTMLElement) => {
			if (!hasClosestByAttribute(item, 'data-type', 'NodeBlockQueryEmbed')) {
				blockElement = item;
				return true;
