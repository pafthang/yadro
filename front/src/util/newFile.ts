						...
						fetchPost(
							'/api/filetree/createDocWithMd',
							{
								...
								path: pathPosix().join(
									responseHPath.data,
									data.data.path,
									optios.name || (data.data.path.endsWith('/') ? 'Untitled' : '')
								),
...
export const newFileBySelect = (protyle: IProtyle, selectText: string, nodeElement: HTMLElement, pathDir: string) => {
	const newFileName =
		replaceFileName(
			selectText.trim() ? selectText.trim() : protyle.lute.BlockDOM2Content(nodeElement.outerHTML).replace(/\n/g, '')
		) || 'Untitled';
	...
	fetchPost(
		...
		(idResponse) => {
			...
			if (idResponse.data && idResponse.data.length > 0) {
				insertHTML(
					`<span data-type="block-ref" data-id="${idResponse.data[0]}" data-subtype="d">${refText}</span>`,
					protyle,
					false,
					true
				);
				...
				fetchPost(
					...
					(response) => {
						insertHTML(
							`<span data-type="block-ref" data-id="${response.data}" data-subtype="d">${refText}</span>`,
							protyle,
							false,
							true
						);
