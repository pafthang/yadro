	...
	private genHTML(data: IBlockTree[]) {
		...
		data.forEach((item) => {
			...
			if (item.type === 'bookmark') {
				...
				iconHTML = `<svg class="b3-list-item__graphic popover__block" data-id="${
					item.id
				}"><use xlink:href="#${getIconByType(item.nodeType, item.subType)}"></use></svg>`;
			} else if (item.type === 'outline') {
				titleTip = ` aria-label="${escapeAriaLabel(Lute.BlockDOM2Content(item.name))}"`;
				iconHTML = `<svg class="b3-list-item__graphic popover__block" data-id="${
					item.id
				}" style="height: 22px;width: 10px;"><use xlink:href="#${getIconByType(
					item.nodeType,
					item.subType
				)}"></use></svg>`;
			}
...
${item.label ? "data-label='" + item.label + "'" : ''}>
    <span style="${style}" class="b3-list-item__toggle${showArrow ? ' b3-list-item__toggle--hl' : ''}${
				showArrow ? '' : ' fn__hidden'
			}">
        <svg data-id="${encodeURIComponent(item.name + item.depth)}" class="b3-list-item__arrow${
				hasChild ? ' b3-list-item__arrow--open' : ''
	...
	private genBlockHTML(data: IBlock[], show = false, type: string) {
		...
		data.forEach(
			(
				...
				let iconHTML;
				if (type === 'outline') {
					iconHTML = `<svg data-defids='["${item.defID}"]' class="b3-list-item__graphic popover__block" data-id="${
						item.id
					}" style="height: 22px;width: 10px;"><use xlink:href="#${getIconByType(
						item.type,
						item.subType
					...
					if (item.type === 'NodeDocument') {
						iconHTML = `<span data-defids='["${item.defID}"]' class="b3-list-item__graphic popover__block" data-id="${
							item.id
						}">${unicode2Emoji(item.ial.icon || Constants.SIYUAN_IMAGE_FILE)}</span>`;
					} else {
						iconHTML = `<svg data-defids='["${item.defID}"]' class="b3-list-item__graphic popover__block" data-id="${
							item.id
...
data-def-path="${item.defPath}">
    <span style="${style}" class="b3-list-item__toggle${item.children ? ' b3-list-item__toggle--hl' : ''}${
					item.children ? '' : ' fn__hidden'
				}">
        <svg data-id="${item.id}" class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
    </span>
    ${iconHTML}
    <span class="b3-list-item__text ariaLabel" data-position="parentE" ${
			type === 'outline' ? ' aria-label="' + escapeAriaLabel(Lute.BlockDOM2Content(item.content)) + '"' : ''
		}>${item.content}</span>
    ${countHTML}
    ${this.blockExtHTML || ''}
</li>`;
	...
	public toggleBlocks(liElement: Element) {
		...
		if (svgElement.classList.contains('b3-list-item__arrow--open')) {
			...
			if (
				liElement.nextElementSibling.nextElementSibling &&
				liElement.nextElementSibling.nextElementSibling.tagName === 'UL'
			) {
		...
		this.element.querySelectorAll('.b3-list-item__arrow').forEach((item) => {
			if (ids.includes(item.getAttribute('data-id'))) {
				...
				if (
					...
					item.parentElement.parentElement.nextElementSibling.tagName === 'UL'
				) {
