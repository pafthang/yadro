...
export class Protyle {
	...
	constructor(app: App, id: HTMLElement, options?: IOptions) {
		...
		if (!mergedOptions.action.includes(Constants.CB_GET_HISTORY)) {
				...
				msgCallback: (data) => {
					switch (data.cmd) {
						...
						case 'refreshAttributeView':
							Array.from(this.protyle.wysiwyg.element.querySelectorAll(`[data-av-id="${data.data.id}"]`)).forEach(
								(item: HTMLElement) => {
						...
						case 'rename':
							...
							this.protyle.wysiwyg.element
								.querySelectorAll(`[data-type~="block-ref"][data-id="${data.data.id}"]`)
								.forEach((item) => {
			...
			if (
				this.protyle.options.mode !== 'preview' &&
				options.rootId &&
	...
	private getDoc(mergedOptions: IOptions) {
		fetchPost(
			'/api/filetree/getDoc',
			{
				...
				size: mergedOptions.action?.includes(Constants.CB_GET_ALL)
	...
	private afterOnGet(mergedOptions: IOptions) {
		...
		this.protyle.wysiwyg.element.addEventListener('focusin', () => {
			...
			if (this.protyle && this.protyle.model) {
				let needUpdate = true;
				if (
					this.protyle.model.element.parentElement.parentElement.classList.contains('layout__wnd--active') &&
