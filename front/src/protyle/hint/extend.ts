...
<input class="b3-form__upload" type="file" ${
				protyle.options.upload.accept ? 'multiple="' + protyle.options.upload.accept + '"' : ''
			}></div>`
		},
		{
...
export const genHintItemHTML = (item: IBlock) => {
	...
	if (item.type === 'NodeDocument' && item.ial.icon) {
		...
		iconHTML = `<svg class="b3-list-item__graphic popover__block" data-id="${
			item.id
		}"><use xlink:href="#${getIconByType(item.type)}"></use></svg>`;
	}
	let attrHTML = '';
	if (item.name) {
...
<span class="b3-list-item__text">${window.siyuan.languages.newFile} <mark>${response.data.k}</mark></span></div>`
			...
			response.data.blocks.forEach((item: IBlock) => {
				let value = `<span data-type="block-ref" data-id="${item.id}" data-subtype="d">${
					item.name || item.refText.replace(new RegExp(Constants.ZWSP, 'g'), '')
				}</span>`;
				if (source === 'search') {
					value = `<span data-type="block-ref" data-id="${item.id}" data-subtype="s">${key}${Constants.ZWSP}${
						item.name || item.refText.replace(new RegExp(Constants.ZWSP, 'g'), '')
					}</span>`;
				}
				dataList.push({
...
export const hintRenderWidget = (value: string, protyle: IProtyle) => {
	...
	insertHTML(
		protyle.lute.SpinBlockDOM(
			`<iframe src="/widgets/${value}" data-subtype="widget" border="0" frameborder="no" framespacing="0" allowfullscreen="true"></iframe>`
		),
		protyle,
		true
	);
...
export const hintMoveBlock = (pathString: string, sourceElements: Element[], protyle: IProtyle) => {
	...
	if (topSourceElement) {
		...
		parentElement.getAttribute('data-subtype') === 'o' &&
		...
		parentElement.childElementCount === 0
	) {
		setTimeout(() => {
