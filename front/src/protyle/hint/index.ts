...
export class Hint {
	...
	constructor(protyle: IProtyle) {
		...
		this.element.addEventListener('click', (event) => {
			...
			if (
				btnElement &&
				!btnElement.classList.contains('emojis__item') &&
				!btnElement.classList.contains('emojis__type')
			) {
				this.fill(
					decodeURIComponent(btnElement.getAttribute('data-value')),
					protyle,
					false,
					this.source === 'search' ? isNotCtrl(event) : isOnlyMeta(event)
				);
			...
			if (typeElement) {
				const titleElement = emojisContentElement.querySelector(
					`[data-type="${typeElement.getAttribute('data-type')}"]`
				) as HTMLElement;
				if (titleElement) {
					...
					if (index) {
						...
						window.siyuan.emojis[parseInt(index)].items.forEach((emoji) => {
							html += `<button data-unicode="${emoji.unicode}" class="emojis__item ariaLabel" aria-label="${
								window.siyuan.config.lang === 'zh_CN' ? emoji.description_zh_cn : emoji.description
							}">
${unicode2Emoji(emoji.unicode)}</button>`;
	...
	public render(protyle: IProtyle) {
		...
		if (
			protyle.toolbar.range.startContainer.nodeType === 3 &&
			protyle.toolbar.range.startContainer.textContent === ''
		) {
	...
	public genLoading(protyle: IProtyle) {
		if (this.element.classList.contains('fn__none')) {
			this.element.innerHTML =
				'<div class="fn__loading" style="height: 128px;position: initial"><img width="64px" src="/stage/loading-pure.svg"></div>';
			...
			this.element.insertAdjacentHTML(
				'beforeend',
				'<div class="fn__loading"><img width="64px" src="/stage/loading-pure.svg"></div>'
			);
		}
	}
	...
	private getHTMLByData(data: IHintData[]) {
		...
		data.forEach((hintData, i) => {
			...
			let focusClass = '';
			if ((i === 1 && data[i].focus) || (i === 0 && (data.length === 1 || !data[1].focus))) {
				focusClass = ' b3-list-item--focus';
			}
			if (hintData.html === 'separator') {
				hintsHTML += '<div class="b3-menu__separator"></div>';
			} else {
				hintsHTML += `<button style="width: calc(100% - 16px)" class="b3-list-item b3-list-item--two${focusClass}" data-value="${encodeURIComponent(
					hintData.value
				)}">${hintData.html}</button>`;
			}
		});
	...
	public genHTML(data: IHintData[], protyle: IProtyle, hide = false, source: THintSource) {
		...
		if (this.source !== 'hint') {
			...
			searchElement.addEventListener('keydown', (event: KeyboardEvent) => {
				...
				if (event.key === 'Enter') {
					this.fill(
						decodeURIComponent(this.element.querySelector('.b3-list-item--focus').getAttribute('data-value')),
						protyle,
						false,
						isNotCtrl(event)
					);
	...
	private genSearchHTML(
		...
		nodeElement: false | HTMLElement,
		oldValue: string
	) {
		this.element.lastElementChild.innerHTML =
			'<div class="ft__center"><img style="height:32px;width:32px;" src="/stage/loading-pure.svg"></div>';
		fetchPost(
			...
			(response) => {
				...
				if (response.data.newDoc) {
					...
					searchHTML += `<button style="width: calc(100% - 16px)" class="b3-list-item b3-list-item--two${
						response.data.blocks.length === 0 ? ' b3-list-item--focus' : ''
...
<span class="b3-list-item__text">${window.siyuan.languages.newFile} <mark>${
						response.data.k
					}</mark></span></div></button>`;
				}
				response.data.blocks.forEach((item: IBlock, index: number) => {
					...
					}" data-value="${encodeURIComponent(blockRefHTML)}">
...
<div class="fn__flex${value ? ' fn__none' : ''}">
    <button data-type="0" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.languages.recentEmoji
		}">${unicode2Emoji('2b50')}</button>
    <button data-type="1" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[0][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f527')}</button>
    <button data-type="2" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[1][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f60d')}</button>
    <button data-type="3" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[2][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f433')}</button>
    <button data-type="4" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[3][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f96a')}</button>
    <button data-type="5" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[4][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f3a8')}</button>
    <button data-type="6" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[5][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f3dd-fe0f')}</button>
    <button data-type="7" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[6][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f52e')}</button>
    <button data-type="8" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[7][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('267e-fe0f')}</button>
    <button data-type="9" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[8][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f6a9')}</button>
</div>
</div>`;
	...
	public fill(value: string, protyle: IProtyle, updateRange = true, refIsS = false) {
				...
				fetchPost(
					...
					(response) => {
						...
						protyle.toolbar.setInlineMark(protyle, 'block-ref', 'range', {
							type: 'id',
							color: `${response.data}${Constants.ZWSP}${refIsS ? 's' : 'd'}${Constants.ZWSP}${(refIsS
								? fileNames[0]
								: realFileName
		...
		if (Constants.BLOCK_HINT_KEYS.includes(this.splitChar)) {
			...
			protyle.toolbar.setInlineMark(protyle, 'block-ref', 'range', {
				type: 'id',
				color: `${tempElement.getAttribute('data-id')}${Constants.ZWSP}${tempElement.getAttribute('data-subtype')}${
					Constants.ZWSP
			...
			if (value === '((' || value === '{{') {
				...
				fetchPost(
					...
					() => {
						insertHTML(
							`<span data-type="block-ref" data-id="${newSubDocId}" data-subtype="d">Untitled</span>`,
							protyle
						);
				...
				if (value === '![]()') {
					// https://github.com/siyuan-note/siyuan/issues/4586 1
					let newHTML = '';
					...
					let imgElement: HTMLElement =
						(range.startContainer.childNodes[range.startOffset - 1] as HTMLElement) ||
						(range.startContainer as HTMLElement);
					if (imgElement && imgElement.nodeType !== 3 && imgElement.classList.contains('img')) {
						// 已经找到图片
					} else if (
						imgElement.previousSibling?.nodeType !== 3 &&
						(imgElement.previousSibling as HTMLElement).classList.contains('img')
					) {
					...
					let newHTML = '';
					if (value === '<div>') {
						newHTML = `<div data-node-id="${id}" data-type="NodeHTMLBlock" class="render-node" data-subtype="block">${genIconHTML()}<div><protyle-html data-content=""></protyle-html><span style="position: absolute">${
							Constants.ZWSP
						}</span></div><div class="protyle-attr" contenteditable="false"></div></div>`;
					} else {
					...
					if (value === '<div>') {
						newHTML = `<div data-node-id="${Lute.NewNodeID()}" data-type="NodeHTMLBlock" class="render-node" data-subtype="block">${genIconHTML()}<div><protyle-html data-content=""></protyle-html><span style="position: absolute">${
							Constants.ZWSP
						}</span></div><div class="protyle-attr" contenteditable="false"></div></div>`;
					}
		...
		if (isEmojiPanel) {
			...
			if (event.key === 'ArrowLeft') {
				if (currentElement.previousElementSibling) {
					...
					currentElement.classList.remove('emojis__item--current');
					newCurrentElement = currentElement.parentElement.previousElementSibling.previousElementSibling
				...
				if (currentElement.nextElementSibling) {
					...
					currentElement.classList.remove('emojis__item--current');
					newCurrentElement = currentElement.parentElement.nextElementSibling.nextElementSibling
			...
			if (newCurrentElement) {
				...
				if (newCurrentElement.offsetTop - 8 < emojisContentElement.scrollTop) {
					...
					if (
						newCurrentElement.offsetTop + topHeight - this.element.clientHeight + newCurrentElement.clientHeight >
	...
	private getKey(currentLineValue: string, extend: IHintExtend[]) {
		...
		extend.forEach((item) => {
			...
			if (Constants.BLOCK_HINT_KEYS.includes(item.key) && currentLastIndex > -1) {
				...
				if (thirdLastIndex > -1) {
					currentLastIndex = Math.min(
						currentLastIndex,
						currentLineValue.lastIndexOf(item.key + item.key.substring(0, 1))
					);
				}
			}
			if (this.lastIndex < currentLastIndex) {
				if (
					...
					// 块搜索中忽略以上符号
				} else if (this.splitChar === '#' && (item.key === '/' || item.key === '、')) {
					// 标签中忽略以上符号
				} else {
