...
export const initUI = (protyle: IProtyle) => {
	...
	protyle.contentElement.addEventListener(
		...
		(event: WheelEvent) => {
			if (
				!window.siyuan.config.editor.fontSizeScrollZoom ||
...
export const addLoading = (protyle: IProtyle, msg?: string) => {
	...
	setTimeout(() => {
		if (protyle.element.getAttribute('data-loading') !== 'finished') {
			protyle.element.insertAdjacentHTML(
				'beforeend',
				`<div style="background-color: var(--b3-theme-background);flex-direction: column;" class="fn__loading wysiwygLoading">
...
export const setPadding = (protyle: IProtyle) => {
	...
	if (protyle.options.render.background) {
		...
		protyle.background.element
			.querySelector('.protyle-background__img .protyle-icons')
			.setAttribute('style', `right:${left}px`);
	}
	if (protyle.options.render.title) {
	...
	if (window.siyuan.config.editor.displayBookmarkIcon) {
		...
		if (editorAttrElement) {
			editorAttrElement.innerHTML = `.protyle-wysiwyg--attr .b3-tooltips:after { max-width: ${
				protyle.wysiwyg.element.clientWidth - left - right
			}px; }`;
		}
	}
