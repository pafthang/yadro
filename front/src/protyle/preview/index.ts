...
import { openAsset, updateOutline } from '../../editor/util';
	...
	constructor(protyle: IProtyle) {
		...
		for (let i = 0; i < actions.length; i++) {
			...
			if (typeof action === 'object') {
				actionHtml.push(
					`<button type="button" data-type="${action.key}" class="${action.className}">${action.text}</button>`
				);
				continue;
			}
			switch (action) {
				case 'desktop':
					actionHtml.push(
						`<button type="button"${
							protyle.wysiwyg.element.style.padding ? ' class="protyle-preview__action--current"' : ''
						} data-type="desktop">Desktop</button>`
					);
					break;
				case 'tablet':
				...
				case 'mp-wechat':
					actionHtml.push(
						'<button type="button" data-type="mp-wechat" class="b3-tooltips b3-tooltips__w" aria-label="复制到公众号"><svg><use xlink:href="#iconMp"></use></svg></button>'
					);
					break;
				case 'zhihu':
					actionHtml.push(
						'<button type="button" data-type="zhihu" class="b3-tooltips b3-tooltips__w" aria-label="复制到知乎"><svg><use xlink:href="#iconZhihu"></use></svg></button>'
					);
					break;
				case 'yuque':
					actionHtml.push(
						'<button type="button" data-type="yuque" class="b3-tooltips b3-tooltips__w" aria-label="复制到语雀"><svg><use xlink:href="#iconYuque"></use></svg></button>'
					);
					break;
			}
		}
		...
		if (type === 'mp-wechat') {
			...
			copyElement.querySelectorAll('[data-subtype="math"]').forEach((item: HTMLElement) => {
				...
				item.outerHTML = `<img class="Formula-image" data-eeimg="true" src="//www.zhihu.com/equation?tex=" alt="${item.getAttribute(
					'data-content'
				)}" style="${item.tagName === 'DIV' ? 'display: block; max-width: 100%;' : ''}margin: 0 auto;">`;
			});
