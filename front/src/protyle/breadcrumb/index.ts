...
<button class="protyle-breadcrumb__icon fn__none ariaLabel" aria-label="${updateHotkeyTip(
			window.siyuan.config.keymap.editor.general.exitFocus.custom
		)}" data-type="exit-focus">${window.siyuan.languages.exitFocus}</button>
${padHTML}
<button class="block__icon fn__flex-center ariaLabel${window.siyuan.config.readonly ? ' fn__none' : ''}" aria-label="${
			window.siyuan.languages.lockEdit
		}" data-type="readonly"><svg><use xlink:href="#iconUnlock"></use></svg></button>
<button class="block__icon fn__flex-center ariaLabel" data-type="doc" aria-label="${
			isMac() ? window.siyuan.languages.gutterTip2 : window.siyuan.languages.gutterTip2.replace('⇧', 'Shift+')
		}"><svg><use xlink:href="#iconFile"></use></svg></button>
<button class="block__icon fn__flex-center ariaLabel" data-type="more" aria-label="${
			window.siyuan.languages.more
		}"><svg><use xlink:href="#iconMore"></use></svg></button>
<button class="block__icon fn__flex-center fn__none ariaLabel" data-type="context" aria-label="${
			window.siyuan.languages.context
				...
				if (id) {
					...
					if (protyle.options.render.breadcrumbDocName && window.siyuan.ctrlIsPressed) {
						openFileById({
							...
							id,
							action:
								id === protyle.block.rootID ? [Constants.CB_GET_FOCUS] : [Constants.CB_GET_FOCUS, Constants.CB_GET_ALL]
						});
					} else {
						zoomOut({ protyle, id });
					}
		...
		this.element.addEventListener('mouseover', (event) => {
			...
			if (svgElement) {
				...
				const nodeElement = protyle.wysiwyg.element.querySelector(
					`[data-node-id="${svgElement.getAttribute('data-node-id')}"]`
				);
				if (nodeElement) {
...
<button class="b3-button b3-button--white">${window.siyuan.languages.endRecord}</button></div>`,
			-1
		);
		document.querySelector(`#message [data-id="${this.messageId}"] button`).addEventListener('click', () => {
			this.mediaRecorder.stopRecording();
			hideMessage(this.messageId);
			const file: File = new File([this.mediaRecorder.buildWavFileBlob()], `record${new Date().getTime()}.wav`, {
				type: 'video/webm'
			});
	...
	private genMobileMenu(protyle: IProtyle) {
		...
		if (getSelection().rangeCount > 0) {
			...
			if (
				!protyle.wysiwyg.element.isEqualNode(range.startContainer) &&
				!protyle.wysiwyg.element.contains(range.startContainer)
			) {
				blockElement =
					getNoContainerElement(protyle.wysiwyg.element.firstElementChild) || protyle.wysiwyg.element.firstElementChild;
			} else {
		...
		fetchPost(
			...
			(response) => {
				...
				if (!protyle.contentElement.classList.contains('fn__none') && !protyle.disabled) {
					...
					uploadMenu.querySelector('input').addEventListener(
						'change',
						(
							event: InputEvent & {
					...
					if (!isInAndroid()) {
						window.siyuan.menus.menu.append(
							new MenuItem({
								...
								icon: 'iconRecord',
								label: this.mediaRecorder?.isRecording
									? window.siyuan.languages.endRecord
									: window.siyuan.languages.startRecord,
								click: async () => {
							...
							new MenuItem({
								...
								click() {
									confirmDialog(
										...
										window.siyuan.languages.share2LiandiConfirmTip,
										() => {
	...
	public render(protyle: IProtyle, update = false) {
		...
		if (getSelection().rangeCount > 0) {
			...
			if (
				...
				if (protyle.element.id === 'searchPreview') {
					...
					blockElement =
						getNoContainerElement(protyle.wysiwyg.element.firstElementChild) ||
		...
		if (id === this.id && !update) {
			...
			const currentElement = protyle.breadcrumb.element.querySelector(
				`[data-node-id="${protyle.block.showAll ? protyle.block.id : protyle.block.parentID}"]`
			);
			if (currentElement) {
		...
		if (
			...
			this.element.parentElement.parentElement.classList.contains('card__block')
		) {
			// 闪卡面包屑不能显示答案
			...
			response.data.forEach((item: IBreadcrumb, index: number) => {
				...
				if (index === 0 && !protyle.options.render.breadcrumbDocName) {
					html += `<span class="protyle-breadcrumb__item${
						isCurrent ? ' protyle-breadcrumb__item--active' : ''
					}" data-node-id="${item.id}"${response.data.length === 1 ? ' style="max-width:none"' : ''}>
    <svg class="popover__block" data-id="${item.id}"><use xlink:href="#${getIconByType(
						item.type,
						item.subType
					)}"></use></svg>
</span>`;
					...
					html += `<span class="protyle-breadcrumb__item${
						isCurrent ? ' protyle-breadcrumb__item--active' : ''
					}" data-node-id="${item.id}"${response.data.length === 1 || index === 0 ? ' style="max-width:none"' : ''}>
    <svg class="popover__block" data-id="${item.id}"><use xlink:href="#${getIconByType(
						item.type,
						item.subType
					)}"></use></svg>
    <span class="protyle-breadcrumb__text" title="${item.name}">${item.name}</span>
			...
			if (this.element.lastElementChild) {
				this.element.scrollLeft =
					(this.element.lastElementChild as HTMLElement).offsetLeft - this.element.clientWidth + 14;
			}
		});
		/// #endif
	}
