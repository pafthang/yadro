...
export const exportImage = (id: string) => {
    ...
    <div style="${isMobile() ? 'padding: 16px;margin: 8px 0' : 'padding: 48px;margin: 8px 0'};" class="export-img">
        <div class="protyle-wysiwyg${
					window.siyuan.config.editor.displayBookmarkIcon ? ' protyle-wysiwyg--attr' : ''
				}"></div>
...
<div class="b3-dialog__action">
    <label class="fn__flex">
        ...
        <input id="keepFold" class="b3-switch fn__flex-center" type="checkbox" ${
					window.siyuan.storage[Constants.LOCAL_EXPORTIMG].keepFold ? 'checked' : ''
				}>
    </label>
    <label class="fn__flex" style="margin-left: 24px">
        ${window.siyuan.languages.export30}
        <span class="fn__space"></span>
        <input id="watermark" class="b3-switch fn__flex-center" type="checkbox" ${
					window.siyuan.storage[Constants.LOCAL_EXPORTIMG].watermark ? 'checked' : ''
				}>
    </label>
 ...
 <div class="fn__loading"><img height="128px" width="128px" src="stage/loading-pure.svg"></div>`,
	...
	btnsElement[1].addEventListener('click', () => {
		...
		setTimeout(() => {
			addScript('/stage/protyle/js/html2canvas.min.js?v=1.4.1', 'protyleHtml2canvas').then(() => {
				window
					.html2canvas(exportDialog.element.querySelector('.b3-dialog__content'), {
						useCORS: true
					})
					.then((canvas) => {
		...
		if (watermarkElement.checked) {
			if (window.siyuan.config.export.imageWatermarkDesc) {
				...
				if (window.siyuan.config.export.imageWatermarkStr.startsWith('http')) {
					watermarkPreviewElement.setAttribute(
						'style',
						`background-image: url(${window.siyuan.config.export.imageWatermarkStr});background-repeat: repeat;position: absolute;top: 0;left: 0;width: 100%;height: 100%;border-radius: var(--b3-border-radius-b);`
					);
				} else {
					addScript('/stage/protyle/js/html2canvas.min.js?v=1.4.1', 'protyleHtml2canvas').then(() => {
						...
						watermarkPreviewElement.setAttribute(
							'style',
							`width: ${width}px;height: ${width}px;display: flex;justify-content: center;align-items: center;color: var(--b3-border-color);font-size: 14px;`
						);
						watermarkPreviewElement.innerHTML = `<div style="transform: rotate(-45deg)">${window.siyuan.config.export.imageWatermarkStr}</div>`;
						window
							.html2canvas(watermarkPreviewElement, {
								useCORS: true,
								scale: 1
							})
							.then((canvas) => {
								...
								watermarkPreviewElement.setAttribute(
									...
									`background-image: url(${canvas.toDataURL(
										'image/png'
									)});background-repeat: repeat;position: absolute;top: 0;left: 0;width: 100%;height: 100%;border-radius: var(--b3-border-radius-b);`
								);
							});
					});
				}
	...
	const refreshPreview = (response: IWebSocketData) => {
		...
		previewElement
			.querySelectorAll(
				'[data-type~="mark"], [data-type~="u"], [data-type~="text"], [data-type~="code"], [data-type~="tag"], [data-type~="kbd"]'
			)
			.forEach((markItem: HTMLElement) => {
				markItem.childNodes.forEach((item) => {
					let spanHTML = '';
					Array.from(item.textContent).forEach((str) => {
						spanHTML += `<span style="${
							markItem.getAttribute('style') || ''
						};border-radius: 0;padding-left: 0;padding-right: 0;box-shadow: none;border-left:0;border-right:0;border-top:0" data-type="${markItem.getAttribute(
							'data-type'
						)}">${str}</span>`;
					});
				...
				if (markItem.childNodes.length > 0) {
					...
					markItem.setAttribute(
						'data-type',
						markItem.getAttribute('data-type').replace(/mark|u|text|code|tag|kbd/g, '')
					);
				}
			});
			...
			if (item.clientWidth > item.parentElement.clientWidth) {
				item.setAttribute(
					...
					`margin-bottom:${
						...
						1
					}px;transform: scale(${item.parentElement.clientWidth / item.clientWidth});transform-origin: top left;`
				);
