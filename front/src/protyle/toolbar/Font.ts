...
export const appearanceMenu = (protyle: IProtyle, nodeElements?: Element[]) => {
    ...
    <kbd class="ft__on-surface fn__flex-center">${updateHotkeyTip(
			window.siyuan.config.keymap.editor.insert.lastUsed.custom
		)}</kbd>
</div>
...
<div class="fn__hr--small"></div>
<div class="fn__flex">
    <button data-type="style2" class="protyle-font__style" style="-webkit-text-stroke: 0.2px var(--b3-theme-on-background);-webkit-text-fill-color : transparent;">${
			window.siyuan.languages.hollow
		}</button>
    <button data-type="style4" class="protyle-font__style" style="text-shadow: 1px 1px var(--b3-theme-surface-lighter), 2px 2px var(--b3-theme-surface-lighter), 3px 3px var(--b3-theme-surface-lighter), 4px 4px var(--b3-theme-surface-lighter)">${
			window.siyuan.languages.shadow
		}</button>
</div>
			...
			if (target.tagName === 'BUTTON') {
				if (dataType === 'style1') {
					fontEvent(
						...
						nodeElements,
						dataType,
						target.style.backgroundColor + Constants.ZWSP + target.style.color
					);
	...
	if (nodeElements && nodeElements.length > 0) {
		updateBatchTransaction(nodeElements, protyle, (e: HTMLElement) => {
			if (type === 'clear') {
				...
				e.style.textShadow =
					'1px 1px var(--b3-theme-surface-lighter), 2px 2px var(--b3-theme-surface-lighter), 3px 3px var(--b3-theme-surface-lighter), 4px 4px var(--b3-theme-surface-lighter)';
			} else if (type === 'color') {
				e.style.color = color;
...
export const hasSameTextStyle = (currentElement: HTMLElement, sideElement: HTMLElement, textObj: ITextOption) => {
	...
	if (textObj.type === 'id') {
		if (currentElement.nodeType !== 3) {
			return (
				currentElement.getAttribute('data-id') === sideElement.getAttribute('data-id') &&
		...
		return (
			blockRefData[0] === sideElement.getAttribute('data-id') &&
	...
	if (textObj.type === 'color') {
		return (
			textObj.color === sideElement.style.color &&
			webkitTextFillColor === sideElement.style.webkitTextFillColor &&
			webkitTextStroke === sideElement.style.webkitTextStroke &&
			textShadow === sideElement.style.textShadow &&
			fontSize === sideElement.style.fontSize &&
	...
	if (textObj.type === 'style1') {
		return (
			textObj.color.split(Constants.ZWSP)[0] === sideElement.style.color &&
			webkitTextFillColor === sideElement.style.webkitTextFillColor &&
			webkitTextStroke === sideElement.style.webkitTextStroke &&
			textShadow === sideElement.style.textShadow &&
			fontSize === sideElement.style.fontSize &&
