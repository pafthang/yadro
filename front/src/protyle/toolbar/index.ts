...
export class Toolbar {
	...
	public render(protyle: IProtyle, range: Range, event?: KeyboardEvent) {
		...
		if (
			...
			(range.commonAncestorContainer.nodeType !== 3 &&
				(range.commonAncestorContainer as HTMLElement).classList.contains('img'))
		) {
			this.element.classList.add('fn__none');
			return;
		}
	...
	public setInlineMark(protyle: IProtyle, type: string, action: 'range' | 'toolbar', textObj?: ITextOption) {
		...
		if (
			previousElement &&
			nextElement &&
			previousElement.isSameNode(nextElement) &&
			contents.firstChild?.nodeType === 3
		) {
		...
		const toolbarElement = isMobile()
			? document.querySelector('#keyboardToolbar .keyboard__dynamic').nextElementSibling
			: this.element;
		...
		if (
			...
			if (type === 'clear') {
				toolbarElement
					.querySelectorAll(
						'[data-type="em"],[data-type="u"],[data-type="s"],[data-type="mark"],[data-type="sup"],[data-type="sub"],[data-type="strong"]'
					)
					.forEach((item) => {
						item.classList.remove('protyle-toolbar__item--current');
					});
		...
		if (
			...
			(this.range.startContainer as HTMLElement).tagName === 'SPAN' &&
		...
		for (let i = 0; i < newNodes.length; i++) {
			...
			if (
				currentNewNode.nodeType !== 3 &&
				nextNewNode &&
				nextNewNode.nodeType !== 3 &&
				nextNewNode.tagName === currentNewNode.tagName &&
				isArrayEqual(
					(nextNewNode.getAttribute('data-type') || '').split(' '),
					(currentNewNode.getAttribute('data-type') || '').split(' ')
				) &&
				...
				if (currentType.indexOf('inline-math') > -1) {
					...
					nextNewNode.setAttribute(
						'data-content',
						currentNewNode.getAttribute('data-content') + nextNewNode.getAttribute('data-content')
					);
				} else {
					...
					if (currentType.indexOf('inline-memo') > -1) {
						nextNewNode.setAttribute(
							...
							(currentNewNode.getAttribute('data-inline-memo-content') || '') +
		...
		if (nextIndex) {
			...
			if (
				lastNewNode.nodeType !== 3 &&
				(lastNewNode as HTMLElement).getAttribute('data-type').indexOf('inline-math') > -1
			) {
				...
				if (mathNextSibling && mathNextSibling.nodeType === 3) {
					// https://github.com/siyuan-note/siyuan/issues/6065
...
- baz`;
		...
		const isPin =
			this.subElement.querySelector('[data-type="pin"]')?.getAttribute('aria-label') === window.siyuan.languages.unpin;
		...
		this.subElement.innerHTML = `<div ${
			isPin && this.subElement.firstElementChild.getAttribute('data-drag') === 'true' ? 'data-drag="true"' : ''
    ...
    <button data-type="refresh" class="block__icon block__icon--show b3-tooltips b3-tooltips__nw${
			isPin && !this.subElement.querySelector('[data-type="refresh"]').classList.contains('block__icon--active')
				? ''
				: ' block__icon--active'
		}${types.includes('NodeBlockQueryEmbed') ? ' fn__none' : ''}" aria-label="${
			window.siyuan.languages.refresh
		}"><svg><use xlink:href="#iconRefresh"></use></svg></button>
    <span class="fn__space"></span>
    <button data-type="before" class="block__icon block__icon--show b3-tooltips b3-tooltips__nw${
			protyle.disabled ? ' fn__none' : ''
		}" aria-label="${window.siyuan.languages['insert-before']}"><svg><use xlink:href="#iconBefore"></use></svg></button>
    <span class="fn__space${protyle.disabled ? ' fn__none' : ''}"></span>
    <button data-type="after" class="block__icon block__icon--show b3-tooltips b3-tooltips__nw${
			protyle.disabled ? ' fn__none' : ''
		}" aria-label="${window.siyuan.languages['insert-after']}"><svg><use xlink:href="#iconAfter"></use></svg></button>
    <span class="fn__space${protyle.disabled ? ' fn__none' : ''}"></span>
    <button data-type="export" class="block__icon block__icon--show b3-tooltips b3-tooltips__nw" aria-label="${
			window.siyuan.languages.export
		} ${window.siyuan.languages.image}"><svg><use xlink:href="#iconImage"></use></svg></button>
    <span class="fn__space"></span>
    <button data-type="pin" class="block__icon block__icon--show b3-tooltips b3-tooltips__nw" aria-label="${
			isPin ? window.siyuan.languages.unpin : window.siyuan.languages.pin
		}"><svg><use xlink:href="#icon${isPin ? 'Unpin' : 'Pin'}"></use></svg></button>
    <span class="fn__space"></span>
    <button data-type="close" class="block__icon block__icon--show b3-tooltips b3-tooltips__nw" aria-label="${
			window.siyuan.languages.close
		}"><svg style="width: 10px"><use xlink:href="#iconClose"></use></svg></button>
</div>
<textarea ${
			protyle.disabled ? ' readonly' : ''
		} spellcheck="false" class="b3-text-field b3-text-field--text fn__block" placeholder="${placeholder}" style="${
			isMobile() ? '' : 'width:' + Math.max(480, renderElement.clientWidth * 0.7) + 'px'
		};max-height:calc(80vh - 44px);min-height: 48px;min-width: 268px;border-radius: 0 0 var(--b3-border-radius-b) var(--b3-border-radius-b);font-family: var(--b3-font-family-code);"></textarea></div>`;
		const autoHeight = () => {
			...
			if (this.subElement.clientHeight <= window.innerHeight - bottom || this.subElement.clientHeight <= nodeRect.top) {
				if (types.includes('inline-math') || isInlineMemo) {
					...
					setPosition(
						this.subElement,
						nodeRect.left + (nodeRect.width - this.subElement.clientWidth) / 2,
						bottom,
						nodeRect.height || 26
					);
				}
		...
		const exportImg = () => {
			...
			if (renderElement.getAttribute('data-subtype') === 'plantuml') {
				fetch(renderElement.querySelector('img').getAttribute('src'))
		...
		if (types.includes('NodeHTMLBlock')) {
			textElement.value = Lute.UnEscapeHTMLStr(
				renderElement.querySelector('protyle-html').getAttribute('data-content') || ''
			);
		} else if (isInlineMemo) {
		...
		this.subElementCloseCB = () => {
			...
			if (
				...
				// $$ 中间输入后再 ESC 光标无法定位
				(getSelection().rangeCount > 0 &&
					hasClosestByClassName(getSelection().getRangeAt(0).startContainer, 'protyle-util'))
			) {
    ...
    <div class="b3-list fn__flex-1 b3-list--background" style="position: relative">${html}</div>
		...
		inputElement.addEventListener('keydown', (event: KeyboardEvent) => {
			...
			if (event.key === 'Enter') {
				...
				if (
					lineNumber === 'true' ||
					(lineNumber !== 'false' && window.siyuan.config.editor.codeSyntaxHighlightLineNum)
				) {
		...
		listElement.addEventListener('click', (event) => {
			...
			languageElement.textContent =
				listElement.textContent === window.siyuan.languages.clear ? '' : listElement.textContent;
			window.siyuan.storage[Constants.LOCAL_CODELANG] = languageElement.textContent;
			setStorageVal(Constants.LOCAL_CODELANG, window.siyuan.storage[Constants.LOCAL_CODELANG]);
			const nodeElement = hasClosestBlock(languageElement);
			if (nodeElement) {
...
<div style="width: 520px;${
			isMobile() || window.outerWidth < window.outerWidth / 2 + 520 ? 'display:none' : ''
		};overflow: auto;"></div>
</div>`;
		...
		inputElement.addEventListener('keydown', (event: KeyboardEvent) => {
			...
			if (event.key === 'Enter') {
				if (!isEmpty) {
					hintRenderTemplate(
						decodeURIComponent(this.subElement.querySelector('.b3-list-item--focus').getAttribute('data-value')),
						protyle,
						nodeElement
					);
				} else {
			...
			fetchPost(
				...
				(response) => {
					...
					response.data.blocks.forEach((item: { path: string; content: string }, index: number) => {
						searchHTML += `<div data-value="${item.path}" class="b3-list-item${
							index === 0 ? ' b3-list-item--focus' : ''
						}">${item.content}</div>`;
					});
					listElement.innerHTML =
						searchHTML || `<li class="b3-list--empty">${window.siyuan.languages.emptyContent}</li>`;
						...
						() => {
							if (iconElement.parentElement.parentElement.childElementCount === 1) {
								...
								if (iconElement.parentElement.classList.contains('b3-list-item--focus')) {
									const sideElement =
										iconElement.parentElement.previousElementSibling || iconElement.parentElement.nextElementSibling;
		...
		fetchPost(
			...
			(response) => {
				let html = '';
				response.data.blocks.forEach((item: { path: string; content: string }, index: number) => {
					html += `<div data-value="${item.path}" class="b3-list-item--hide-action b3-list-item${
						index === 0 ? ' b3-list-item--focus' : ''
					}">
<span class="b3-list-item__text">${item.content}</span>`;
    ...
    <svg><use xlink:href="#iconTrashcan"></use></svg>
				...
				this.subElement.querySelector('.b3-list--background').innerHTML =
				...
				(this.subElement.firstElementChild as HTMLElement).style.maxHeight =
					Math.min(window.innerHeight * 0.8, window.innerHeight - this.subElement.getBoundingClientRect().top) -
					16 +
					'px';
				/// #else
				setPosition(this.subElement, 0, 0);
	...
	public showContent(protyle: IProtyle, range: Range, nodeElement: Element) {
		...
		let html = '';
		const hasCopy =
			range.toString() !== '' || (range.cloneContents().childNodes[0] as HTMLElement)?.classList?.contains('emoji');
		if (hasCopy) {
			html +=
				'<button class="keyboard__action" data-action="copy"><svg><use xlink:href="#iconCopy"></use></svg></button>';
			if (!protyle.disabled) {
...
<button class="keyboard__action" data-action="select"><svg><use xlink:href="#iconSelect"></use></svg></button>`;
		...
		if (hasCopy || !protyle.disabled) {
			html +=
				'<button class="keyboard__action" data-action="more"><svg><use xlink:href="#iconMore"></use></svg></button>';
		}
		...
		this.subElement.lastElementChild.addEventListener('click', async (event) => {
			...
			if (action === 'copy') {
				...
				this.subElement.lastElementChild.innerHTML = `<button class="keyboard__action${
					hasCopy ? '' : ' fn__none'
				}" data-action="copyPlainText"><span>${window.siyuan.languages.copyPlainText}</span></button>
<div class="keyboard__split${hasCopy ? '' : ' fn__none'}"></div>
<button class="keyboard__action${protyle.disabled ? ' fn__none' : ''}" data-action="pasteAsPlainText"><span>${
					window.siyuan.languages.pasteAsPlainText
				}</span></button>
<div class="keyboard__split${protyle.disabled ? ' fn__none' : ''}"></div>
<button class="keyboard__action${protyle.disabled ? ' fn__none' : ''}" data-action="pasteEscaped"><span>${
					window.siyuan.languages.pasteEscaped
				}</span></button>
<div class="keyboard__split${protyle.disabled ? ' fn__none' : ''}"></div>
