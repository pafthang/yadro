	...
	constructor(protyle: IProtyle) {
		...
		this.element.addEventListener('dragstart', (event: DragEvent & { target: HTMLElement }) => {
			...
			if (buttonElement.dataset.rowId) {
				avElement = Array.from(
					protyle.wysiwyg.element.querySelectorAll(`.av[data-node-id="${buttonElement.dataset.nodeId}"]`)
			...
			selectElements.forEach((item) => {
				if (item.getAttribute('data-type') === 'NodeIFrame') {
					...
					getContenteditableElement(embedElement).innerHTML =
						'<svg class="svg"><use xlink:href="#iconLanguage"></use></svg> IFrame';
			...
			ghostElement.setAttribute(
				'style',
				`position:fixed;opacity:.1;width:${selectElements[0].clientWidth}px;padding:0;`
			);
			...
			event.dataTransfer.setData(
				`${Constants.SIYUAN_DROP_GUTTER}${buttonElement.getAttribute('data-type')}${
					Constants.ZWSP
				}${buttonElement.getAttribute('data-subtype')}${Constants.ZWSP}${selectIds}`,
		...
		this.element.addEventListener('click', (event: MouseEvent & { target: HTMLInputElement }) => {
			...
			if (!id) {
				...
				Array.from(
					protyle.wysiwyg.element.querySelectorAll(
						`[data-node-id="${(buttonElement.previousElementSibling || buttonElement.nextElementSibling).getAttribute(
							'data-node-id'
						)}"]`
					)
				).find((item) => {
					if (
						!hasClosestByAttribute(item.parentElement, 'data-type', 'NodeBlockQueryEmbed') &&
			...
			if (
				buttonElement.dataset.type === 'NodeAttributeViewRowMenu' ||
				buttonElement.dataset.type === 'NodeAttributeViewRow'
			) {
				const rowElement = Array.from(
					protyle.wysiwyg.element.querySelectorAll(
						`.av[data-node-id="${buttonElement.dataset.nodeId}"] .av__row[data-id="${buttonElement.dataset.rowId}"]`
					)
				).find((item: HTMLElement) => {
					if (!hasClosestByAttribute(item, 'data-type', 'NodeBlockQueryEmbed')) {
						return true;
				...
				if (buttonElement.dataset.type === 'NodeAttributeViewRow') {
					...
					const previousID = event.altKey
						? rowElement.previousElementSibling.getAttribute('data-id') || ''
						: buttonElement.dataset.rowId;
					transaction(
						protyle,
						[
							{
			...
			if (isOnlyMeta(event)) {
				...
				if (
					buttonElement.getAttribute('data-type') === 'NodeListItem' &&
					foldElement.parentElement.getAttribute('data-node-id')
				) {
					// 折叠同级
					let hasFold = true;
				...
				if (isMobile()) {
					...
					window.siyuan.menus.menu.element.setAttribute(
						'data-from',
						popoverElement ? popoverElement.dataset.level + 'popover' : 'app'
					);
		...
		this.element.addEventListener('mouseover', (event: MouseEvent & { target: HTMLInputElement }) => {
			...
			Array.from(
				protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${buttonElement.getAttribute('data-node-id')}"]`)
			).find((item) => {
				if (!hasClosestByAttribute(item.parentElement, 'data-type', 'NodeBlockQueryEmbed') && this.isMatchNode(item)) {
					...
					Array.from(protyle.wysiwyg.element.querySelectorAll('.protyle-wysiwyg--hl, av__row--hl')).forEach(
						(hlItem) => {
	...
	private turnsOneInto(options: {
		...
		return {
			...
			async click() {
				...
				const parentId =
					options.nodeElement.parentElement.getAttribute('data-node-id') || options.protyle.block.parentID;
				// @ts-ignore
	...
	public renderMultipleMenu(protyle: IProtyle, selectsElement: Element[]) {
		...
		window.siyuan.menus.menu.append(
			new MenuItem({
				...
				iconHTML:
					'<svg class="b3-menu__icon" style="color:var(--b3-theme-primary)"><use xlink:href="#iconRiffCard"></use></svg>',
				icon: 'iconRiffCard',
				click() {
...
<input type="checkbox" class="b3-switch fn__flex-center"${
								ligatures === 'true'
									? ' checked'
									: window.siyuan.config.editor.codeLigatures && ligatures !== 'false'
									? ' checked'
									: ''
							}></div>`,
							bind(element) {
...
<input type="checkbox" class="b3-switch fn__flex-center"${
								linenumber === 'true'
									? ' checked'
									: window.siyuan.config.editor.codeSyntaxHighlightLineNum && linenumber !== 'false'
									? ' checked'
									: ''
							}></div>`,
							bind(element) {
			...
			type === 'NodeCodeBlock' &&
			!protyle.disabled &&
			['echarts', 'mindmap'].includes(nodeElement.getAttribute('data-subtype'))
		) {
			window.siyuan.menus.menu.append(new MenuItem({ type: 'separator' }).element);
...
<input style="margin: 4px 0;width: 84px" type="number" step="1" min="148" class="b3-text-field" value="${
								height ? parseInt(height) : '420'
							}">`,
							bind: (element) => {
								element.querySelector('input').addEventListener('change', (event) => {
									...
									const chartInstance = window.echarts.getInstanceById(
										nodeElement.firstElementChild.nextElementSibling.getAttribute('_echarts_instance_')
									);
									if (chartInstance) {
...
<input type="checkbox" class="b3-switch fn__flex-center"${
								breadcrumb === 'true'
									? ' checked'
									: window.siyuan.config.editor.embedBlockBreadcrumb && breadcrumb !== 'false'
									? ' checked'
									: ''
							}></div>`,
							bind(element) {
...
<input type="checkbox" class="b3-switch fn__flex-center"${
								nodeElement.getAttribute('custom-heading-mode') === '1' ? ' checked' : ''
							}></div>`,
							bind(element) {
								element.addEventListener('click', (event: MouseEvent & { target: HTMLElement }) => {
									const inputElement = element.querySelector('input');
		...
		if (!protyle.options.backlinkData) {
			window.siyuan.menus.menu.append(
				new MenuItem({
					accelerator: `${updateHotkeyTip(window.siyuan.config.keymap.general.enter.custom)}/${updateHotkeyTip(
						'⌘Click'
					)}`,
		...
		if (type !== 'NodeThematicBreak') {
			window.siyuan.menus.menu.append(
				new MenuItem({
					...
					accelerator: `${updateHotkeyTip(
						window.siyuan.config.keymap.editor.general.collapse.custom
					)}/${updateHotkeyTip('⌥Click')}`,
					click() {
		...
		window.siyuan.menus.menu.append(
			new MenuItem({
				iconHTML: '',
				type: 'readonly',
				label: `${updateHTML}${window.siyuan.languages.createdAt} ${dayjs(id.substr(0, 14)).format(
					'YYYY-MM-DD HH:mm:ss'
				)}`
			}).element
		);
			...
			if (!embedElement) {
				...
				if (type === 'NodeAttributeView' && target) {
					...
					if (rowElement && !rowElement.classList.contains('av__row--header')) {
						element = rowElement;
						html = `<button data-type="NodeAttributeViewRowMenu" data-node-id="${dataNodeId}" data-row-id="${
							rowElement.dataset.id
						}" class="ariaLabel" data-position="right" aria-label="${
							window.siyuan.languages.rowTip
						}"><svg><use xlink:href="#iconDrag"></use></svg><span ${
							protyle.disabled ? '' : 'draggable="true" class="fn__grab"'
						}></span></button>`;
						if (!protyle.disabled) {
							html = `<button data-type="NodeAttributeViewRow" data-node-id="${dataNodeId}" data-row-id="${
								rowElement.dataset.id
							}" class="ariaLabel" data-position="right" aria-label="${
								isMac()
									? window.siyuan.languages.addBelowAbove
									: window.siyuan.languages.addBelowAbove.replace('⌥', 'Alt+')
							}"><svg><use xlink:href="#iconAdd"></use></svg></button>${html}`;
						}
						break;
					}
				}
...
data-type="${type}" data-subtype="${nodeElement.getAttribute('data-subtype')}" data-node-id="${nodeElement.getAttribute(
					'data-node-id'
				)}">
...
data-type="fold"><svg style="width:10px${
						fold && fold === '1' ? '' : ';transform:rotate(90deg)'
					}"><use xlink:href="#iconPlay"></use></svg></button>`;
				}
		...
		if (listItem) {
			...
			if (
				rect.height < Math.floor(window.siyuan.config.editor.fontSize * 1.625) + 8 ||
				(rect.height > Math.floor(window.siyuan.config.editor.fontSize * 1.625) + 8 &&
					rect.height < Math.floor(window.siyuan.config.editor.fontSize * 1.625) * 2 + 8)
			) {
		...
		if (
			(nodeElement.getAttribute('data-type') === 'NodeBlockQueryEmbed' && this.element.childElementCount === 1) || // 嵌入块为列表时
			// 为数据库行
