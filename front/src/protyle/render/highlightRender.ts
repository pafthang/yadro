	...
	addScript(`${cdn}/js/highlight.js/highlight.min.js?v=11.7.0`, 'protyleHljsScript').then(() => {
		addScript(`${cdn}/js/highlight.js/third-languages.js?v=1.0.1`, 'protyleHljsThirdScript').then(() => {
			codeElements.forEach((block: HTMLElement) => {
				...
				if (
					!isPreview &&
					(lineNumber === 'true' || (lineNumber !== 'false' && window.siyuan.config.editor.codeSyntaxHighlightLineNum))
				) {
				...
				if (
					...
					block.parentElement.getAttribute('data-node-id') ===
						layoutElement.querySelector('#searchList > .b3-list-item--focus')?.getAttribute('data-node-id')
				) {
...
export const lineNumberRender = (block: HTMLElement) => {
	...
	block.classList.add('protyle-linenumber');
	// clientHeight 总是取的整数
	block.parentElement.style.lineHeight = `${(
		(parseInt(block.parentElement.style.fontSize) || window.siyuan.config.editor.fontSize) *
		1.625 *
		0.85
	).toFixed(0)}px`;
	...
	lineNumberTemp.setAttribute(
		'style',
		`box-sizing: border-box;width: calc(100% - 3.6em);position: absolute;padding-top:0 !important;padding-bottom:0 !important;min-height:auto !important;white-space:${block.style.whiteSpace};word-break:${block.style.wordBreak};font-variant-ligatures:${block.style.fontVariantLigatures};`
	);
