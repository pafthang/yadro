...
export const genCellValueByElement = (colType: TAVCol, cellElement: HTMLElement) => {
	...
	if (colType === 'number') {
		...
		cellValue.relation = {
			blockIDs: Array.from(cellElement.querySelectorAll('span')).map((item: HTMLElement) =>
...
export const getTypeByCellElement = (cellElement: Element) => {
	...
	return scrollElement
		...
		.querySelector(`[data-col-id="${cellElement.getAttribute('data-col-id')}"]`)
		.getAttribute('data-dtype') as TAVCol;
};
...
export const popTextCell = (protyle: IProtyle, cellElements: HTMLElement[], type?: TAVCol) => {
	...
	const style = `style="padding-top: 6.5px;position:absolute;left: ${cellRect.left}px;top: ${
		cellRect.top
	}px;width:${Math.max(cellRect.width, 25)}px;height: ${height}px"`;
	if (['text', 'url', 'email', 'phone', 'block', 'template'].includes(type)) {
		...
		html = `<input type="number" value="${cellElements[0].firstElementChild.getAttribute(
			'data-content'
		)}" ${style} class="b3-text-field">`;
	} else {
	...
	document.body.insertAdjacentHTML(
		'beforeend',
		`<div class="av__mask" style="z-index: ${++window.siyuan.zIndex}">
    ${html}
</div>`
	);
	...
	if (inputElement) {
			...
			inputElement.addEventListener('input', (event: InputEvent) => {
				if (Constants.BLOCK_HINT_KEYS.includes(inputElement.value.substring(0, 2))) {
					...
					if (!blockElement.contains(cellElements[0])) {
						...
						if (cellElements[0]) {
							cellElements[0] = blockElement.querySelector(
								`.av__row[data-id="${rowElement.dataset.id}"] .av__cell[data-col-id="${cellElements[0].dataset.colId}"]`
							) as HTMLElement;
						}
					}
		...
		inputElement.addEventListener('keydown', (event) => {
			...
			if (
				...
				updateCellValueByInput(protyle, type, blockElement, cellElements);
				if (event.key === 'Tab') {
					protyle.wysiwyg.element.dispatchEvent(
	...
	if (type === 'template') {
		...
		updateCellsValue(
			...
			type === 'checkbox'
				...
				: (avMaskElement.querySelector('.b3-text-field') as HTMLInputElement).value,
			cellElements
		);
	}
	if (!hasClosestByClassName(cellElements[0], 'custom-attr')) {
...
export const updateCellsValue = (
	...
	cellElements.forEach((item: HTMLElement, elementIndex) => {
		...
		if (!nodeElement.contains(item)) {
			item = cellElements[elementIndex] = nodeElement.querySelector(
				`.av__row[data-id="${rowElement.dataset.id}"] .av__cell[data-col-id="${item.dataset.colId}"]`
			) as HTMLElement;
		}
		...
		text +=
			getCellText(item) +
			(cellElements[elementIndex + 1] &&
			item.nextElementSibling &&
			item.nextElementSibling.isSameNode(cellElements[elementIndex + 1])
				? '\t'
				: '\n\n');
		...
		cellValue.id = cellId;
		if (
			(cellValue.type === 'date' && typeof cellValue.date === 'string') ||
...
<span class="b3-chip b3-chip--info b3-chip--small" data-type="block-more">${window.siyuan.languages.more}</span>`;
			...
			text = `<span data-type="block-ref" data-id="${
				cellValue.block.id
			}" data-subtype="s" class="av__celltext av__celltext--ref">${cellValue.block.content || 'Untitled'}</span>
<span class="b3-chip b3-chip--info b3-chip--small popover__block" data-id="${
				cellValue.block.id
			}" data-type="block-more">${window.siyuan.languages.update}</span>`;
		}
	} else if (cellValue.type === 'number') {
		text = `<span class="av__celltext" data-content="${
			cellValue?.number.isNotEmpty ? cellValue?.number.content : ''
		}">${cellValue?.number.formattedContent || cellValue?.number.content || ''}</span>`;
		...
		if (dataValue && dataValue.hasEndDate && dataValue.isNotEmpty && dataValue.isNotEmpty2) {
			text += `<svg class="av__cellicon"><use xlink:href="#iconForward"></use></svg>${dayjs(dataValue.content2).format(
				dataValue.isNotTime ? 'YYYY-MM-DD' : 'YYYY-MM-DD HH:mm'
			)}`;
		}
		text += '</span>';
		...
		text += `<svg class="av__checkbox"><use xlink:href="#icon${
			cellValue?.checkbox?.checked ? 'Check' : 'Uncheck'
		}"></use></svg>`;
	} else if (cellValue.type === 'rollup') {
		cellValue?.rollup?.contents?.forEach((item) => {
			const rollupText = ['select', 'mSelect', 'mAsset', 'checkbox', 'relation'].includes(item.type)
				? renderCell(item)
				: renderRollup(item);
			if (rollupText) {
				text += rollupText + ', ';
		...
		cellValue?.relation?.contents?.forEach((item, index) => {
			text += `<span class="av__celltext--ref" style="margin-right: 8px" data-id="${
				cellValue?.relation?.blockIDs[index]
			}">${item || 'Untitled'}</span>`;
		});
	}
	if (
		['text', 'template', 'url', 'email', 'phone', 'number', 'date', 'created', 'updated'].includes(cellValue.type) &&
		cellValue &&
		cellValue[cellValue.type as 'url'].content
	) {
		text += `<span ${
			cellValue.type !== 'number' ? '' : 'style="right:auto;left:5px"'
		} data-type="copy" class="block__icon"><svg><use xlink:href="#iconCopy"></use></svg></span>`;
	}
	return text;
};
...
export const updateHeaderCell = (
	...
	if (typeof headerValue.icon !== 'undefined') {
		...
		cellElement.querySelector('.av__cellheadericon').outerHTML = headerValue.icon
			? unicode2Emoji(headerValue.icon, 'av__cellheadericon', true)
			: `<svg class="av__cellheadericon"><use xlink:href="#${getColIconByType(
					cellElement.dataset.dtype as TAVCol
					// eslint-disable-next-line no-mixed-spaces-and-tabs
			  )}"></use></svg>`;
	}
