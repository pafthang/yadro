...
export const bindAssetEvent = (options: {
	...
	options.menuElement.querySelector('input').addEventListener(
		'change',
		(
			event: InputEvent & {
			...
			uploadFiles(options.protyle, event.target.files, event.target, (res) => {
				...
				Object.keys(resData.data.succMap).forEach((key) => {
					value.push({
						...
						type: Constants.SIYUAN_ASSETS_IMAGE.includes(pathPosix().extname(resData.data.succMap[key]).toLowerCase())
							? 'image'
							: 'file'
					});
				});
...
export const updateAssetCell = (options: {
	...
	options.cellElements.forEach((item, elementIndex) => {
		if (!options.blockElement.contains(item)) {
			...
			if (rowElement) {
				item = options.cellElements[elementIndex] = options.blockElement.querySelector(
	...
	if (menuElement) {
		...
		const cellRect = (
			options.cellElements[0].classList.contains('custom-attr__avvalue')
				? options.cellElements[0]
				: options.protyle.wysiwyg.element.querySelector(`.av__cell[data-id="${options.cellElements[0].dataset.id}"]`)
...
export const editAssetItem = (
	...
	if (type === 'file') {
		menu.addItem({
			iconHTML: '',
			type: 'readonly',
			label: `${window.siyuan.languages.title}<textarea rows="1" style="margin:4px 0;width: ${
				isMobile() ? '200' : '360'
			}px" class="b3-text-field"></textarea>`
		});
	} else {
		menu.addItem({
			icon: 'iconPreview',
