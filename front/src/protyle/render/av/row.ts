 ...
 */
export const insertAttrViewBlockAnimation = (
	protyle: IProtyle,
	...
	const previousElement =
		blockElement.querySelector(`.av__row[data-id="${previousId}"]`) || blockElement.querySelector('.av__row--header');
	let colHTML = '<div class="av__firstcol av__colsticky"><svg><use xlink:href="#iconUncheck"></use></svg></div>';
	const pinIndex = previousElement.querySelectorAll('.av__colsticky .av__cell').length - 1;
	if (pinIndex > -1) {
		colHTML =
			'<div class="av__colsticky"><div class="av__firstcol av__colsticky"><svg><use xlink:href="#iconUncheck"></use></svg></div>';
	}
...
${item.getAttribute('data-block-id') || item.dataset.dtype === 'block' ? ' data-detached="true"' : ''}><span class="${
			avId ? 'av__celltext' : 'av__pulse'
		}"></span></div>`;
		if (pinIndex === index) {
			colHTML += '</div>';
		}
	...
	if (avId) {
		...
		const sideRow = previousElement.classList.contains('av__row--header')
