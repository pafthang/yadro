...
const genSearchList = (element: Element, keyword: string, avId: string, cb?: () => void) => {
	fetchPost('/api/av/searchAttributeView', { keyword }, (response) => {
		...
		response.data.results.forEach(
			(
				...
				}" data-av-id="${item.avID}" data-block-id="${item.blockID}">
    ...
    <svg aria-label="${
			window.siyuan.languages.thisDatabase
		}" style="margin: 0 0 0 4px" class="b3-list-item__hinticon ariaLabel${
					item.avID === avId ? '' : ' fn__none'
				}"><use xlink:href="#iconInfo"></use></svg>
</div>`;
			}
		);
...
const setDatabase = (avId: string, element: HTMLElement, item: HTMLElement) => {
	...
	element.querySelector('.b3-menu__accelerator').textContent = item
		.querySelector('.b3-list-item__hinticon')
...
export const toggleUpdateRelationBtn = (menuItemsElement: HTMLElement, avId: string, resetData = false) => {
	...
	if (oldValue.avID) {
		...
		if (
			(searchElement.dataset.avId && oldValue.avID !== searchElement.dataset.avId) ||
			oldValue.isTwoWay !== switchElement.checked ||
...
const genSelectItemHTML = (
    ...
    <span class="b3-menu__label${isDetached ? '' : ' popover__block'}" ${
				...
				if (hasId) {
					cells.find((item) => {
						if (item.block.id === hasId) {
							selectHTML += genSelectItemHTML(
								'selected',
								item.block.id,
								item.isDetached,
								item.block.content || 'Untitled'
							);
							return true;
						}
					});
	...
	if (target.classList.contains('b3-menu__item')) {
		...
		if (target.getAttribute('draggable')) {
			...
			target.outerHTML = genSelectItemHTML(
				'unselect',
				targetId,
				!target.querySelector('.popover__block'),
				target.querySelector('.b3-menu__label').textContent
			);
