...
const updateCol = (
	...
	options.target.querySelector('.b3-menu__accelerator').textContent =
		itemElement.querySelector('.b3-list-item__text').textContent;
...
const genSearchList = (element: Element, keyword: string, avId: string, isRelation: boolean, cb?: () => void) => {
	...
	fetchPost(
		isRelation ? '/api/av/searchAttributeViewRelationKey' : '/api/av/searchAttributeViewNonRelationKey',
		{
			avID: avId,
			keyword
		},
		(response) => {
			...
			response.data.keys.forEach((item: IAVColumn, index: number) => {
				html += `<div class="b3-list-item b3-list-item--narrow${
					index === 0 ? ' b3-list-item--focus' : ''
				}" data-col-id="${item.id}" ${
					isRelation ? `data-target-av-id="${item.relation.avID}"` : `data-col-type="${item.type}"`
				}>
        ${
					item.icon
						? unicode2Emoji(item.icon, 'b3-list-item__graphic', true)
						: `<svg class="b3-list-item__graphic"><use xlink:href="#${getColIconByType(item.type)}"></use></svg>`
				}
...
export const goSearchRollupCol = (options: {
    ...
    <input class="b3-text-field fn__flex-shrink" placeholder="${
			window.siyuan.languages[options.isRelation ? 'searchRelation' : 'searchRollupProperty']
		}"/>
		...
		bind(element) {
			...
			inputElement.addEventListener('input', (event) => {
				...
				genSearchList(
					listElement,
					inputElement.value,
					options.isRelation ? options.data.id : options.target.dataset.avId,
					options.isRelation
				);
			});
...
export const getRollupHTML = (options: { data?: IAV; cellElements?: HTMLElement[]; colData?: IAVColumn }) => {
	...
	return `<button class="b3-menu__item" data-type="goSearchRollupCol" data-old-value='${JSON.stringify(
		colData.rollup || {}
	)}'>
...
export const bindRollupData = (options: { protyle: IProtyle; data: IAV; menuElement: HTMLElement }) => {
	...
	if (goSearchRollupColElement) {
		...
		const goSearchRollupTargetElement = options.menuElement.querySelector(
			'[data-type="goSearchRollupTarget"]'
		) as HTMLElement;
		let targetKeyAVId = '';
			...
			fetchPost('/api/av/getAttributeView', { id: targetKeyAVId }, (response) => {
				response.data.av.keyValues.find((item: { key: { id: string; name: string; type: TAVCol } }) => {
					if (item.key.id === oldValue.keyID) {
						...
						const goSearchRollupCalcElement = options.menuElement.querySelector(
							'[data-type="goSearchRollupCalc"]'
						) as HTMLElement;
