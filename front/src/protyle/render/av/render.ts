...
export const avRender = (element: Element, protyle: IProtyle, cb?: () => void, viewID?: string) => {
    ...
    <div class="av__cell" style="width: 200px"><span class="av__pulse"></span></div>
			...
			if (selectCellElement) {
				selectCellId =
					(hasClosestByClassName(selectCellElement, 'av__row') as HTMLElement).dataset.id +
					Constants.ZWSP +
					selectCellElement.getAttribute('data-col-id');
			}
			...
			fetchPost(
				...
				(response) => {
					...
					// header
					let tableHTML =
						'<div class="av__row av__row--header"><div class="av__firstcol av__colsticky"><svg><use xlink:href="#iconUncheck"></use></svg></div>';
					let calcHTML = '';
					let pinIndex = -1;
					let pinMaxIndex = -1;
					let indexWidth = 0;
					...
					if (pinIndex > -1) {
						tableHTML =
							'<div class="av__row av__row--header"><div class="av__colsticky"><div class="av__firstcol"><svg><use xlink:href="#iconUncheck"></use></svg></div>';
...
style="width: ${column.width || '200px'};">
    ${
			column.icon
				? unicode2Emoji(column.icon, 'av__cellheadericon', true)
				: `<svg class="av__cellheadericon"><use xlink:href="#${getColIconByType(column.type)}"></use></svg>`
		}
    <span class="av__celltext fn__flex-1">${column.name}</span>
    ${
			column.pin
				? '<svg class="av__cellheadericon av__cellheadericon--pin"><use xlink:href="#iconPin"></use></svg>'
				: ''
		}
    <div class="av__widthdrag"></div>
						...
						calcHTML += `<div class="av__calc${
							column.calc && column.calc.operator !== '' ? ' av__calc--ashow' : ''
						}" data-col-id="${column.id}" data-dtype="${column.type}" data-operator="${column.calc?.operator || ''}"  
style="width: ${index === 0 ? parseInt(column.width || '200') + 24 + 'px' : column.width || '200px'}">${
							getCalcValue(column) || '<svg><use xlink:href="#iconDown"></use></svg>' + window.siyuan.languages.calc
						}</div>`;
						if (pinIndex === index) {
							calcHTML += '</div>';
						}
    ...
    <div class="block__icon block__icon--show"  data-type="av-header-more"><svg><use xlink:href="#iconMore"></use></svg></div>
					...
					data.rows.forEach((row: IAVRow) => {
						...
						if (pinIndex > -1) {
							tableHTML +=
								'<div class="av__colsticky"><div class="av__firstcol"><svg><use xlink:href="#iconUncheck"></use></svg></div>';
						} else {
							tableHTML +=
								'<div class="av__firstcol av__colsticky"><svg><use xlink:href="#iconUncheck"></use></svg></div>';
						}
						...
						row.cells.forEach((cell, index) => {
							...
							tableHTML += `<div class="av__cell${checkClass}" data-id="${cell.id}" data-col-id="${
								data.columns[index].id
							}"
${cell.valueType === 'block' ? 'data-block-id="' + (cell.value.block.id || '') + '"' : ''} data-wrap="${
								data.columns[index].wrap
							}" 
...
${cell.color ? `color:${cell.color};` : ''}">${renderCell(cell.value)}</div>`;
    ...
    ${
			item.icon
				? unicode2Emoji(item.icon, 'item__graphic', true)
    ...
    <span class="item__text">${item.name}</span>
					...
					e.firstElementChild.outerHTML = `<div class="av__container" style="--av-background:${
						e.style.backgroundColor || 'var(--b3-theme-background)'
					}">
        ...
        <div class="fn__flex av__views">
            ...
            ${
							response.data.isMirror
								? ` <span class="block__icon block__icon--show ariaLabel" aria-label="${window.siyuan.languages.mirrorTip}">
    <svg><use xlink:href="#iconSplitLR"></use></svg></span><div class="fn__space"></div>`
								: ''
						}
        </div>
        <div contenteditable="${
					protyle.disabled ? 'false' : 'true'
				}" spellcheck="${window.siyuan.config.editor.spellcheck.toString()}" class="av__title${
						viewData.hideAttrViewName ? ' av__title--hide' : ''
					}" data-title="${response.data.name || ''}" data-tip="${window.siyuan.languages.title}">${
						response.data.name || ''
					}</div>
    ...
    <div class="av__scroll">
        <div class="av__body">
            ...
            <div class="av__row--util">
                <div class="av__colsticky">
                    ...
                    <button class="b3-button${data.rowCount > data.rows.length ? '' : ' fn__none'}">
                        ...
                        <svg data-type="set-page-size" data-size="${
													data.pageSize
												}"><use xlink:href="#iconMore"></use></svg>
                    </button>
                </div>
            </div>
            <div class="av__row--footer">${calcHTML}</div>
					...
					if (selectCellId) {
						const newCellElement = e.querySelector(
							`.av__row[data-id="${selectCellId.split(Constants.ZWSP)[0]}"] .av__cell[data-col-id="${
								selectCellId.split(Constants.ZWSP)[1]
							}"]`
						);
						if (newCellElement) {
					...
					e.querySelector('.layout-tab-bar').scrollLeft = (
						e.querySelector('.layout-tab-bar .item--focus') as HTMLElement
					).offsetLeft;
					if (cb) {
						cb();
					}
...
export const refreshAV = (protyle: IProtyle, operation: IOperation) => {
	if (operation.action === 'setAttrViewName') {
		Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-av-id="${operation.id}"]`)).forEach(
			(item: HTMLElement) => {
	...
	refreshTimeouts[protyle.id] = window.setTimeout(
		() => {
			if (operation.action === 'setAttrViewColWidth') {
				Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-av-id="${operation.avID}"]`)).forEach(
					(item: HTMLElement) => {
				...
				Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-av-id="${operation.avID}"]`)).forEach(
					(item: HTMLElement) => {
						...
						avRender(
							...
							protyle,
							() => {
								const attrElement = document.querySelector(
									`.b3-dialog--open[data-key="${Constants.DIALOG_ATTR}"] div[data-av-id="${operation.avID}"]`
								) as HTMLElement;
								if (attrElement) {
									// 更新属性面板
							...
							['addAttrViewView', 'duplicateAttrViewView'].includes(operation.action)
								? operation.id
								: operation.action === 'removeAttrViewView'
								? null
								: undefined
						);
					}
