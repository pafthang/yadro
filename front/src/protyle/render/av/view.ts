...
export const bindViewEvent = (options: { protyle: IProtyle; data: IAV; menuElement: HTMLElement }) => {
...
<button class="b3-menu__item" data-type="nobg">
    <span style="padding: 5px;margin-right: 8px;width: 14px;font-size: 14px;" class="block__icon block__icon--show" data-icon="${
			data.icon
		}" data-type="update-view-icon">${
		data.icon ? unicode2Emoji(data.icon) : '<svg><use xlink:href="#iconTable"></use></svg>'
	}</span>
    <span class="b3-menu__label" style="padding: 4px;display: flex;"><input data-type="name" class="b3-text-field fn__block" type="text" value="${
			data.name
		}" data-value="${data.name}"></span>
</button>
<button class="b3-menu__separator"></button>
<button class="b3-menu__item" data-type="go-properties">
    <svg class="b3-menu__icon"></svg>
    <span class="b3-menu__label">${window.siyuan.languages.attr}</span>
    <span class="b3-menu__accelerator">${data.columns.filter((item: IAVColumn) => !item.hidden).length}/${
		data.columns.length
	}</span>
...
<button class="b3-menu__item">
    ...
    <svg class="b3-menu__action" data-type="toggle-view-title"><use xlink:href="#iconEye${
			data.hideAttrViewName ? '' : 'off'
		}"></use></svg>
</button>
<button class="b3-menu__separator"></button>
...
export const getSwitcherHTML = (views: IAVView[], viewId: string) => {
	let html = '';
	views.forEach((item) => {
		html += `<button draggable="true" class="b3-menu__item${
			item.id === viewId ? ' b3-menu__item--current' : ''
		}" data-id="${item.id}">
    ...
    <div class="b3-menu__label fn__flex" data-type="av-view-switch">
        ${
					item.icon
						...
						: '<svg class="b3-menu__icon"><use xlink:href="#iconTable"></use></svg>'
				}
        ${item.name}
    </div>
