...
export const avClick = (protyle: IProtyle, event: MouseEvent & { target: HTMLElement }) => {
	...
	while (target && !target.isEqualNode(blockElement)) {
		...
		if (type === 'av-header-add') {
			...
			blockElement.dataset.pageSize = (
				parseInt(blockElement.dataset.pageSize) +
				parseInt(blockElement.querySelector('[data-type="set-page-size"]').getAttribute('data-size'))
			).toString();
...
export const avContextmenu = (protyle: IProtyle, rowElement: HTMLElement, position: IPosition) => {
	...
	if (!protyle.disabled) {
		...
		rowElement.parentElement.querySelectorAll('.av__row--header .av__cell').forEach((cellElement: HTMLElement) => {
			let hideBlock = false;
			const selectElements: HTMLElement[] = Array.from(
				blockElement.querySelectorAll(
					`.av__row--select:not(.av__row--header) .av__cell[data-col-id="${cellElement.dataset.colId}"]`
				)
			);
			...
			if (!hideBlock && !['updated', 'created'].includes(type)) {
				...
				editAttrSubmenu.push({
					iconHTML: icon
						? unicode2Emoji(icon, 'b3-menu__icon', true)
