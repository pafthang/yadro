	...
	switch (value.type) {
		...
		case 'date':
			...
			if (
				...
				html += `<svg class="av__cellicon"><use xlink:href="#iconForward"></use></svg>${dayjs(
					value[value.type].content2
				).format(value[value.type].isNotTime ? 'YYYY-MM-DD' : 'YYYY-MM-DD HH:mm')}`;
			}
			if (html) {
...
export const genAVValueHTML = (value: IAVCellValue) => {
	...
	switch (value.type) {
		...
		case 'text':
			...
			}" class="b3-text-field b3-text-field--text fn__flex-1">${value.text.content}</textarea>`;
			break;
		case 'number':
			html = `<input value="${
				value.number.content || ''
			}" type="number" class="b3-text-field b3-text-field--text fn__flex-1">`;
			break;
		case 'mSelect':
		case 'select':
		...
		case 'updated':
			if (value[value.type].isNotEmpty) {
				html = `<span data-content="${value[value.type].content}">${dayjs(value[value.type].content).format(
					'YYYY-MM-DD HH:mm'
				)}</span>`;
			}
			break;
...
<a href="tel:${value.phone.content}" target="_blank" aria-label="${window.siyuan.languages.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconPhone"></use></svg></a>`;
			break;
		case 'checkbox':
			html = `<svg class="av__checkbox" style="height: 17px;"><use xlink:href="#icon${
				value.checkbox.checked ? 'Check' : 'Uncheck'
			}"></use></svg>`;
			break;
		case 'template':
...
<a href="mailto:${value.email.content}" target="_blank" aria-label="${window.siyuan.languages.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconEmail"></use></svg></a>`;
		...
		case 'relation':
			value.relation?.blockIDs?.forEach((item, index) => {
				html += `<span class="av__celltext--url" style="margin-right: 8px" data-id="${item}">${
					value.relation?.contents[index] || 'Untitled'
				}</span>`;
			});
			break;
		case 'rollup':
...
<div class="custom-attr__avheader block__logo popover__block" data-id='${JSON.stringify(table.blockIDs)}'>
    ...
    <svg class="block__logoicon"><use xlink:href="#iconDatabase"></use></svg><span>${
			table.avName || window.siyuan.languages.database
		}</span>
    <div class="fn__flex-1"></div>
</div>`;
    ...
    <div class="block__logo ariaLabel${
			item.values[0].type === 'block' ? '' : ' fn__pointer'
		}" data-type="editCol" data-position="parentW" aria-label="${escapeAttr(item.key.name)}">
        ${
					item.key.icon
						? unicode2Emoji(item.key.icon, 'block__logoicon', true)
						: `<svg class="block__logoicon"><use xlink:href="#${getColIconByType(item.key.type)}"></use></svg>`
				}
        <span>${item.key.name}</span>
    </div>
    <div data-av-id="${table.avID}" data-col-id="${item.values[0].keyID}" data-block-id="${
						item.values[0].blockID
					}" data-id="${item.values[0].id}" data-type="${item.values[0].type}" 
data-options="${item.key?.options ? escapeAttr(JSON.stringify(item.key.options)) : '[]'}"
class="fn__flex-1 fn__flex${
						['url', 'text', 'number', 'email', 'phone', 'block'].includes(item.values[0].type)
							? ''
					...
					transaction(protyle, [
						{
							...
							previousID: isBottom
								? targetElement.dataset.colId
								: targetElement.previousElementSibling?.getAttribute('data-col-id'),
