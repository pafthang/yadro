...
export const openMenuPanel = (options: {
    ...
    <div class="b3-menu">${html}</div>
			...
			const tabRect = options.blockElement
				.querySelector(`.av__views, .av__row[data-col-id="${options.colId}"] > .block__logo`)
			...
			avPanelElement.addEventListener('drop', (event) => {
						...
						[
							{
								...
								id: sourceId,
								previousID: isTop
									? targetElement.previousElementSibling?.getAttribute('data-id')
				...
				if (targetElement.querySelector('[data-type="setColOption"]')) {
					const colId = options.cellElements
						? options.cellElements[0].dataset.colId
				...
				if (targetElement.getAttribute('data-type') === 'setRelationCell') {
					...
					setRelationCell(
						options.protyle,
						options.blockElement as HTMLElement,
					...
					[
						{
							...
							avID,
							previousID:
								(targetElement.classList.contains('dragover__top')
									? targetElement.previousElementSibling?.getAttribute('data-id')
									: targetElement.getAttribute('data-id')) || '',
							id: sourceId
						}
			...
			avPanelElement.addEventListener('dragover', (event: DragEvent) => {
				...
				if (!targetElement) {
					targetElement = hasClosestByAttribute(
						...
						openEmojiPanel(
							...
							(unicode) => {
								...
								target.innerHTML = unicode
									...
									: `<svg><use xlink:href="#${getColIconByType(target.dataset.colType as TAVCol)}"></use></svg>`;
								if (isCustomAttr) {
									const iconElement = options.blockElement.querySelector(
										`.av__row[data-col-id="${colId}"] .block__logoicon`
									);
									iconElement.outerHTML = unicode
										? unicode2Emoji(unicode, 'block__logoicon', true)
										: `<svg class="block__logoicon"><use xlink:href="#${getColIconByType(
												iconElement.nextElementSibling.getAttribute('data-type') as TAVCol
												// eslint-disable-next-line no-mixed-spaces-and-tabs
										  )}"></use></svg>`;
								} else {
									updateAttrViewCellAnimation(
										options.blockElement.querySelector(`.av__row--header .av__cell[data-col-id="${colId}"]`),
										undefined,
										{ icon: unicode }
									);
								}
...
const getPropertiesHTML = (data: IAVTable) => {
    ...
    <svg class="b3-menu__action${
			item.type === 'block' ? ' fn__none' : ''
		}" data-type="hideCol"><use xlink:href="#iconEyeoff"></use></svg>
    <svg class="b3-menu__action${
			item.type === 'block' ? ' fn__none' : ''
		}" data-type="editCol"><use xlink:href="#iconEdit"></use></svg>
</button>`;
		}
	});
	if (hideHTML) {
