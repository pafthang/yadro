...
export const getDefaultOperatorByType = (type: TAVCol) => {
	...
	if (
		['rollup', 'relation', 'rollup', 'text', 'mSelect', 'url', 'block', 'email', 'phone', 'template'].includes(type)
	) {
		return 'Contains';
	}
};
...
export const setFilter = async (options: {
	...
	if (rectTarget.height === 0) {
		rectTarget = options.protyle.wysiwyg.element
			.querySelector(`[data-col-id="${options.target.dataset.colId}"]`)
	...
	const menu = new Menu('set-filter-' + options.filter.column, () => {
		...
		if (textElements.length > 0) {
			if (['date', 'updated', 'created'].includes(filterType)) {
				...
				const directElements = menu.element.querySelectorAll(
					'.b3-select[data-type="dataDirection"]'
				) as NodeListOf<HTMLSelectElement>;
				if (typeElement.value === 'custom') {
					newFilter.relativeDate = {
						count: parseInt(
							(directElements[0].parentElement.querySelector('.b3-text-field') as HTMLInputElement).value || '1'
						),
					...
					newFilter.relativeDate2 = {
						count: parseInt(
							(directElements[1].parentElement.querySelector('.b3-text-field') as HTMLInputElement).value || '1'
						),
	...
	switch (filterType) {
		case 'checkbox':
			selectHTML = `<option ${
				'Is true' === options.filter.operator && !options.filter.type ? 'selected' : ''
			} value="Is true">${window.siyuan.languages.checked}</option>
<option ${'Is false' === options.filter.operator && !options.filter.type ? 'selected' : ''} value="Is false">${
				window.siyuan.languages.unchecked
			}</option>`;
		...
		case 'phone':
		case 'email':
			selectHTML = `<option ${'=' === options.filter.operator ? 'selected' : ''} value="=">${
				window.siyuan.languages.filterOperatorIs
			}</option>
<option ${'!=' === options.filter.operator ? 'selected' : ''} value="!=">${
				window.siyuan.languages.filterOperatorIsNot
			}</option>
<option ${'Contains' === options.filter.operator ? 'selected' : ''} value="Contains">${
				window.siyuan.languages.filterOperatorContains
			}</option>
<option ${'Does not contains' === options.filter.operator ? 'selected' : ''} value="Does not contains">${
				window.siyuan.languages.filterOperatorDoesNotContain
			}</option>
<option ${'Starts with' === options.filter.operator ? 'selected' : ''} value="Starts with">${
				window.siyuan.languages.filterOperatorStartsWith
			}</option>
<option ${'Ends with' === options.filter.operator ? 'selected' : ''} value="Ends with">${
				window.siyuan.languages.filterOperatorEndsWith
			}</option>
<option ${'Is empty' === options.filter.operator ? 'selected' : ''} value="Is empty">${
				window.siyuan.languages.filterOperatorIsEmpty
			}</option>
<option ${'Is not empty' === options.filter.operator ? 'selected' : ''} value="Is not empty">${
				window.siyuan.languages.filterOperatorIsNotEmpty
			}</option>`;
			break;
		case 'template':
			selectHTML = `<option ${'=' === options.filter.operator ? 'selected' : ''} value="=">${
...
<option ${'>' === options.filter.operator ? 'selected' : ''} value=">">${
				window.siyuan.languages.filterOperatorIsAfter
			}</option>
<option ${'<' === options.filter.operator ? 'selected' : ''} value="<">${
				window.siyuan.languages.filterOperatorIsBefore
			}</option>
<option ${'>=' === options.filter.operator ? 'selected' : ''} value=">=">${
				window.siyuan.languages.filterOperatorIsOnOrAfter
			}</option>
<option ${'<=' === options.filter.operator ? 'selected' : ''} value="<=">${
				window.siyuan.languages.filterOperatorIsOnOrBefore
			}</option>
<option ${'Is between' === options.filter.operator ? 'selected' : ''} value="Is between">${
				window.siyuan.languages.filterOperatorIsBetween
			}</option>
<option ${'Is empty' === options.filter.operator ? 'selected' : ''} value="Is empty">${
				window.siyuan.languages.filterOperatorIsEmpty
			}</option>
...
<option ${'Is not empty' === options.filter.operator ? 'selected' : ''} value="Is not empty">${
		...
		case 'mSelect':
		case 'relation':
			selectHTML = `<option ${'Contains' === options.filter.operator ? 'selected' : ''} value="Contains">${
				window.siyuan.languages.filterOperatorContains
			}</option>
<option ${'Does not contains' === options.filter.operator ? 'selected' : ''} value="Does not contains">${
				window.siyuan.languages.filterOperatorDoesNotContain
...
<option ${'Is not empty' === options.filter.operator ? 'selected' : ''} value="Is not empty">${
    ...
    <span class="fn__ellipsis">${option.name}</span>
		...
		menu.addItem({
			iconHTML: '',
			type: 'readonly',
			label: `<input style="margin: 4px 0" value="${
				options.filter.value?.number.isNotEmpty ? options.filter.value.number.content : ''
    ...
    <div class="fn__size200">
        <select class="b3-select fn__block" data-type="dateType">
            <option value="time"${!options.filter.relativeDate ? ' selected' : ''}>${
				window.siyuan.languages.includeTime
			}</option>
            <option value="custom"${options.filter.relativeDate ? ' selected' : ''}>${
				window.siyuan.languages.relativeToToday
			}</option>
        </select>
    </div>
    ...
    <div class="fn__size200 ${options.filter.relativeDate ? 'fn__none' : ''}">
        <input value="${
					dateValue && (dateValue.isNotEmpty || filterType !== 'date')
						? dayjs(dateValue.content).format('YYYY-MM-DD')
						: ''
				}" type="date" max="9999-12-31" class="b3-text-field fn__block">
    </div>
    <div class="fn__flex fn__size200 ${options.filter.relativeDate ? '' : 'fn__none'}">
        <select class="b3-select" data-type="dataDirection">
            <option value="-1"${options.filter.relativeDate?.direction === -1 ? ' selected' : ''}>${
				window.siyuan.languages.pastDate
			}</option>
            <option value="1"${options.filter.relativeDate?.direction === 1 ? ' selected' : ''}>${
				window.siyuan.languages.nextDate
			}</option>
            <option value="0"${showToday ? ' selected' : ''}>${window.siyuan.languages.current}</option>
        </select>
        <span class="fn__space"></span>
        <input type="number" min="1" step="1" value="${
					options.filter.relativeDate?.count || 1
				}" class="b3-text-field fn__flex-1${showToday ? ' fn__none' : ''}"/>
        <span class="fn__space${showToday ? ' fn__none' : ''}"></span>
        <select class="b3-select fn__flex-1">
            <option value="0"${options.filter.relativeDate?.unit === 0 ? ' selected' : ''}>${
				window.siyuan.languages.day
			}</option>
            <option value="1"${
							!options.filter.relativeDate || options.filter.relativeDate?.unit === 1 ? ' selected' : ''
						}>${window.siyuan.languages.week}</option>
            <option value="2"${options.filter.relativeDate?.unit === 2 ? ' selected' : ''}>${
				window.siyuan.languages.month
			}</option>
            <option value="3"${options.filter.relativeDate?.unit === 3 ? ' selected' : ''}>${
				window.siyuan.languages.year
			}</option>
        </select>
    </div>
...
<div data-type="filter2 fn__none">
    ...
    <div class="fn__size200 ${options.filter.relativeDate2 ? 'fn__none' : ''}">
        <input value="${
					dateValue && dateValue.isNotEmpty2 ? dayjs(dateValue.content2).format('YYYY-MM-DD') : ''
				}" type="date" max="9999-12-31" class="b3-text-field fn__block">
    </div>
    <div class="fn__flex fn__size200 ${options.filter.relativeDate2 ? '' : 'fn__none'}">
        <select class="b3-select" data-type="dataDirection">
            <option value="-1"${options.filter.relativeDate2?.direction === -1 ? ' selected' : ''}>${
				window.siyuan.languages.pastDate
			}</option>
            <option value="1"${options.filter.relativeDate2?.direction === 1 ? ' selected' : ''}>${
				window.siyuan.languages.nextDate
        ...
        <span class="fn__space"></span>
        <input type="number" min="1" step="1" value="${
					options.filter.relativeDate2?.count || 1
				}" class="b3-text-field fn__flex-1${showToday2 ? ' fn__none' : ''}"/>
        <span class="fn__space${showToday2 ? ' fn__none' : ''}"></span>
        <select class="b3-select fn__flex-1">
            <option value="0"${options.filter.relativeDate2?.unit === 0 ? ' selected' : ''}>${
				window.siyuan.languages.day
			}</option>
            <option value="1"${
							!options.filter.relativeDate2 || options.filter.relativeDate2?.unit === 1 ? ' selected' : ''
						}>${window.siyuan.languages.week}</option>
            <option value="2"${options.filter.relativeDate2?.unit === 2 ? ' selected' : ''}>${
				window.siyuan.languages.month
			}</option>
            <option value="3"${options.filter.relativeDate2?.unit === 3 ? ' selected' : ''}>${
				window.siyuan.languages.year
			}</option>
        </select>
    </div>
...
export const addFilter = (options: {
	...
	options.data.view.columns.forEach((column) => {
		...
		if (!filter && column.type !== 'mAsset') {
			menu.addItem({
				label: column.name,
				iconHTML: column.icon
					? unicode2Emoji(column.icon, 'b3-menu__icon', true)
					: `<svg class="b3-menu__icon"><use xlink:href="#${getColIconByType(column.type)}"></use></svg>`,
				click: () => {
					...
					setPosition(
						options.menuElement,
						options.tabRect.right - options.menuElement.clientWidth,
						options.tabRect.bottom,
...
export const getFiltersHTML = (data: IAVTable) => {
			...
			if (item.id === filter.column) {
				...
				if (filter.operator === 'Is empty') {
					...
					if (filter.relativeDate) {
						dateValue = `${
							window.siyuan.languages[['pastDate', 'current', 'nextDate'][filter.relativeDate.direction + 1]]
						}
 ...
 ${window.siyuan.languages[['day', 'week', 'month', 'year'][filter.relativeDate.unit]]}`;
						if (filter.relativeDate2) {
							dateValue2 = `${
								window.siyuan.languages[['pastDate', 'current', 'nextDate'][filter.relativeDate2.direction + 1]]
							}
 ...
 ${window.siyuan.languages[['day', 'week', 'month', 'year'][filter.relativeDate2.unit]]}`;
					...
					filter.value?.text?.content ||
					filter.value?.block?.content ||
					filter.value?.url?.content ||
					filter.value?.phone?.content ||
					filter.value?.email?.content ||
					filter.value?.relation?.contents.length > 0
				) {
					const content =
						filter.value?.text?.content ||
						filter.value?.block?.content ||
    ...
    ${
			item.icon
				? unicode2Emoji(item.icon, 'icon', true)
				: `<svg class="icon"><use xlink:href="#${getColIconByType(item.type)}"></use></svg>`
		}
