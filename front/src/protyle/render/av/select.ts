...
<span class="b3-menu__accelerator">Enter</span>
		...
		if (key) {
			html = html.replace(
				`class="b3-menu__item" data-name="${key}"`,
				`class="b3-menu__item b3-menu__item--current" data-name="${key}"`
			);
		} else {
...
export const removeCellOption = (
	...
	cellElements.forEach((item, elementIndex) => {
		if (!blockElement.contains(item)) {
			item = cellElements[elementIndex] = blockElement.querySelector(
				`.av__cell[data-id="${item.dataset.id}"]`
			) as HTMLElement;
		}
...
export const setColOption = (
	...
	const colId = cellElements
		? cellElements[0].dataset.colId
	...
	Array.from(Array(13).keys()).forEach((index) => {
		menu.addItem({
			accelerator:
				parseInt(color) === index + 1
					? '<svg class="svg" style="height: 30px; float: left;"><use xlink:href="#iconSelect"></use></svg>'
					: undefined,
			iconHTML: '',
			label: `<span class="color__square"  style="padding: 5px;margin: 2px;color: var(--b3-font-color${
				index + 1
			});background-color: var(--b3-font-background${index + 1});">A</span>`,
			click(element) {
				...
				element.insertAdjacentHTML(
					'beforeend',
					'<span class="b3-menu__accelerator"><svg class="svg" style="height: 30px; float: left;"><use xlink:href="#iconSelect"></use></svg></span>'
				);
				transaction(
					protyle,
					[
...
export const getSelectHTML = (data: IAVTable, cellElements: HTMLElement[]) => {
	...
	genCellValueByElement(colData.type, cellElements[0]).mSelect?.forEach((item) => {
		selectedHTML += `<div class="b3-chip b3-chip--middle" data-content="${escapeAttr(
			item.content
		)}" style="background-color:var(--b3-font-background${item.color});color:var(--b3-font-color${item.color})">${
			item.content
		}<svg class="b3-chip__close" data-type="removeCellOption"><use xlink:href="#iconCloseRound"></use></svg></div>`;
	});
