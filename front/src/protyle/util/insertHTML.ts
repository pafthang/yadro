	...
	if (html.endsWith(']') && html.startsWith('[')) {
		try {
			...
			const cellElements: Element[] =
				Array.from(blockElement.querySelectorAll('.av__cell--active, .av__cell--select')) || [];
...
export const insertHTML = (
	...
	if (range.toString() !== '') {
		...
		if (inlineMathElement) {
			...
			range.startContainer.parentElement.getAttribute('data-type')?.indexOf('block-ref') > -1
		) {
	...
	tempElement.innerHTML =
		tableInlineHTML || // 在 table 中插入需要使用转换好的行内元素 https://github.com/siyuan-note/siyuan/issues/9358
	...
	if (!isBlock) {
		if (
			...
			(tempElement.content.firstChild.nodeType !== 3 &&
				((tempElement.content.firstElementChild.classList.contains('p') &&
	...
	if (tempElement.content.children[0]?.getAttribute('data-type') === 'NodeListItem') {
		if (cursorLiElement) {
			...
			tempElement.innerHTML = `<div${
				subType === 'o' ? ' data-marker="1."' : ''
			} data-subtype="${subType}" data-node-id="${Lute.NewNodeID()}" data-type="NodeList" class="list">${html}<div class="protyle-attr" contenteditable="false">${
				Constants.ZWSP
			}</div></div>`;
		}
	}
	let lastElement: Element;
	Array.from(tempElement.content.children)
		.reverse()
		.forEach((item) => {
