...
export const onGet = (options: {
	...
	// 防止动态加载加载过多的内容
	if (
		options.action.includes(Constants.CB_GET_APPEND) ||
		options.action.includes(Constants.CB_GET_BEFORE) ||
		options.action.includes(Constants.CB_GET_HTML)
	) {
		setHTML(
			{
...
const setHTML = (
	...
	const updateReadonly =
		typeof options.updateReadonly === 'undefined' ? protyle.wysiwyg.element.innerHTML === '' : options.updateReadonly;
	if (options.action.includes(Constants.CB_GET_APPEND)) {
		...
		if (
			...
			protyle.contentElement.scrollHeight > REMOVED_OVER_HEIGHT
		) {
			let removeElement = protyle.wysiwyg.element.firstElementChild as HTMLElement;
			const removeElements = [];
			while (
				protyle.wysiwyg.element.childElementCount > 2 &&
				removeElements &&
				!protyle.wysiwyg.element.lastElementChild.isSameNode(removeElement)
			) {
			...
			protyle.contentElement.scrollTop =
				protyle.contentElement.scrollTop + (removeElement.getBoundingClientRect().top - lastRemoveTop);
		...
		protyle.contentElement.scrollTop =
			protyle.contentElement.scrollTop + (firstElement.getBoundingClientRect().top - lastTop);
		protyle.scroll.lastScrollTop = protyle.contentElement.scrollTop;
		// 动态加载移除
		if (!protyle.wysiwyg.element.querySelector('.protyle-wysiwyg--select') && !protyle.scroll.keepLazyLoad) {
			...
			while (
				childCount > 2 &&
				scrollHeight > REMOVED_OVER_HEIGHT &&
				lastElement.getBoundingClientRect().top > window.innerHeight
			) {
...
export const disabledProtyle = (protyle: IProtyle) => {
	...
	if (protyle.breadcrumb) {
		protyle.breadcrumb.element.parentElement
			.querySelector('[data-type="readonly"] use')
			.setAttribute('xlink:href', '#iconLock');
		protyle.breadcrumb.element.parentElement
			.querySelector('[data-type="readonly"]')
			.setAttribute(
				'aria-label',
				window.siyuan.config.editor.readOnly ? window.siyuan.languages.tempUnlock : window.siyuan.languages.unlockEdit
			);
...
export const enableProtyle = (protyle: IProtyle) => {
	...
	if (protyle.breadcrumb) {
		protyle.breadcrumb.element.parentElement
			.querySelector('[data-type="readonly"] use')
			.setAttribute('xlink:href', '#iconUnlock');
		protyle.breadcrumb.element.parentElement
			.querySelector('[data-type="readonly"]')
			.setAttribute(
				'aria-label',
				window.siyuan.config.editor.readOnly
					? window.siyuan.languages.cancelTempUnlock
...
const focusElementById = (protyle: IProtyle, action: string[], scrollAttr?: IScrollAttr) => {
	...
	if (scrollAttr && scrollAttr.focusId) {
		...
		Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${protyle.block.id}"]`)).find(
			(item: HTMLElement) => {
	...
	if (
		...
		action.includes(Constants.CB_GET_HL) ||
		action.includes(Constants.CB_GET_FOCUSFIRST)
	) {
		const contentRect = protyle.contentElement.getBoundingClientRect();
