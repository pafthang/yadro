...
export const insertRow = (protyle: IProtyle, range: Range, cellElement: HTMLElement, nodeElement: Element) => {
	...
	if (cellElement.tagName === 'TH') {
		...
		if (tbodyElement) {
			...
			newRowElememt = cellElement.parentElement.parentElement.nextElementSibling
...
export const insertRowAbove = (protyle: IProtyle, range: Range, cellElement: HTMLElement, nodeElement: Element) => {
	...
	for (let m = 0; m < cellElement.parentElement.childElementCount; m++) {
		...
		if (cellElement.tagName === 'TH') {
			rowHTML += `<th class="${currentCellElement.className}" colspan="${
				currentCellElement.colSpan
			}" align="${currentCellElement.getAttribute('align')}"></th>`;
		} else {
			rowHTML += `<td class="${currentCellElement.className}" colspan="${
				currentCellElement.colSpan
			}" align="${currentCellElement.getAttribute('align')}"></td>`;
		}
	}
	...
	if (
		...
		cellElement.parentElement.parentElement.nextElementSibling.insertAdjacentHTML(
			'afterbegin',
			cellElement.parentElement.parentElement.innerHTML
		);
...
export const insertColumn = (
	...
	for (let i = 0; i < tableElement.rows.length; i++) {
		...
		if (colCellElement.isSameNode(cellElement)) {
			...
			// 滚动条横向定位
			if (
				newCellElement.offsetLeft + newCellElement.clientWidth >
				nodeElement.firstElementChild.scrollLeft + nodeElement.firstElementChild.clientWidth
			) {
				nodeElement.firstElementChild.scrollLeft =
					newCellElement.offsetLeft + newCellElement.clientWidth - nodeElement.firstElementChild.clientWidth;
			}
		} else {
...
export const deleteColumn = (protyle: IProtyle, range: Range, nodeElement: Element, cellElement: HTMLElement) => {
	...
	if (sideCellElement) {
		...
		// 滚动条横向定位
		if (
			sideCellElement.offsetLeft + sideCellElement.clientWidth >
			nodeElement.firstElementChild.scrollLeft + nodeElement.firstElementChild.clientWidth
		) {
			nodeElement.firstElementChild.scrollLeft =
				sideCellElement.offsetLeft + sideCellElement.clientWidth - nodeElement.firstElementChild.clientWidth;
		}
	}
	const tableElement = nodeElement.querySelector('table');
	for (let i = 0; i < tableElement.rows.length; i++) {
...
export const moveRowToDown = (protyle: IProtyle, range: Range, cellElement: HTMLElement, nodeElement: Element) => {
	const rowElement = cellElement.parentElement;
	if (
		(rowElement.parentElement.tagName === 'TBODY' && !rowElement.nextElementSibling) ||
...
export const moveColumnToRight = (protyle: IProtyle, range: Range, cellElement: HTMLElement, nodeElement: Element) => {
	...
	// 滚动条横向定位
	if (
		cellElement.offsetLeft + cellElement.clientWidth >
		nodeElement.firstElementChild.scrollLeft + nodeElement.firstElementChild.clientWidth
	) {
		nodeElement.firstElementChild.scrollLeft =
			cellElement.offsetLeft + cellElement.clientWidth - nodeElement.firstElementChild.clientWidth;
	}
	const colElements = nodeElement.querySelectorAll('col');
...
export const fixTable = (protyle: IProtyle, event: KeyboardEvent, range: Range) => {
	...
	if (isNotCtrl(event) && !event.shiftKey && !event.altKey && event.key === 'Enter') {
		...
		if (
			(!trElement.nextElementSibling && trElement.parentElement.tagName === 'TBODY') ||
	...
	if (event.key === 'ArrowUp' && isNotCtrl(event) && !event.shiftKey && !event.altKey) {
		...
		if (startContainer.nodeType !== 3 && (startContainer.tagName === 'TH' || startContainer.tagName === 'TD')) {
			previousBrElement = (startContainer.childNodes[Math.max(0, range.startOffset - 1)] as HTMLElement)
	...
	if (
		...
		!event.altKey &&
		event.key === 'Backspace' &&
		getSelectionOffset(cellElement, protyle.wysiwyg.element, range).start === 0 &&
		range.toString() === '' &&
	...
	if (matchHotKey(window.siyuan.config.keymap.editor.table['delete-row'].custom, event)) {
		if (
			(!hasNone && !hasRowSpan) || //https://github.com/siyuan-note/siyuan/issues/5045
