...
export const fixTableRange = (range: Range) => {
	...
	if (range.toString() !== '' && tableElement && range.commonAncestorContainer.nodeType !== 3) {
		...
		if (parentTag !== 'TH' && parentTag !== 'TD') {
			...
			const endCellElement =
				hasClosestByMatchTag(range.endContainer, 'TD') || hasClosestByMatchTag(range.endContainer, 'TH');
...
export const selectAll = (protyle: IProtyle, nodeElement: Element, range: Range) => {
			...
			if (position.start !== 0 || position.end !== editElement.textContent.length) {
				...
				while (firstChild) {
					if (firstChild.nodeType === 3) {
						...
						if (
							(firstChild as HTMLElement).classList.contains('render-node') ||
				...
				while (lastChild) {
					if (lastChild.nodeType === 3) {
						...
						if (
							lastChild.classList.contains('render-node') ||
	...
	if (
		protyle.wysiwyg.element.childElementCount === selectElements.length &&
		selectElements[0].parentElement.isSameNode(protyle.wysiwyg.element)
	) {
		return true;
	}
	...
	if (range.getClientRects().length === 0) {
		if (range.startContainer.nodeType === 3) {
			...
			if (parentRects.length > 0 || previousRects.length > 0) {
				if (
					parentRects.length === 0 ||
					(previousRects &&
						previousRects.length > 0 &&
						parentRects[0].top < previousRects[previousRects.length - 1].bottom)
				) {
					cursorRect = {
