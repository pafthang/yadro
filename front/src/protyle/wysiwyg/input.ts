...
export const input = async (protyle: IProtyle, blockElement: HTMLElement, range: Range, needRender = true) => {
	...
	if (!editElement) {
		...
		if (type === 'NodeThematicBreak') {
			...
			blockElement.innerHTML =
				'<wbr>' + blockElement.firstElementChild.outerHTML + blockElement.lastElementChild.outerHTML;
		} else if (type === 'NodeVideo') {
			blockElement.firstElementChild.innerHTML =
				'<wbr>' +
				Constants.ZWSP +
				blockElement.firstElementChild.firstElementChild.outerHTML +
				blockElement.firstElementChild.lastElementChild.outerHTML;
		} else if (type === 'NodeAudio') {
			blockElement.firstElementChild.innerHTML =
				blockElement.firstElementChild.firstElementChild.outerHTML + '<wbr>' + Constants.ZWSP;
	...
	if (
		brElement &&
		brElement.parentElement.tagName !== 'TD' &&
		brElement.parentElement.tagName !== 'TH' &&
	...
	if (editElement.textContent === '---' && type !== 'NodeCodeBlock') {
		...
		if (
			...
			if (
				...
				let replaceInnerHTML = editElement.innerHTML
					.replace(/^(~|·|`){3,}/g, '```')
					.replace(/\n(~|·|`){3,}/g, '\n```')
					.trim();
	...
	if (
		needRender &&
		(getContenteditableElement(tempElement.content.firstElementChild)?.innerHTML !==
			getContenteditableElement(blockElement).innerHTML ||
			...
			(tempElement.content.childElementCount === 1 &&
				getContenteditableElement(tempElement.content.firstElementChild)?.innerHTML === '<wbr>')) &&
		!(
			tempElement.content.childElementCount === 1 &&
			tempElement.content.firstElementChild.classList.contains('code-block') &&
			type === 'NodeCodeBlock'
		)
	) {
	...
	Array.from(tempElement.content.children).forEach((item, index) => {
		...
		if (tempId === id) {
			...
			doOperations.push({
				...
				previousID:
					index === 0
						? firstElement?.previousElementSibling?.getAttribute('data-node-id')
