...
const listEnter = (protyle: IProtyle, blockElement: HTMLElement, range: Range) => {
	...
	if (
		...
		updateTransaction(
			protyle,
			listItemElement.parentElement.getAttribute('data-node-id'),
			listItemElement.parentElement.outerHTML,
			html
		);
	...
	if (
		subListElement &&
		listItemElement.getAttribute('fold') !== '1' &&
		...
		if (
			position.end >=
			editableElement.textContent.length -
	...
	if (
		(range.toString() === '' || range.toString() === Constants.ZWSP) &&
	...
	if (
		(editableElement?.lastElementChild?.getAttribute('data-type') || '').indexOf('inline-math') > -1 &&
...
export const enter = (blockElement: HTMLElement, range: Range, protyle: IProtyle) => {
	...
	if (disableElement) {
		if (blockElement.parentElement.classList.contains('li')) {
			...
			updateTransaction(
				protyle,
				blockElement.parentElement.parentElement.getAttribute('data-node-id'),
				blockElement.parentElement.parentElement.outerHTML,
				oldHTML
			);
			focusByWbr(newElement, range);
			removeEmptyNode(newElement);
	...
	if (
		...
		if (
			...
			// ```test` 不处理，正常渲染为段落块
		} else if (blockElement.classList.contains('p')) {
			// https://github.com/siyuan-note/siyuan/issues/6953
			...
			blockElement = protyle.wysiwyg.element.querySelector(
	...
	// bq
	if (
		editableElement.textContent.replace(Constants.ZWSP, '').replace('\n', '') === '' &&
		...
		if (topId === blockId) {
			...
			doInsert.previousID = topElement.previousElementSibling
	...
	if (
		...
		(blockElement.nextElementSibling.classList.contains('protyle-attr') ||
			(blockElement.nextElementSibling.classList.contains('list') &&
				blockElement.previousElementSibling?.classList.contains('protyle-action')) ||
			...
			[
				{
					...
					data: newElement.outerHTML,
					id: newId,
					previousID: blockElement.previousElementSibling
						? blockElement.previousElementSibling.getAttribute('data-node-id')
						: '',
