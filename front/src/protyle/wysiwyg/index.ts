...
export class WYSIWYG {
	...
	public renderCustom(ial: IObject) {
		...
		for (let i = 0; i < this.element.attributes.length; i++) {
			...
			if (
				!['type', 'class', 'spellcheck', 'contenteditable', 'data-doc-type', 'style', 'data-realwidth'].includes(
					oldKey
				) &&
	...
	private escapeInline(protyle: IProtyle, range: Range, event: InputEvent) {
		...
		if (
			...
			range.toString() === '' &&
			range.startOffset === inputData.length &&
		...
		if (
			...
			(currentTypes.includes('inline-memo') ||
				currentTypes.includes('text') ||
				currentTypes.includes('block-ref') ||
				currentTypes.includes('file-annotation-ref') ||
	...
	private setEmptyOutline(protyle: IProtyle, element: HTMLElement) {
		...
		protyle.wysiwyg.element
			.querySelectorAll('.img--select, .av__cell--select, .av__cell--active, .av__row--select')
	...
	private bindCommonEvent(protyle: IProtyle) {
		this.element.addEventListener('copy', (event: ClipboardEvent & { target: HTMLElement }) => {
			...
			if (selectElements.length > 0) {
				...
				if (
					selectElements[0].getAttribute('data-type') === 'NodeListItem' &&
					...
					if (isRefText) {
						...
						if (cloneEditElement) {
							cloneEditElement.insertAdjacentHTML(
								'beforeend',
								` <span data-type="block-ref" data-subtype="s" data-id="${cloneElement.getAttribute(
									'data-node-id'
								)}">*</span>`
							);
						}
					...
					selectElements.forEach((item, index) => {
						...
						if (isRefText && index === 0) {
							...
							if (cloneEditElement) {
								cloneEditElement.insertAdjacentHTML(
									'beforeend',
									` <span data-type="block-ref" data-subtype="s" data-id="${item.getAttribute(
										'data-node-id'
									)}">*</span>`
								);
							}
				...
				if (cellElements.length > 0) {
					...
					cellElements.forEach((item: HTMLElement, index) => {
						...
						textPlain +=
							...
							item.nextElementSibling.isSameNode(cellElements[index + 1])
								? '\t'
								: '\n\n');
					});
				...
				if (
					(selectTypes.length > 0 ||
						range.startContainer.parentElement.parentElement.getAttribute('data-type') === 'NodeHeading') &&
					((range.startContainer.nodeType === 3 &&
						range.startContainer.parentElement.textContent === range.toString()) ||
					...
					if (
						spanElement.getAttribute('data-type').indexOf('block-ref') > -1 &&
		...
		this.element.addEventListener('mousedown', (event: MouseEvent) => {
			...
			const mostRight =
				...
				(protyle.wysiwyg.element.clientWidth -
					(parseInt(protyle.wysiwyg.element.style.paddingLeft) || 24) -
					(parseInt(protyle.wysiwyg.element.style.paddingRight) || 16)) -
				2;
			...
			if (!protyle.disabled && avDragFillElement) {
				...
				documentSelf.onmouseup = () => {
					...
					if (lastCellElement) {
						...
						lastCellElement.insertAdjacentHTML(
							'beforeend',
			...
			if (!protyle.disabled && avCellElement && avCellElement.dataset.id) {
				...
				documentSelf.onmousemove = (moveEvent: MouseEvent) => {
					...
					if (
						tempCellElement &&
						tempCellElement.dataset.id &&
						(event.clientX !== moveEvent.clientX || event.clientY !== moveEvent.clientY)
					) {
						...
						nodeElement.querySelectorAll('.av__row').forEach((rowElement: HTMLElement, index: number) => {
							if (
								index >= Math.min(originIndex.rowIndex, newIndex.rowIndex) &&
								index <= Math.max(originIndex.rowIndex, newIndex.rowIndex)
							) {
								rowElement.querySelectorAll('.av__cell').forEach((cellElement: HTMLElement, cellIndex: number) => {
									if (
										cellIndex >= Math.min(originIndex.celIndex, newIndex.celIndex) &&
										cellIndex <= Math.max(originIndex.celIndex, newIndex.celIndex)
									) {
										cellElement.classList.add('av__cell--active');
										lastCellElement = cellElement;
									}
			...
			if (!protyle.disabled && target.classList.contains('protyle-action__drag')) {
				...
				documentSelf.onmousemove = (moveEvent: MouseEvent) => {
					...
					if (moveEvent.clientX > x - dragWidth + 8 && moveEvent.clientX < mostRight) {
						if (
							(dragElement.tagName === 'IMG' && dragElement.parentElement.parentElement.style.display !== 'block') ||
							!isCenter
						) {
			...
			if (
				target.tagName === 'TH' ||
				target.tagName === 'TD' ||
				target.firstElementChild?.tagName === 'TABLE' ||
				target.classList.contains('table__resize') ||
				target.classList.contains('table__select')
			) {
			...
			if (!protyle.disabled && target.classList.contains('table__resize')) {
				...
				if (colElement.style.minWidth) {
					colElement.style.width =
						(nodeElement.querySelectorAll('table td, table th')[colIndex] as HTMLElement).offsetWidth + 'px';
			...
			const mostTop =
				rect.top + (protyle.options.render.breadcrumb ? protyle.breadcrumb.element.parentElement.clientHeight : 0);
			...
			documentSelf.onmousemove = (moveEvent: MouseEvent) => {
				...
				if (
					!protyle.disabled &&
					tableBlockElement &&
					tableBlockElement.contains(moveTarget) &&
					!hasClosestByClassName(tableBlockElement, 'protyle-wysiwyg__embed')
				) {
					if (
						(moveTarget.tagName === 'TH' || moveTarget.tagName === 'TD') &&
						!moveTarget.isSameNode(target) &&
						...
						Array.from(tableBlockElement.querySelectorAll('th, td')).find((item: HTMLElement) => {
							...
							if (item.offsetTop < top && item.offsetTop + item.clientHeight > top) {
								if (
									(item.offsetLeft + 6 > left && item.offsetLeft + item.clientWidth - 6 < left + width) ||
									updateWidth ||
									updateWidth2
								) {
								...
								updateWidth2 &&
								item.offsetTop + 6 > top &&
								item.offsetTop + item.clientHeight - 6 < top + height
							) {
								width = left + width - item.offsetLeft;
								left = item.offsetLeft;
							} else if (
						...
						tableBlockElement
							.querySelector('.table__select')
							.setAttribute(
								'style',
								`left:${
									left - tableBlockElement.firstElementChild.scrollLeft
								}px;top:${top}px;height:${height}px;width:${width + 1}px;`
							);
				...
				protyle.selectElement.setAttribute(
					'style',
					`background-color: ${
						protyle.selectElement.style.backgroundColor
					};top:${newTop}px;height:${newHeight}px;left:${newLeft + 2}px;width:${newWidth - 2}px;`
				);
				const newMouseElement = document.elementFromPoint(moveEvent.clientX, moveEvent.clientY);
				if (
					mouseElement &&
					mouseElement.isSameNode(newMouseElement) &&
					!mouseElement.classList.contains('protyle-wysiwyg') &&
					!mouseElement.classList.contains('list') &&
					!mouseElement.classList.contains('bq') &&
					!mouseElement.classList.contains('sb')
				) {
					// 性能优化，同一个p元素不进行选中计算
					return;
				} else {
				...
				if (
					firstElement.classList.contains('protyle-wysiwyg') ||
					firstElement.classList.contains('list') ||
					firstElement.classList.contains('sb') ||
				...
				if (
					...
					!selectElements[0].classList.contains('list') &&
					!selectElements[0].classList.contains('bq') &&
			...
			documentSelf.onmouseup = (mouseUpEvent) => {
								...
								click: () => {
									if (tableBlockElement) {
										...
										tableBlockElement
											...
											.forEach((item: HTMLTableCellElement, index: number) => {
												if (item.classList.contains('fn__none')) {
													// 合并的元素中间有 fn__none 的元素
													if (
														item.previousElementSibling &&
														item.previousElementSibling.isSameNode(selectCellElements[selectCellElements.length - 1])
													) {
													...
													if (
														item.offsetLeft + 6 > tableSelectElement.offsetLeft + scrollLeft &&
														item.offsetLeft + item.clientWidth - 6 <
															tableSelectElement.offsetLeft + scrollLeft + tableSelectElement.clientWidth &&
														item.offsetTop + 6 > tableSelectElement.offsetTop &&
														item.offsetTop + item.clientHeight - 6 <
															tableSelectElement.offsetTop + tableSelectElement.clientHeight
													) {
														selectCellElements.push(item);
														if (!isTHead && item.parentElement.parentElement.tagName === 'THEAD') {
															isTHead = true;
										...
										while (
											...
											cellElement.nextElementSibling.isSameNode(selectCellElements[index])
										) {
											cellElement = cellElement.nextElementSibling as HTMLTableCellElement;
											if (!cellElement.classList.contains('fn__none')) {
												// https://github.com/siyuan-note/insider/issues/1007#issuecomment-1046195608
										...
										selectCellElements.forEach((item, index) => {
											...
											if (index !== 0) {
												if (!rowElement.isSameNode(item.parentElement)) {
													if (!item.classList.contains('fn__none')) {
														// https://github.com/siyuan-note/insider/issues/1011
														rowSpan += item.rowSpan;
													}
													rowElement = item.parentElement;
													if (
														selectCellElements[0].parentElement.parentElement.tagName === 'THEAD' &&
														item.parentElement.parentElement.tagName !== 'THEAD'
													) {
														selectCellElements[0].parentElement.parentElement.insertAdjacentElement(
															'beforeend',
															item.parentElement
														);
													}
										...
										setTimeout(() => {
											if (tableBlockElement) {
												...
												updateTransaction(
													protyle,
													tableBlockElement.getAttribute('data-node-id'),
													tableBlockElement.outerHTML,
													oldHTML
												);
											}
										});
									...
									if (tableBlockElement) {
										...
										tableBlockElement.querySelectorAll('th, td').forEach((item: HTMLTableCellElement) => {
											if (
												...
												item.offsetTop + item.clientHeight - 6 <
													tableSelectElement.offsetTop + tableSelectElement.clientHeight &&
												(selectCellElements.length === 0 ||
													(selectCellElements.length > 0 && item.offsetTop === selectCellElements[0].offsetTop))
											) {
												selectCellElements.push(item);
											}
										});
										...
										setTableAlign(
											...
											tableBlockElement,
											'left',
											getEditorRange(tableBlockElement)
										);
									}
								}
							}).element
				...
				if (getSelection().rangeCount > 0) {
					...
					if (
						mouseUpEvent.detail > 2 &&
						range.endContainer.nodeType !== 3 &&
						(range.endContainer as HTMLElement).tagName === 'DIV' &&
						range.endOffset === 0
					) {
	...
	private bindEvent(protyle: IProtyle) {
			...
			if (selectElements.length > 0) {
				if (
					...
					if (selectElements[0].getAttribute('data-type') === 'NodeListItem') {
						html = `<div data-subtype="${selectElements[0].getAttribute(
							'data-subtype'
						)}" data-node-id="${Lute.NewNodeID()}" data-type="NodeList" class="list">${html}<div class="protyle-attr" contenteditable="false">${
							Constants.ZWSP
						}</div></div>`;
					}
				}
				...
				if (headElement && range.toString() === headElement.firstElementChild.textContent) {
					...
					if (headElement.getAttribute('fold') === '1') {
						...
						if (
							(headElement.parentElement.childElementCount === 3 &&
								headElement.parentElement.classList.contains('li')) ||
							(headElement.parentElement.childElementCount === 2 &&
								(headElement.parentElement.classList.contains('bq') ||
									headElement.parentElement.classList.contains('sb'))) ||
							(headElement.parentElement.childElementCount === 1 &&
								headElement.parentElement.classList.contains('protyle-wysiwyg')) // 全选剪切标题
						) {
					...
					range.toString() !== '' &&
					startContainer.isSameNode(range.endContainer) &&
					range.startContainer.nodeType === 3 &&
		...
		this.element.addEventListener('contextmenu', (event: MouseEvent & { detail: any }) => {
			...
			if (target.tagName === 'SPAN') {
				// https://ld246.com/article/1665141518103
				...
				if (types.includes('block-ref')) {
					...
					if (
						window.siyuan.config.editor.floatWindowMode === 0 &&
			...
			if (
				...
				(isMobile() ||
					event.detail.target ||
					(beforeContextmenuRange && nodeElement.contains(beforeContextmenuRange.startContainer)))
			) {
				if (
					(!isMobile() || protyle.toolbar?.element.classList.contains('fn__none')) &&
		...
		this.element.addEventListener('keyup', (event) => {
			...
			if (
				...
				if (
					range.toString() === '' && // windows 下回车新建块输入abc，选中 bc ctrl+m 后光标错误
					nodeElement &&
					typeof protyle.wysiwyg.lastHTMLs[nodeElement.getAttribute('data-node-id')] === 'undefined'
				) {
					range.insertNode(document.createElement('wbr'));
					protyle.wysiwyg.lastHTMLs[nodeElement.getAttribute('data-node-id')] = nodeElement.outerHTML;
					nodeElement.querySelector('wbr').remove();
			...
			if (
				(event.key === 'ArrowLeft' || event.key === 'ArrowRight' || event.key === 'Alt' || event.key === 'Shift') && // 选中后 alt+shift+arrowRight 会导致光标和选中块不一致
				nodeElement &&
				!nodeElement.classList.contains('protyle-wysiwyg--select')
			) {
		...
		this.element.addEventListener('click', (event: MouseEvent & { target: HTMLElement }) => {
						...
						checkFold(breadcrumbId, (zoomIn) => {
							openFileById({
								...
								action: zoomIn
									...
									: [Constants.CB_GET_FOCUS, Constants.CB_GET_CONTEXT],
								zoomIn
							});
						});
					} else {
			...
			this.element.querySelectorAll('.table').forEach((item) => {
				...
				if (
					tableElement &&
					tableElement.isSameNode(item) &&
					item.querySelector('.table__select').getAttribute('style')
				) {
					// 防止合并单元格的菜单消失
					event.stopPropagation();
				}
			...
			if (fileElement && range.toString() === '') {
				...
				if (!ctrlIsPressed || !event.shiftKey) {
			...
			if (aElement && !event.altKey) {
				...
				if (isLocalPath(linkAddress)) {
					...
					if (
						Constants.SIYUAN_ASSETS_EXTS.includes(pathPosix().extname(linkPathname)) &&
						(!linkPathname.endsWith('.pdf') || (linkPathname.endsWith('.pdf') && !linkAddress.startsWith('file://')))
					) {
						if (!ctrlIsPressed || !event.shiftKey) {
				...
				checkFold(embedId, (zoomIn, action) => {
					if (event.shiftKey) {
						...
						openFileById({
							...
							action: zoomIn
								? [Constants.CB_GET_HL, Constants.CB_GET_ALL]
								: [Constants.CB_GET_HL, Constants.CB_GET_CONTEXT],
							zoomIn,
							keepCursor: true
						});
				...
				if (type === 'img' && !protyle.disabled) {
					...
					if (event.altKey && !protyle.disabled) {
						...
						if (actionElement.parentElement.parentElement.classList.contains('protyle-wysiwyg')) {
							...
							updateTransaction(
								protyle,
								actionElement.parentElement.parentElement.getAttribute('data-node-id'),
								actionElement.parentElement.parentElement.outerHTML,
								oldHTML
							);
						}
						hideElements(['gutter'], protyle);
			...
			const selectElement = hasClosestByClassName(event.target, 'hr') || hasClosestByClassName(event.target, 'iframe');
			...
			if (event.target.contains(this.element) && this.element.lastElementChild && !protyle.disabled) {
				...
				if (event.y > lastRect.bottom) {
					...
					if (
						!lastEditElement ||
						(this.element.lastElementChild.getAttribute('data-type') !== 'NodeParagraph' &&
							protyle.wysiwyg.element.getAttribute('data-doc-type') !== 'NodeListItem') ||
						(this.element.lastElementChild.getAttribute('data-type') === 'NodeParagraph' &&
							getContenteditableElement(lastEditElement).innerHTML !== '')
					) {
			...
			setTimeout(
				...
				isMobile() || isInIOS() ? 520 : 0
			); // Android/iPad 双击慢了出不来
