			...
			if ((expand && index > 2) || (!expand && index > 1)) {
				if ((expand && index === 3) || (!expand && index === 2)) {
					item.insertAdjacentHTML(
						'beforebegin',
						'<div style="max-width: 100%;justify-content: center;" contenteditable="false" class="protyle-breadcrumb__item"><svg><use xlink:href="#iconMore"></use></svg></div>'
					);
				}
	...
	fetchPost(
		...
		(getResponse) => {
			element.parentElement
				.querySelector('.protyle-breadcrumb__item--active')
				.classList.remove('protyle-breadcrumb__item--active');
