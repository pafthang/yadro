...
export const genListItemElement = (listItemElement: Element, offset = 0, wbr = false) => {
	...
	if (type === 'o') {
		...
		element.innerHTML = `<div data-marker="${
			index + 1
		}." data-subtype="${type}" data-node-id="${Lute.NewNodeID()}" data-type="NodeListItem" class="li"><div contenteditable="false" class="protyle-action protyle-action--order" draggable="true">${
			index + 1
		}.</div>${genEmptyBlock(false, wbr)}<div class="protyle-attr" contenteditable="false"></div></div>`;
	} else if (type === 't') {
		element.innerHTML = `<div data-marker="*" data-subtype="${type}" data-node-id="${Lute.NewNodeID()}" data-type="NodeListItem" class="li"><div class="protyle-action protyle-action--task" draggable="true"><svg><use xlink:href="#iconUncheck"></use></svg></div>${genEmptyBlock(
			false,
			wbr
		)}<div class="protyle-attr" contenteditable="false"></div></div>`;
	} else {
		element.innerHTML = `<div data-marker="*" data-subtype="${type}" data-node-id="${Lute.NewNodeID()}" data-type="NodeListItem" class="li"><div class="protyle-action" draggable="true"><svg><use xlink:href="#iconDot"></use></svg></div>${genEmptyBlock(
			false,
			wbr
		)}<div class="protyle-attr" contenteditable="false"></div></div>`;
	}
...
export const listIndent = (protyle: IProtyle, liItemElements: Element[], range: Range) => {
	...
	if (previousElement.lastElementChild.previousElementSibling.getAttribute('data-type') === 'NodeList') {
		...
		let previousID =
			previousElement.lastElementChild.previousElementSibling.lastElementChild.previousElementSibling.getAttribute(
				'data-node-id'
			);
	...
	if (!previousElement.parentElement.classList.contains('protyle-wysiwyg')) {
		updateTransaction(
			protyle,
			previousElement.parentElement.getAttribute('data-node-id'),
			previousElement.parentElement.outerHTML,
			html
		);
	}
...
export const breakList = (protyle: IProtyle, blockElement: Element, range: Range) => {
	...
	newListHTML = `<div data-subtype="${listItemElement.getAttribute(
		'data-subtype'
	)}" data-node-id="${newListId}" data-type="NodeList" class="list" updated="${dayjs().format(
		'YYYYMMDDHHmmss'
	)}">${newListHTML}<div class="protyle-attr" contenteditable="false">${Constants.ZWSP}</div></div>`;
	...
	Array.from(listItemElement.children)
		.reverse()
		.forEach((item) => {
...
export const listOutdent = (
	...
	if (
		...
		!parentParentElement.getAttribute('data-node-id')
	) {
		// https://ld246.com/article/1691981936960 情况下 zoom in 列表项
		return;
	}
	if (
		parentLiItemElement.classList.contains('protyle-wysiwyg') ||
		parentLiItemElement.classList.contains('sb') ||
		parentLiItemElement.classList.contains('bq')
	) {
		// 顶层列表
		const doOperations: IOperation[] = [];
		const undoOperations: IOperation[] = [];
		...
		if (!window.siyuan.config.editor.listLogicalOutdent && !nextElement.classList.contains('protyle-attr')) {
			...
			while (nextElement && !nextElement.classList.contains('protyle-attr')) {
				doOperations.push({
					...
					previousID:
						topOldPreviousID || lastBlockElement.lastElementChild.previousElementSibling?.getAttribute('data-node-id'),
	...
	liItemElements.reverse().forEach((item) => {
		...
		if (
			(item.getAttribute('data-subtype') === 'o' || item.getAttribute('data-subtype') === 't') &&
			...
			(item.getAttribute('data-subtype') === 'u' || item.getAttribute('data-subtype') === 't') &&
			...
			item.querySelector('.protyle-action').outerHTML =
				'<div contenteditable="false" draggable="true" class="protyle-action protyle-action--order">1.</div>';
			...
			(item.getAttribute('data-subtype') === 'u' || item.getAttribute('data-subtype') === '0') &&
			...
			item.querySelector('.protyle-action').outerHTML =
				'<div class="protyle-action protyle-action--task" draggable="true"><svg><use xlink:href="#iconUncheck"></use></svg></div>';
	...
	if (!window.siyuan.config.editor.listLogicalOutdent && !nextElement.classList.contains('protyle-attr')) {
		...
		while (nextElement && !nextElement.classList.contains('protyle-attr')) {
			...
			if (nextElement.getAttribute('data-subtype') !== lastBlockElement.getAttribute('data-subtype')) {
				...
				nextElement.querySelector('.protyle-action').outerHTML =
					lastBlockElement.querySelector('.protyle-action').outerHTML;
			...
			doOperations.push({
				...
				id: nextId,
				previousID:
					subPreviousID || lastBlockElement.lastElementChild.previousElementSibling?.getAttribute('data-node-id'),
				parentID: lastBlockElement.getAttribute('data-node-id')
			});
			undoOperations.push({
				action: 'move',
