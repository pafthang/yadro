...
import { focusBlock, getSelectionOffset, setFirstNodeRange } from '../util/selection';
	...
	if (selectElements.length > 0) {
		...
		if (getSelectionOffset(options.nodeElement, options.editorElement, options.range).start !== 0) {
			...
			if (editElement.tagName === 'TABLE') {
				const cellElement =
					hasClosestByMatchTag(options.range.startContainer, 'TH') ||
					hasClosestByMatchTag(options.range.startContainer, 'TD') ||
					editElement.querySelector('th, td');
...
export const downSelect = (options: {
	...
	if (selectElements.length > 0) {
		...
		if (
			getSelectionOffset(options.nodeElement, options.editorElement, options.range).end <
			getContenteditableElement(options.nodeElement).textContent.length
		) {
...
export const alignImgCenter = (
	protyle: IProtyle,
	nodeElement: Element,
	assetElements: Element[],
	id: string,
	html: string
) => {
