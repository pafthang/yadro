...
export const moveToUp = (protyle: IProtyle, nodeElement: HTMLElement, range: Range) => {
	...
	if (
		...
		if (sourceElements[0].parentElement.parentElement.previousElementSibling?.classList.contains('li')) {
			...
			if (!previousElement) {
				...
				sourceElements[0].parentElement.parentElement.previousElementSibling.lastElementChild.insertAdjacentHTML(
					'beforebegin',
					`<div data-subtype="${sourceElements[0].getAttribute(
						'data-subtype'
					)}" data-node-id="${newId}" data-type="NodeList" class="list" updated="${
						newId.split('-')[0]
					}"><div id="moveTempLi"></div><div class="protyle-attr" contenteditable="false">&ZeroWidthSpace;</div></div>`
				);
				previousElement = sourceElements[0].parentElement.parentElement.previousElementSibling.querySelector('.list');
			}
		} else {
	...
	if (
		...
		if (sourceElements[0].parentElement.previousElementSibling?.classList.contains('li')) {
			...
			if (!previousElement) {
				const newId = Lute.NewNodeID();
				sourceElements[0].parentElement.previousElementSibling.lastElementChild.insertAdjacentHTML(
					'beforebegin',
					`<div data-subtype="${sourceElements[0].getAttribute(
						'data-subtype'
					)}" data-node-id="${newId}" data-type="NodeList" class="list" updated="${
	...
	if (previousElement) {
		...
		const sourceParentElement = sourceElements[0].classList.contains('list')
			? sourceElements[0]
			: sourceElements[0].parentElement;
		...
		if (sourceParentElement.childElementCount === 1) {
			...
			sourceParentElement.getAttribute('data-subtype') === 'o' &&
			sourceParentElement.classList.contains('list')
		) {
		...
		updateTransaction(
			protyle,
			previousElement.parentElement.parentElement.parentElement.getAttribute('data-node-id'),
			previousElement.parentElement.parentElement.parentElement.outerHTML,
			oldListHTML
		);
	...
	if (
		!sourceElements[0].previousElementSibling ||
		sourceElements[0].previousElementSibling?.classList.contains('protyle-action')
	) {
		return;
	}
	...
	if (sourceElements[0].getAttribute('data-subtype') === 'o' && type === 'NodeListItem') {
		...
		updateTransaction(
			protyle,
			sourceElements[0].parentElement.getAttribute('data-node-id'),
			sourceElements[0].parentElement.outerHTML,
			html
		);
	} else {
...
export const moveToDown = (protyle: IProtyle, nodeElement: HTMLElement, range: Range) => {
	...
	if (
		...
		if (sourceElements[0].parentElement.parentElement.nextElementSibling?.classList.contains('li')) {
			...
			if (!nextElement) {
				const newId = Lute.NewNodeID();
				sourceElements[0].parentElement.parentElement.nextElementSibling.lastElementChild.insertAdjacentHTML(
					'beforebegin',
					`<div data-subtype="${sourceElements[0].getAttribute(
						'data-subtype'
					)}" data-node-id="${newId}" data-type="NodeList" class="list" updated="${
	...
	if (
		...
		if (sourceElements[0].parentElement.nextElementSibling?.classList.contains('li')) {
			...
			if (!nextElement) {
				const newId = Lute.NewNodeID();
				sourceElements[0].parentElement.nextElementSibling.lastElementChild.insertAdjacentHTML(
					'beforebegin',
					`<div data-subtype="${sourceElements[0].getAttribute(
						'data-subtype'
					)}" data-node-id="${newId}" data-type="NodeList" class="list" updated="${
	...
	if (nextElement) {
		...
		updateTransaction(
			protyle,
			nextElement.parentElement.parentElement.parentElement.getAttribute('data-node-id'),
			nextElement.parentElement.parentElement.parentElement.outerHTML,
			oldListHTML
		);
		preventScroll(protyle);
		scrollCenter(protyle);
	...
	if (
		!sourceElements[sourceElements.length - 1].nextElementSibling ||
		sourceElements[sourceElements.length - 1].nextElementSibling?.classList.contains('protyle-attr')
	) {
		return;
	}
	...
	if (nextElement.getAttribute('data-subtype') === 'o' && nextElement.getAttribute('data-type') === 'NodeListItem') {
		...
		updateTransaction(
			protyle,
			nextElement.parentElement.getAttribute('data-node-id'),
			nextElement.parentElement.outerHTML,
			html
		);
	} else {
