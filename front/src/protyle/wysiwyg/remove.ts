...
const removeLi = (protyle: IProtyle, blockElement: Element, range: Range, isDelete = false) => {
	if (
		!blockElement.parentElement.previousElementSibling &&
		blockElement.parentElement.nextElementSibling &&
		blockElement.parentElement.nextElementSibling.classList.contains('protyle-attr')
	) {
	...
	if (
		!blockElement.parentElement.previousElementSibling &&
		blockElement.parentElement.parentElement.parentElement.classList.contains('list')
	) {
	...
	if (!blockElement.parentElement.previousElementSibling) {
		...
		Array.from(tempElement.children).forEach((item, index) => {
			doOperations.push({
				...
				data: item.outerHTML,
				previousID:
					index === 0 ? listElement.previousElementSibling?.getAttribute('data-node-id') : doOperations[index - 1].id,
	...
	if (
		listItemElement.previousElementSibling &&
		listItemElement.previousElementSibling.classList.contains('protyle-breadcrumb__bar')
	) {
		return;
	}
...
export const removeBlock = (
	...
	if (selectElements?.length > 0) {
		...
		selectElements.find((item: HTMLElement) => {
			...
			if (topElement.getAttribute('data-type') === 'NodeHeading' && topElement.getAttribute('fold') === '1') {
				...
				inserts.push({
					...
					id,
					previousID: selectElements[0].previousElementSibling
						? selectElements[0].previousElementSibling.getAttribute('data-node-id')
						: '',
				...
				if (nextElement) {
					...
					if (
						nextType !== 'NodeHeading' ||
						(nextType === 'NodeHeading' &&
							nextElement.getAttribute('data-subtype') > topElement.getAttribute('data-subtype'))
					) {
						return true;
					}
				}
				...
				inserts.push({
					...
					data,
					id,
					previousID: topElement.previousElementSibling
		...
		if (sideElement) {
			if (
				...
				sideElement.classList.contains('protyle-wysiwyg') &&
				protyle.wysiwyg.element.childElementCount === 0
			) {
				setTimeout(() => {
		...
		if (deletes.length > 0) {
			if (
				topParentElement &&
				topParentElement.getAttribute('data-type') === 'NodeSuperBlock' &&
				topParentElement.childElementCount === 2
			) {
	...
	if (
		blockElement.previousElementSibling &&
		blockElement.previousElementSibling.classList.contains('protyle-breadcrumb__bar')
	) {
		return;
	}
	...
	if (
		...
		getSelectionOffset(editableElement).start === 0
	) {
		transaction(
			protyle,
			[
...
export const removeImage = (imgSelectElement: Element, nodeElement: HTMLElement, range: Range, protyle: IProtyle) => {
	...
	if (imgPreviousSibling && imgPreviousSibling.textContent.endsWith(Constants.ZWSP)) {
		imgPreviousSibling.textContent = imgPreviousSibling.textContent.substring(
			0,
			imgPreviousSibling.textContent.length - 1
		);
	}
