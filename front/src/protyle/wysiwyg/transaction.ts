	...
	fetchPost(
		...
		(response) => {
			...
			if (
				...
				window.siyuan.config.sync.enabled
			) {
			...
			response.data[0].doOperations.forEach((operation: IOperation) => {
				if (operation.action === 'unfoldHeading' || operation.action === 'foldHeading') {
					...
					if (
						...
						protyle.contentElement.scrollHeight - protyle.contentElement.scrollTop <
							protyle.contentElement.clientHeight * 2 // https://github.com/siyuan-note/siyuan/issues/7785
					) {
						fetchPost(
							'/api/filetree/getDoc',
							{
				...
				if (operation.action === 'move') {
					if (protyle.options.backlinkData) {
						...
						if (operation.previousID && updateElements.length > 0) {
							Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${operation.previousID}"]`)).forEach(
								(item) => {
									if (
										item.getAttribute('data-type') === 'NodeBlockQueryEmbed' ||
							...
							Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${operation.parentID}"]`)).forEach(
								(item) => {
									if (
										item.getAttribute('data-type') === 'NodeBlockQueryEmbed' ||
										!hasClosestByAttribute(item.parentElement, 'data-type', 'NodeBlockQueryEmbed')
					...
					if (protyle.options.backlinkData) {
						...
						if (operation.previousID) {
							Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${operation.previousID}"]`)).forEach(
								(item) => {
									if (
										item.nextElementSibling?.getAttribute('data-node-id') !== operation.id &&
								...
								(item) => {
									if (
										...
										if (
											...
											item.firstElementChild.classList.contains('protyle-action') &&
											...
											item.firstElementChild &&
											item.firstElementChild.getAttribute('data-node-id') !== operation.id
										) {
...
const updateEmbed = (protyle: IProtyle, operation: IOperation) => {
	...
	protyle.wysiwyg.element
		.querySelectorAll(`[data-type="NodeBlockQueryEmbed"] [data-node-id="${operation.id}"]`)
		.forEach((item) => {
...
const updateBlock = (updateElements: Element[], protyle: IProtyle, operation: IOperation, isUndo: boolean) => {
	...
	Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${operation.id}"]`)).find((item) => {
		if (
			item.getAttribute('data-type') === 'NodeBlockQueryEmbed' || // 引用转换为块嵌入，undo、redo 后也需要更新 updateElement
			!hasClosestByAttribute(item, 'data-type', 'NodeBlockQueryEmbed')
		) {
...
export const onTransaction = (protyle: IProtyle, operation: IOperation, isUndo: boolean) => {
	...
	if (operation.action === 'update') {
		if (updateElements.length > 0) {
			...
			// updateElements 没有包含嵌入块，在悬浮层编辑嵌入块时，嵌入块也需要更新
			// 更新 ws 嵌入块
			updateEmbed(protyle, operation);
			// 更新 ws 引用块
			updateRef(protyle, operation.id);
	...
	if (operation.action === 'updateAttrs') {
		...
		protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${operation.id}"]`).forEach((item: HTMLElement) => {
			...
			Object.keys(data.new).forEach((key) => {
				...
				if (
					key === Constants.CUSTOM_RIFF_DECKS &&
					data.new[Constants.CUSTOM_RIFF_DECKS] !== data.old[Constants.CUSTOM_RIFF_DECKS]
				) {
	...
	if (operation.action === 'move') {
		...
		if (updateElements.length === 0) {
			...
			getAllModels().editor.forEach((editor) => {
				const updateCloneElement = editor.editor.protyle.wysiwyg.element.querySelector(
		...
		protyle.wysiwyg.element.querySelectorAll('[data-type="NodeBlockQueryEmbed"]').forEach((item) => {
			if (
				item.querySelector(
					`[data-node-id="${operation.id}"],[data-node-id="${operation.parentID}"],[data-node-id="${operation.previousID}"]`
				)
			) {
				item.removeAttribute('data-render');
				blockRender(protyle, item);
...
export const turnsIntoOneTransaction = (options: {
	...
	if (options.type === 'BlocksMergeSuperBlock') {
		...
		options.selectsElement.forEach((item, index) => {
			if (options.type === 'Blocks2ULs') {
				...
				html += `<div data-marker="${
					index + 1
				}." data-subtype="o" data-node-id="${Lute.NewNodeID()}" data-type="NodeListItem" class="li"><div class="protyle-action protyle-action--order" contenteditable="false" draggable="true">${
					index + 1
	...
	const parentId =
		options.selectsElement[0].parentElement.getAttribute('data-node-id') || options.protyle.block.parentID;
	const doOperations: IOperation[] = [
		{
			action: 'insert',
			id,
	...
	focusBlock(
		options.protyle.wysiwyg.element.querySelector(
			`[data-node-id="${options.selectsElement[0].getAttribute('data-node-id')}"]`
		)
	);
...
export const turnsIntoTransaction = (options: {
	...
	if (options.nodeElement) {
		...
		if (
			selectsElement.length === 1 &&
			options.type === 'Blocks2Hs' &&
	...
	selectsElement.forEach((item, index) => {
		if (
			(options.type === 'Blocks2Ps' || options.type === 'Blocks2Hs') &&
			item.getAttribute('data-type') === 'NodeHeading' &&
		...
		undoOperations.push({
			...
			data: item.outerHTML,
			parentID:
				item.parentElement?.getAttribute('data-node-id') ||
				options.protyle.block.parentID ||
				options.protyle.block.rootID,
			previousID:
				undoOperations[undoOperations.length - 1]?.id || item.previousElementSibling?.getAttribute('data-node-id')
		});
	...
	if (range) {
		...
		focusBlock(
			options.protyle.wysiwyg.element.querySelector(
				`[data-node-id="${selectsElement[0].getAttribute('data-node-id')}"]`
			)
		);
	}
	hideElements(['gutter'], options.protyle);
