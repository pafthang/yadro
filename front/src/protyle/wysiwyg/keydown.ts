...
export const keydown = (protyle: IProtyle, editorElement: HTMLElement) => {
	editorElement.addEventListener('keydown', (event: KeyboardEvent & { target: HTMLElement }) => {
		...
		} else if (!event.repeat && event.code !== '') {
			// 悬浮工具会触发但 code 为空 https://github.com/siyuan-note/siyuan/issues/6573
		...
		if (!['⌘', '⇧', '⌥', '⌃'].includes(Constants.KEYCODELIST[event.keyCode])) {
			if (
				Constants.KEYCODELIST[event.keyCode] === '/' ||
				// 德语
				event.key === '/' ||
				// windows 中文
		...
		if (
			!['Alt', 'Meta', 'Shift', 'Control', 'CapsLock', 'Escape'].includes(event.key) &&
			protyle.options.render.breadcrumb
		) {
				...
				if (event.key === 'ArrowDown') {
					...
					if (nextElement) {
						if (nextElement.getBoundingClientRect().width === 0) {
							...
							nextElement.getAttribute('fold') === '1' &&
					...
					const bottom =
						nextElement.getBoundingClientRect().bottom - protyle.contentElement.getBoundingClientRect().bottom;
					if (bottom > 0) {
					...
					if (previousElement) {
						...
						const top =
							previousElement.getBoundingClientRect().top - protyle.contentElement.getBoundingClientRect().top;
						if (top < 0) {
		...
		if (matchHotKey(window.siyuan.config.keymap.general.enter.custom, event)) {
			...
			if (
				topNodeElement.parentElement.classList.contains('li') &&
				topNodeElement.parentElement.parentElement.classList.contains('list') &&
				topNodeElement.nextElementSibling?.classList.contains('list') &&
				topNodeElement.previousElementSibling.classList.contains('protyle-action')
			) {
		...
		if (!event.altKey && !event.shiftKey && isNotCtrl(event) && (event.key === 'PageUp' || event.key === 'PageDown')) {
			...
			if (centerElement.classList.contains('protyle-wysiwyg')) {
				centerElement = document.elementFromPoint(
					contentRect.x + contentRect.width / 2,
					contentRect.y + contentRect.height / 2 + Constants.SIZE_TOOLBAR_HEIGHT
				);
			}
			...
			if (selectElements.length === 0) {
				...
				if (range.startOffset === 0 && range.startContainer.nodeType === 3) {
					...
					if (
						...
						range.startContainer.parentElement.previousSibling.isSameNode(
							range.startContainer.parentElement.previousElementSibling
						) &&
						range.startContainer.parentElement.previousElementSibling
							.getAttribute('data-type')
							?.indexOf('inline-math') > -1
					) {
		...
		if (!event.altKey && !event.shiftKey && isNotCtrl(event) && !event.isComposing && event.key.indexOf('Arrow') > -1) {
			...
			if (
				event.key === 'ArrowDown' &&
				nodeEditableElement?.textContent.trimRight().substr(position.start).indexOf('\n') === -1 &&
				((tdElement &&
					!tdElement.parentElement.nextElementSibling &&
					nodeElement.getAttribute('data-type') === 'NodeTable' &&
					!getNextBlock(nodeElement)) ||
					...
					(nodeElement.parentElement.getAttribute('data-type') === 'NodeBlockquote' &&
						nodeElement.nextElementSibling.classList.contains('protyle-attr') &&
						!getNextBlock(nodeElement.parentElement)))
			) {
				// 跳出代码块和bq
				...
				if (
					...
					if (
						getSelectionPosition(nodeElement, range).top - protyle.wysiwyg.element.getBoundingClientRect().top < 40 ||
				...
				nodeElement.isSameNode(getLastBlock(protyle.wysiwyg.element.lastElementChild)) &&
				...
				if (
					...
					lastEditElement.textContent.replace(/\n$/, '').length <=
						getSelectionOffset(lastEditElement, undefined, range).end
				) {
					event.stopPropagation();
					event.preventDefault();
					focusByRange(range);
				...
				selectText === '' &&
				event.key === 'ArrowLeft' &&
				nodeElement.isSameNode(getFirstBlock(protyle.wysiwyg.element.firstElementChild))
			) {
		...
		if ((!event.altKey && (event.key === 'Backspace' || event.key === 'Delete')) || matchHotKey('⌃D', event)) {
			...
			if (imgSelectElement) {
				...
				if (event.key === 'Delete' || matchHotKey('⌃D', event)) {
					...
					if (position.end === editElement.textContent.length) {
						...
						position.end === editElement.textContent.length - 1 &&
					...
					if (
						...
						(range.startOffset === 0 ||
							(currentNode &&
								...
								!hasPreviousSibling(currentNode) &&
					...
					if (
						...
						isOnlyMeta(event) &&
						range.startContainer.nodeType === 3 &&
						range.startContainer.textContent.substring(range.startOffset - 1, range.startOffset) === '\n'
					) {
						event.stopPropagation();
						event.preventDefault();
						return;
			...
			if (selectText === '') {
				...
				getContentByInlineHTML(range, (content) => {
					...
					if (nameElement) {
						...
						topElement.lastElementChild.insertAdjacentHTML(
							'afterbegin',
							`<div class="protyle-attr--name"><svg><use xlink:href="#iconN"></use></svg>${content.trim()}</div>`
						);
					}
		...
		if (matchHotKey(window.siyuan.config.keymap.editor.general.alignLeft.custom, event)) {
			...
			if (imgSelectElements.length > 0) {
				alignImgLeft(
					protyle,
					nodeElement,
					Array.from(imgSelectElements),
					nodeElement.getAttribute('data-node-id'),
					nodeElement.outerHTML
				);
			} else {
				let selectElements: HTMLElement[] = Array.from(
		...
		if (
			matchHotKey(window.siyuan.config.keymap.editor.insert.code.custom, event) &&
			...
			protyle.options.toolbar.find((menuItem: IMenuItem) => {
				...
				if (matchHotKey(menuItem.hotkey, event)) {
					...
					if (['a', 'block-ref', 'inline-math', 'inline-memo', 'text'].includes(menuItem.name)) {
						protyle.toolbar.element
							.querySelector(`[data-type="${menuItem.name}"]`)
		...
		if (!event.repeat && matchHotKey(window.siyuan.config.keymap.editor.general.duplicate.custom, event)) {
			event.preventDefault();
			event.stopPropagation();
			let selectsElement: HTMLElement[] = Array.from(
				protyle.wysiwyg.element.querySelectorAll('.protyle-wysiwyg--select')
			);
