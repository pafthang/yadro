...
export class Title {
	...
	constructor(protyle: IProtyle) {
		...
		this.element.innerHTML = `<span aria-label="${
			isMac() ? window.siyuan.languages.gutterTip2 : window.siyuan.languages.gutterTip2.replace('⇧', 'Shift+')
		}" data-position="right" class="protyle-title__icon ariaLabel"><svg><use xlink:href="#iconFile"></use></svg></span>
<div contenteditable="true" spellcheck="${
			window.siyuan.config.editor.spellcheck
		}" class="protyle-title__input" data-tip="${
		...
		this.element.querySelector('.protyle-attr').addEventListener(
			'click',
			(
				event: MouseEvent & {
					target: HTMLElement;
	...
	public render(protyle: IProtyle, response: IWebSocketData) {
		...
		if (response.data.ial.name) {
			nodeAttrHTML += `<div class="protyle-attr--name"><svg><use xlink:href="#iconN"></use></svg>${Lute.EscapeHTMLStr(
				response.data.ial.name
			)}</div>`;
		}
		if (response.data.ial.alias) {
			nodeAttrHTML += `<div class="protyle-attr--alias"><svg><use xlink:href="#iconA"></use></svg>${Lute.EscapeHTMLStr(
				response.data.ial.alias
			)}</div>`;
		}
		if (response.data.ial.memo) {
			nodeAttrHTML += `<div class="protyle-attr--memo b3-tooltips b3-tooltips__sw" aria-label="${Lute.EscapeHTMLStr(
				response.data.ial.memo
			)}"><svg><use xlink:href="#iconM"></use></svg></div>`;
		}
		...
		if (response.data.refCount !== 0) {
			this.element
				.querySelector('.protyle-attr')
				.insertAdjacentHTML(
					'beforeend',
					`<div class="protyle-attr--refcount popover__block" data-defids='${JSON.stringify([
						protyle.block.rootID
					])}' data-id='${JSON.stringify(response.data.refIDs)}'>${response.data.refCount}</div>`
				);
		}
