...
<div class="protyle-background__iconw">
    ...
    <div class="protyle-icons fn__flex-center">
		...
		this.imgElement.addEventListener('mousedown', (event: MouseEvent & { target: HTMLElement }) => {
			...
			const height =
				(this.imgElement.naturalHeight * this.imgElement.clientWidth) / this.imgElement.naturalWidth -
		...
		this.element.addEventListener(getEventName(), (event) => {
			...
			while (target && !target.isEqualNode(this.element)) {
				...
				if (target.tagName === 'IMG' && target.parentElement.classList.contains('protyle-background__img')) {
					...
					if (type === 'confirm') {
						const style = `background-image:url("${this.imgElement.getAttribute('src')}");object-position:${
        ...
        <input class="b3-text-field fn__block" value="${
					this.imgElement.src.startsWith('data:') ? '' : this.imgElement.getAttribute('src')
				}">
</div>
	...
	public render(ial: IObject, rootId: string) {
		...
		if (tags) {
			...
			tags.split(',').forEach((item, index) => {
				html += `<div class="b3-chip b3-chip--middle b3-chip--pointer b3-chip--${
					colors[index % 7]
				}" data-type="open-search">${item}<svg class="b3-chip__close" data-type="remove-tag"><use xlink:href="#iconCloseRound"></use></svg></div>`;
			});
					...
					fetchPost(
						...
						(response) => {
							...
							if (!hasKey && response.data.k) {
								searchHTML =
									`<div class="b3-list-item b3-list-item--narrow"><mark>${response.data.k}</mark></div>` + searchHTML;
							}
