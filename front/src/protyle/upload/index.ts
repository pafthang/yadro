...
const validateFile = (protyle: IProtyle, files: File[]) => {
	...
	for (let iMax = files.length, i = 0; i < iMax; i++) {
		...
		const filename =
			lastIndex === -1 ? file.name : protyle.options.upload.filename(file.name.substr(0, lastIndex)) + fileExt;
...
const genUploadedLabel = (responseText: string, protyle: IProtyle) => {
	...
	if (response.data.errFiles && response.data.errFiles.length > 0) {
		...
		response.data.errFiles.forEach((data: string) => {
			...
			const filename =
				lastIndex === -1 ? data : protyle.options.upload.filename(data.substr(0, lastIndex)) + data.substr(lastIndex);
	...
	keys.forEach((key, index) => {
		...
		if (
			!Constants.SIYUAN_ASSETS_AUDIO.includes(type) &&
			!Constants.SIYUAN_ASSETS_VIDEO.includes(type) &&
			keys.length - 1 !== index
		) {
