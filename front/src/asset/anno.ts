...
export const initAnno = (element: HTMLElement, pdf: any, pdfConfig: any) => {
	...
	pdfConfig.mainContainer.addEventListener('mousedown', (event: MouseEvent) => {
		...
		documentSelf.onmousemove = (moveEvent) => {
			...
			rectResizeElement.setAttribute(
				'style',
				`top:${newTop}px;height:${newHeight}px;left:${newLeft}px;width:${newWidth}px;background-color:${
					moveEvent.altKey ? 'var(--b3-pdf-background1)' : ''
				}`
			);
		};
		documentSelf.onmouseup = () => {
			...
			const coords = getHightlightCoordsByRect(
				pdf,
			...
			if (coords) {
				coords.forEach((item, index) => {
					...
					if (index === 0) {
						...
						copyAnno(
							`${pdf.appConfig.file.replace(location.origin, '').substr(1)}/${rectElement.getAttribute(
								'data-node-id'
							)}`,
							pdf.appConfig.file
								.replace(location.origin, '')
								.substr(8)
								.replace(/-\d{14}-\w{7}.pdf$/, ''),
							pdf
						);
					}
				});
	...
	element.addEventListener('click', (event) => {
		...
		if (typeof event.detail === 'string') {
			window.siyuan.storage[Constants.LOCAL_PDFTHEME].annoColor =
				event.detail === '0'
					? window.siyuan.storage[Constants.LOCAL_PDFTHEME].annoColor || 'var(--b3-pdf-background1)'
		...
		while (target && !target.classList.contains('pdf__outer')) {
			...
			if (target.classList.contains('color__square')) {
				...
				hideToolbar(element);
				copyAnno(
					`${pdf.appConfig.file.replace(location.origin, '').substr(1)}/${rectElement.getAttribute('data-node-id')}`,
					pdf.appConfig.file
						.replace(location.origin, '')
						.substr(8)
						.replace(/-\d{14}-\w{7}.pdf$/, ''),
...
const getHightlightCoordsByRange = (pdf: any, color: string) => {
	...
	Array.from(rangeContents.children).forEach((item) => {
		if (item.tagName === 'BR' && item.previousElementSibling && item.nextElementSibling) {
			...
			if (
				/^[A-Za-z]$/.test(previousText.substring(previousText.length - 2, previousText.length - 1)) &&
	...
	mergeRects(range).forEach(function (r) {
		startSelected.push(
			startViewport
				.convertToPdfPoint(r.left - startPageRect.x, r.top - startPageRect.y)
	...
	if (startIndex !== endIndex) {
		...
		mergeRects(cloneRange).forEach(function (r) {
			endSelected.push(
				endViewport
					.convertToPdfPoint(r.left - endPageRect.x, r.top - endPageRect.y)
...
const getHightlightCoordsByRect = (pdf: any, color: string, rectResizeElement: HTMLElement, type: string) => {
	...
	const startSelected = startViewport
		.convertToPdfPoint(rect.left - startPageRect.x, rect.top - startPageRect.y)
		.concat(startViewport.convertToPdfPoint(rect.right - startPageRect.x, rect.bottom - startPageRect.y));
	...
	const content =
		pdf.appConfig.file
			.replace(location.origin, '')
			.substr(8)
			.replace(/-\d{14}-\w{7}.pdf$/, '') + `-P${startPage.id}-${dayjs().format('YYYYMMDDHHmmss')}`;
	const result = [
		{
			index: startPage.id - 1,
			coords: [startSelected],
	...
	if (endPageElement) {
		...
		if (endIndex !== startIndex) {
			...
			const endSelected = endViewport
				.convertToPdfPoint(rect.left - endPageRect.x, rect.top - endPageRect.y)
				.concat(endViewport.convertToPdfPoint(rect.right - endPageRect.x, rect.bottom - endPageRect.y));
			pages.push({
				index: endPage.id - 1,
				positions: [endSelected]
			});
...
export const getPdfInstance = (element: HTMLElement) => {
	...
	getAllModels().asset.find((item) => {
		if (
			...
			element &&
			item.element &&
			typeof item.element.contains !== 'undefined' &&
...
const showHighlight = (selected: IPdfAnno, pdf: any, hl?: boolean) => {
	...
	let html = `<div class="pdf__rect popover__block" data-node-id="${selected.id}" data-relations="${
		selected.ids || ''
		...
		if (scrollElement) {
			...
			if (currentRect.top < scrollRect.top) {
				scrollElement.scrollTop =
					scrollElement.scrollTop - (scrollRect.top - currentRect.top) - (scrollRect.height - currentRect.height) / 2;
...
const getCaptureCanvas = async (pdfObj: any, pageNumber: number) => {
	...
	const viewport = pdfPage.getViewport({
		scale: 1.5 * pdfObj.pdfViewer.currentScale * window.pdfjsLib.PixelsPerInch.PDF_TO_CSS_UNITS
	});
...
async function getRectImgData(pdfObj: any) {
	...
	const scale = 1.5;
	const captureImageData = captureCanvas
		.getContext('2d')
		.getImageData(
