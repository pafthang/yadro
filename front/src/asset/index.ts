...
export class Asset extends Model {
			...
			(response) => {
				if (response.code !== 1) {
					...
					if (config[this.pdfId]) {
						this.pdfPage = config[this.pdfId].page
							? config[this.pdfId].page + 1
							: config[this.pdfId].pages[0].index + 1;
					} else {
	...
	private render() {
		...
		if (Constants.SIYUAN_ASSETS_IMAGE.includes(type)) {
			this.element.innerHTML = `<div class="asset"><img src="${
				this.path.startsWith('file')
					? this.path
					: document.getElementById('baseURL').getAttribute('href') + '/' + this.path
			}"></div>`;
		} else if (Constants.SIYUAN_ASSETS_AUDIO.includes(type)) {
			this.element.innerHTML = `<div class="asset"><audio controls="controls" src="${
				this.path.startsWith('file')
					? this.path
					: document.getElementById('baseURL').getAttribute('href') + '/' + this.path
			}"></audio></div>`;
		} else if (Constants.SIYUAN_ASSETS_VIDEO.includes(type)) {
			this.element.innerHTML = `<div class="asset"><video controls="controls" src="${
				this.path.startsWith('file')
					? this.path
					: document.getElementById('baseURL').getAttribute('href') + '/' + this.path
			}"></video></div>`;
      ...
      <div id="sidebarContainer">
        <div id="toolbarSidebar">
          <div id="toolbarSidebarLeft">
              <button id="viewThumbnail" class="toolbarButton toggled b3-tooltips b3-tooltips__ne" aria-label="${
								window.siyuan.languages.thumbsTitle
							}">
                <svg><use xlink:href="#iconImage"></use></svg>
              </button>
              <button id="viewOutline" class="toolbarButton b3-tooltips b3-tooltips__ne" aria-label="${
          ...
          <div id="toolbarSidebarRight">
            <div id="outlineOptionsContainer" class="fn__hidden">
              <button id="currentOutlineItem" class="toolbarButton b3-tooltips b3-tooltips__nw" disabled="disabled" aria-label="${
								window.siyuan.languages.focusOutline
							}">
      ...
      <div id="mainContainer">
        <div class="findbar b3-menu fn__hidden doorHanger" id="findbar">
            ...
            <button id="findPrevious" class="toolbarButton findPrevious b3-tooltips b3-tooltips__n" aria-label="${
							window.siyuan.languages.previous
						}">
                <svg><use xlink:href="#iconUp"></use></svg>
            </button>
            <button id="findNext" class="toolbarButton findNext b3-tooltips b3-tooltips__n" aria-label="${
							window.siyuan.languages.next
						}">
        ...
        <div class="pdf__toolbar">
          <div id="toolbarContainer">
            <div id="toolbarViewer">
                <button id="sidebarToggle" class="toolbarButton b3-tooltips b3-tooltips__se" aria-expanded="false" aria-controls="sidebarContainer" aria-label="${
									window.siyuan.languages.toggleSidebarNotification2Title
								} ${updateHotkeyTip('F4')}">
                    <svg><use xlink:href="#iconLayoutRight"></use></svg>
                </button>
                <button id="viewFind" class="toolbarButton b3-tooltips b3-tooltips__se" aria-expanded="false" aria-controls="findbar" aria-label="${
									window.siyuan.languages.search
								} ${updateHotkeyTip('⌘F')}">
                  <svg><use xlink:href="#iconSearch"></use></svg>
                </button>
                <button id="rectAnno" class="toolbarButton b3-tooltips b3-tooltips__se" aria-expanded="false" aria-controls="findbar" aria-label="${
									window.siyuan.languages.rectAnnotation
								} ${updateHotkeyTip('⌘D')}/${updateHotkeyTip('⌥D')}">
                ...
                <button id="secondaryToolbarToggle" class="toolbarButton b3-tooltips b3-tooltips__sw" aria-label="${
      ...
      <div id="dialogContainer">
        ...
        <div class="dialog" id="documentPropertiesDialog">
            ...
            <div class="buttonRow">
              <button id="documentPropertiesClose" class="b3-button"><span>${
