...
export const filterEmoji = (key = '', max?: number) => {
	...
	window.siyuan.emojis.forEach((category, index) => {
		if (!key) {
			html += `<div class="emojis__title" data-type="${index + 1}">${
				window.siyuan.config.lang === 'zh_CN' ? category.title_zh_cn : category.title
			}</div><div style="min-height:${index === 0 ? '28px' : '336px'}" class="emojis__content"${
			...
			if (key) {
				...
				if (
					...
					if (category.id === 'custom') {
						...
						keyHTML += `<button data-unicode="${emoji.unicode}" class="emojis__item ariaLabel" aria-label="${
							window.siyuan.config.lang === 'zh_CN' ? emoji.description_zh_cn : emoji.description
						}">
	...
	if (key) {
		customStore
			.sort((a, b) => {
				...
				if (
					aKeywords[aKeywords.length - 1].toLowerCase().indexOf(key.toLowerCase()) <
					bKeywords[bKeywords.length - 1].toLowerCase().indexOf(key.toLowerCase())
				) {
					return -1;
				}
				return 0;
			...
			.sort((a, b) => {
				...
				if (
					aKeywords[aKeywords.length - 1].toLowerCase().indexOf(key.toLowerCase()) ===
						bKeywords[bKeywords.length - 1].toLowerCase().indexOf(key.toLowerCase()) &&
					aKeywords[aKeywords.length - 1].length < bKeywords[bKeywords.length - 1].length
				) {
					return -1;
				}
				return 0;
			...
			.forEach((item) => {
				html += `<button data-unicode="${item.unicode}" class="emojis__item ariaLabel" aria-label="${
					window.siyuan.config.lang === 'zh_CN' ? item.description_zh_cn : item.description
				}">
${unicode2Emoji(item.unicode, undefined, false, true)}</button>`;
	...
	if (recentEmojis.length > 0) {
		...
		window.siyuan.config.editor.emoji.forEach((emojiUnicode) => {
			...
			if (emoji[0]) {
				recentHTML += `<button data-unicode="${emoji[0].unicode}" class="emojis__item ariaLabel" aria-label="${
					window.siyuan.config.lang === 'zh_CN' ? emoji[0].description_zh_cn : emoji[0].description
				}">
...
<div class="fn__flex">
    ...
    <span class="fn__space"></span>
    <span class="block__icon block__icon--show fn__flex-center b3-tooltips b3-tooltips__sw" aria-label="${
...
<div class="fn__flex">
    <div data-type="0" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.languages.recentEmoji
		}">${unicode2Emoji('2b50')}</div>
    <div data-type="1" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[0][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f527')}</div>
    <div data-type="2" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[1][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f60d')}</div>
    <div data-type="3" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[2][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f433')}</div>
    <div data-type="4" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[3][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f96a')}</div>
    <div data-type="5" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[4][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f3a8')}</div>
    <div data-type="6" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[5][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f3dd-fe0f')}</div>
    <div data-type="7" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[6][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f52e')}</div>
    <div data-type="8" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[7][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('267e-fe0f')}</div>
    <div data-type="9" class="emojis__type ariaLabel" aria-label="${
			window.siyuan.emojis[8][window.siyuan.config.lang === 'zh_CN' ? 'title_zh_cn' : 'title']
		}">${unicode2Emoji('1f6a9')}</div>
</div>
</div>`
	});
	...
	inputElement.addEventListener('keydown', (event: KeyboardEvent) => {
		...
		if (newCurrentElement) {
			...
			if (newCurrentElement.offsetTop - inputHeight < emojisContentElement.scrollTop) {
				...
				newCurrentElement.offsetTop - inputHeight - emojisContentElement.clientHeight + newCurrentElement.clientHeight >
...
export const updateOutlineEmoji = (unicode: string, id: string) => {
	...
	getAllModels().outline.forEach((model) => {
		if (model.blockId === id) {
			model.headerElement.nextElementSibling.firstElementChild.outerHTML = unicode2Emoji(
				unicode || Constants.SIYUAN_IMAGE_FILE,
...
export const updateFileTreeEmoji = (unicode: string, id: string, icon = 'iconFile') => {
	...
	if (dockFile) {
		...
		if (icon === 'iconFile') {
			...
			emojiElement =
				files.element.querySelector(`[data-node-id="${id}"] .b3-list-item__icon`) ||
				files.element.querySelector(`[data-url="${id}"] .b3-list-item__icon`) ||
				files.closeElement.querySelector(`[data-url="${id}"] .b3-list-item__icon`);
		}
	}
	/// #endif
	if (emojiElement) {
			...
			unicode ||
				(icon === 'iconFile'
					? emojiElement.previousElementSibling.classList.contains('fn__hidden')
						? Constants.SIYUAN_IMAGE_FILE
						: Constants.SIYUAN_IMAGE_FOLDER
					: Constants.SIYUAN_IMAGE_NOTE)
		);
	}
	if (icon !== 'iconFile') {
		setNoteBook();
